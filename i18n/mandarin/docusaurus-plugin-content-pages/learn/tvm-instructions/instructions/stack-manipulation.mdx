...
| **`3i`**        | `s[i] POP`                               |                          | Pops the old `s0` value into the old `s[i]`. 将旧的 `s0` 值弹出到旧的 `s[i]` 中。等同于 [`s[i] XCHG0`](#instr-xchg-0i) [`DROP`](#instr-drop) | `18`         |
...
| **`6B`**        | `ONLYX`                                  |                               | Pops integer `i` from the stack, then leaves only the bottom `i` elements. 从堆栈弹出整数 `i`，然后仅保留底部 `i` 个元素。大致等同于 [`DEPTH`](#instr-depth) [`SWAP`](#instr-swap) [`SUB`](#instr-sub) [`DROPX`](#instr-dropx)。 | `18`         |
