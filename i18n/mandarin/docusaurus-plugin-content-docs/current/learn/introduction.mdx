...
**开放网络 The Open Network(TON)** 是一个由多个组成部分构成的去中心化和开放的互联网平台。这些组成部分包括：TON 区块链、TON DNS、TON 存储和 TON 网站。TON 区块链是连接 TON 底层基础设施的核心协议，从而形成更大的 TON 生态系统。 These include: TON Blockchain, TON DNS, TON Storage, and TON Sites. TON Blockchain is the core protocol that connects TON’s underlying infrastructure together to form the greater TON Ecosystem.

TON 致力于实现广泛的跨链互操作性，同时在高度可扩展的安全框架内运行。TON 的设计用于处理每秒数百万笔交易（TPS），目标是最终达到数亿用户的采用。 TON is designed to process millions of transactions per second (TPS), with the goal of eventually reaching hundreds of millions of users moving forward.
...
我们自豪地推出 **TON 区块链课程**，这是一个关于 TON 区块链的全面指南。该课程是为想要学习如何在 TON 区块链上创建智能合约和去中心化应用的开发者设计的。 The course is designed for developers who want to learn how to create smart contracts and decentralized applications on the TON Blockchain.
...
- \[What is Blockchain? What is a Smart Contract? [什么是区块链？什么是智能合约？什么是 Gas？](https://blog.ton.org/what_is_blockchain)
