...
所有 TON 智能合约都在其自己的 TON 虚拟机（TVM）上执行。TVM 基于 _栈原理_ 构建，这使得它高效且易于实现。 TVM is built on the _stack principle_, which makes it efficient and easy to implement.
...
当 TON 链中的某个帐户发生某些事件时，它会引发一个**交易**。最常见的事件是"接收某个消息"，但一般来说还可能有 `tick-tock`、`merge`、`split` 和其他事件。 The most common event is the "arrival of some message", but generally speaking there could be `tick-tock`, `merge`, `split` and other events.

每个交易包含最多 5 个阶段：

1. **Storage phase** - 在此阶段，由于合约占用链状态中的某些空间而积累的存储费用被计算。阅读更多信息 [存储费用](/develop/smart-contracts/fees#storage-fee)。 Read more in [Storage Fees](/develop/smart-contracts/fees#storage-fee).
...
- `c0` — 包含下一个 continuation 或返回 continuation（类似于常规设计中的子例程返回地址）。此值必须是一个 Continuation。 This value must be a Continuation.
- `c1` — 包含备用（返回） continuation ；此值必须是一个 Continuation。 
- `c2` — 包含异常处理程序。此值是一个 Continuation，在触发异常时调用。 This value is a Continuation, invoked whenever an exception is triggered.
- `c3` — 支持寄存器，包含当前字典，本质上是一个包含程序中使用的所有函数的代码的哈希映射。此值必须是一个 Continuation。 This value must be a Continuation. 
- `c4` — 包含持久数据的根，或简单地说，合约的 `data` 部分。此值是一个 Cell。 This value is a Cell.
- `c5` — 包含输出动作。此值是一个 Cell。 This value is a Cell.
- `c7` — 包含临时数据的根。它是一个 Tuple。 It is a Tuple.
...
请注意，由于最大cell深度 `<1024` 以及特别是 c4 和 c5 深度 `<=512` 的限制，一次 tx 中的输出动作数量将受到限制 `<=255`。如果合约需要发送的消息超过这个限制，它可以发送带有请求 `continue_sending` 的消息给自己，并在随后的交易中发送所有必要的消息。 If a contract needs to send more than that, it may send a message with the request `continue_sending` to itself and send all necessary messages in subsequent transactions.
