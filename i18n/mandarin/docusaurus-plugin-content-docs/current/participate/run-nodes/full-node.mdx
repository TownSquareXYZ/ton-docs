...
要安装和管理您自己的节点，请使用TON基金会开发的**MyTonCtrl**开源工具。大多数TON节点都通过**MyTonCtrl**进行了可靠和测试。 The majority of TON Nodes are reliable and tested by **MyTonCtrl**.

[MyTonCtrl](https://github.com/ton-blockchain/mytonctrl)是一个控制台应用程序，它是fift、lite-client和validator-engine-console的方便包装器。它专门开发用于简化在Linux操作系统上的钱包、域和验证者管理任务。 It has been specifically developed to streamline wallet, domain, and validator management tasks on the Linux operating system.
...
These requirements are for a **full node with a validator**. 这些要求适用于**带验证者的全节点**。如果您想运行没有验证者的全节点（例如[liteserver](/participate/run-nodes/full-node#hardware-requirements-1)），您可以使用较低配置的机器。
...
从具有**sudo**权限的**非root**用户帐户下载并运行安装脚本。选择您的Linux发行版： Choose your Linux distributive:
...
- `-c <path>` - 如果您想使用非公共liteservers进行同步。_(不是必需的)_ _(not required)_
...
**mytoncore** 将自动加入选举。它将钱包余额分为两部分，并使用它们作为质押代币参加选举。您也可以手动设置质押代币大小： It divides the wallet balance into two parts and uses them as a stake to participate in the elections. You can also manually set the stake size:
...
When an endpoint is activated in a full node, the node assumes the role of a **Liteserver**. 在全节点中激活端点时，节点将承担**Liteserver**的角色。这种节点类型可以处理并响应来自轻客户端的请求，允许与TON区块链无缝互动。

### 硬件要求

与[验证者](/participate/run-nodes/full-node#hardware-requirements)相比，liteserver模式需要较少的资源。然而，仍建议使用功能强大的机器运行liteserver。 However, it is still recommended to use a powerful machine to run a liteserver.
...
3. 这个文件将帮助您连接到您的liteserver。将位于指定路径的配置文件复制到您的目录以保存它。 Copy the config file located on the specified path to your home to save it.
...
首先，验证您`config.json`文件中指定的Liteserver端口。这个端口会随着每次新安装`MyTonCtrl`而改变。它位于`port`字段： This port changes with each new installation of `MyTonCtrl`. It is located in the `port` field:
...
If you are using a cloud provider, you need to open this port in the firewall settings. 如果您使用的是云提供商，您需要在防火墙设置中打开此端口。例如，如果您使用的是AWS，您需要在[安全组](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/ec2-security-groups.html)中打开端口。

以下是在裸机服务器防火墙中打开端口的示例。

#### 在防火墙中打开端口

我们将使用`ufw`实用程序（[速查表](https://www.cyberciti.biz/faq/ufw-allow-incoming-ssh-connections-from-a-specific-ip-address-subnet-on-ubuntu-debian/)）。您可以使用您喜欢的一个。 You can use the one you prefer.
...
4. Check the firewall status. 检查防火墙状态。运行此命令后，您可以通过检查UFW状态来验证规则是否已成功添加：
...
Sometimes, this process can take up to 24 hours. 有时，这个过程可能需要长达24小时。但是，如果您已经连续几天收到此错误，这意味着您的节点无法通过当前网络连接进行同步。
...
**问：** 有时我们对各种请求得到`block is not applied`或`block is not ready`的回应 - 这正常吗？

**答：** 这是正常的，通常这意味着您试图检索尚未到达您请求的节点的区块。

**问：** 如果比较频繁出现，是否意味着某处存在问题？

**A:** No. **答：** 不。您需要检查mytonctrl中的“Local validator out of sync”值。如果它少于60秒，那么一切正常。 If it's less than 60 seconds, then everything is fine.
...
:::info
将所有密钥和配置从旧节点转移到工作节点并启动它。如果新节点出现问题，原来的节点仍然保留着所有已设置的内容。 In case something goes wrong on the new one, there is still the source where everything is set up.
:::
