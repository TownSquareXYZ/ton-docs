...
For a **Jettons** contract must implement [standard's interface](https://github.com/ton-blockchain/TEPs/blob/master/text/0074-jettons-standard.md) and return data on *get_wallet_data()* or *get_jetton_data()* methods.
...
Account address is generated deterministically from its *initial state*, which includes *initial code* and *initial data* (for wallets, initial data includes public key among other parameters).
