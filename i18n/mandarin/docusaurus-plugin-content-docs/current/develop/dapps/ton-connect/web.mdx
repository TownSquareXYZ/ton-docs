...
安装包后，您应创建一个`manifest.json`文件给您的应用程序。有关如何创建manifest.json文件的更多信息，请查看[这里](/develop/dapps/ton-connect/manifest)。 More information on how to create a manifest.json file can be found [here](/develop/dapps/ton-connect/manifest).
...
TonConnect UI提供了一个用户在使用各种应用时应该熟悉且可识别的界面。然而，应用开发者可以对这个接口进行更改，以保持与应用接口的一致性。 However, the app developer can make changes to this interface to keep it consistent with the app interface.
...
The principle located in Payment Processing (using tonweb). 位于支付处理（使用tonweb）中的原则。[查看更多](/develop/dapps/asset-processing/#checking-contracts-transactions)
