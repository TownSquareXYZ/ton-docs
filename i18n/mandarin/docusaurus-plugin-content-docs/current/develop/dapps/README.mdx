import Button from '@site/src/components/button'

# Getting Started

Before diving into DApps, make sure you understand the underlying blockchain principles. 在深入研究DApps之前，请确保您理解底层的区块链原理。我们的[The Open Network](/learn/introduction)和[Blockchain of Blockchains](/learn/overviews/ton-blockchain)文章可能会对您有所帮助。

TON DApps are applications without backend, which interact with the blockchain. In most cases, they interact with custom [smart contracts](/develop/smart-contracts/); this documentation provides ways to process standard assets available in TON, both as an example and for quicker DApps development.

DApps generally can be written in any programming language having SDK for TON. Actually, the most frequent choice is a website, followed by Telegram mini-apps.

<Button href="/develop/dapps/telegram-apps/" colorType={'primary'} sizeType={'sm'}>构建小程序</Button>
<Button href="/develop/dapps/apis/sdk"
colorType="secondary" sizeType={'sm'}>
选择SDK
</Button>

## TON 课程：DApps

[TON区块链课程](https://stepik.org/course/176754/)是关于TON区块链开发的全面指南。

Module 5 and 6 completely covers DApps development. 第5和6模块完全涵盖了DApps开发。您将学习如何创建DApp，如何使用TON Connect，如何使用SDK以及如何与区块链交互。

<Button href="https://stepik.org/course/176754/" colorType={'primary'} sizeType={'sm'}>查看TON区块链课程</Button>

## 基础工具和资源

以下是您在DApp开发过程中需要的一些关键资源：

1. [开发者钱包](/participate/wallets/apps)
2. [区块链浏览器](/participate/explorers)
3. [API文档](/develop/dapps/apis/)
4. [各种语言的SDK](/develop/dapps/apis/sdk)
5. [使用测试网](/develop/smart-contracts/environment/testnet)
6. [TON解冻器](https://unfreezer.ton.org/)

### 资产管理

Working with assets? 处理资产？这些指南涵盖了基础内容：

- [支付处理](/develop/dapps/asset-processing/)
- [代币（Jetton）处理](/develop/dapps/asset-processing/jettons)
- [处理NFTs](/develop/dapps/asset-processing/nft)
- [解析元数据](/develop/dapps/asset-processing/metadata)

### DeFi 入门

Interested in decentralized finance (DeFi)? 对去中心化金融（DeFi）感兴趣？以下是处理不同类型资产的方法：

- [理解Toncoin](/develop/dapps/defi/coins)
- [代币：Jettons和NFTs](/develop/dapps/defi/tokens)
- [TON支付](/develop/dapps/defi/ton-payments)
- [设置订阅](/develop/dapps/defi/subscriptions)

## 教程和示例

### DeFi 基础

- 创建您的第一个代币：[铸造您的第一个Jetton](/develop/dapps/tutorials/jetton-minter)
- 逐步操作：[NFT系列铸造](/develop/dapps/tutorials/collection-minting)

### 针对特定语言的指南

#### JavaScript

- [支付流程](https://github.com/toncenter/examples)
- [TON桥接](https://github.com/ton-blockchain/bridge)
- [网络钱包](https://github.com/toncenter/ton-wallet)
- [饺子销售机器人](/develop/dapps/tutorials/accept-payments-in-a-telegram-bot-js)

#### Python

- [示例项目](https://github.com/psylopunk/pytonlib/tree/main/examples)
- [商店前端机器人](/develop/dapps/tutorials/accept-payments-in-a-telegram-bot)

#### Go

- [示例](https://github.com/xssnick/tonutils-go/tree/master/example)

### 高级主题

- [零知识证明](/develop/dapps/tutorials/simple-zk-on-ton)

### 钱包示例

- [桌面标准钱包（C++和Qt）](https://github.com/ton-blockchain/wallet-desktop)
- [Android标准钱包（Java）](https://github.com/ton-blockchain/wallet-android)
- [iOS标准钱包（Swift）](https://github.com/ton-blockchain/wallet-ios)
- [TonLib CLI（C++）](https://github.com/ton-blockchain/ton/blob/master/tonlib/tonlib/tonlib-cli.cpp)

## 👨‍💻 贡献

Missing some crucial material? You can either write a tutorial yourself or describe the problem for the community.

<Button href="/contribute/participate" colorType="primary" sizeType={'sm'}>贡献</Button>
  <Button href="https://github.com/ton-community/ton-docs/issues/new?assignees=&labels=feature+%3Asparkles%3A%2Ccontent+%3Afountain_pen%3A&template=suggest_tutorial.yaml&title=Suggest+a+tutorial" colorType={'secondary'} sizeType={'sm'}>建议教程</Button>
