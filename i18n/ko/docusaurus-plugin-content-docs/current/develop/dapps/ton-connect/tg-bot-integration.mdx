...
이 튜토리얼에서는 자바스크립트 톤 커넥트 SDK를 사용해 톤 커넥트 2.0 인증을 지원하는 샘플 텔레그램 봇을 만들어 보겠습니다.
...
먼저, 노드 JS 프로젝트를 생성해야 합니다. 여기서는 타입스크립트와 [node-telegram-bot-api](https://www.npmjs.com/package/node-telegram-bot-api) 라이브러리를 사용합니다(다른 적합한 라이브러리도 선택할 수 있습니다). 또한, QR코드 생성을 위해 [qrcode](https://www.npmjs.com/package/qrcode) 라이브러리를 사용하겠지만, 다른 라이브러리로 대체할 수 있습니다.
...
이제 `src` 디렉터리 내에 엔트리포인트 파일 `main.ts`를 만들 수 있습니다:
...
이미 `@tonconnect/sdk`를 설치했으므로 이를 가져와서 시작하면 됩니다.
...
더 나은 UX를 위해, `콜백 쿼리`와 `인라인 키보드` 텔레그램 기능을 사용하려고 합니다. 익숙하지 않으시다면, [여기](https://core.telegram.org/bots/api#callbackquery)에서 자세히 읽어보실 수 있습니다.
...
이제 글로벌 `콜백_쿼리` 핸들러를 추가하고 거기에 `온선택지클릭`을 등록합니다:
...
이제 `main.ts`에 `conenct-wallet-menu.ts` 가져오기를 추가해야 합니다.
...
여기에서는 첫 화면(유니버설 QR 화면)에 @wallet에 대한 별도의 버튼을 추가합니다. 이제 
...
사용자가 @wallet으로 QR을 스캔할 때 리디렉션이 필요하지 않고, 동시에 사용자가 버튼 링크를 사용하여 @wallet을 연결할 때 봇으로 다시 리디렉션해야 하기 때문에 QR과 버튼 링크(`qrLink` 및 `buttonLink`)에 다른 링크( 
)를 배치합니다.
