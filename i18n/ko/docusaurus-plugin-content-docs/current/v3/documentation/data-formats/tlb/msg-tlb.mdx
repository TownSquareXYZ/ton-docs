...
| info      | [CommonMsgInfo](#commonmsginfo) | 필수 | Contains detailed message properties such as destination and value. It is always stored in the message's root cell.             |   |
| init      | [StateInit](#stateinit-tl-b)    | 선택 | The general structure is used in TON to initialize new contracts. It can be placed in either a cell reference or the root cell. |   |
| body      | X                               | 필수 | Message payload. It can be placed in either a cell reference or the root cell.                                                  |   |
...
| created_at     | uint32                                    | 필수       | UNIX timestamp indicating when the message was created.                                                                         |
...
| Structure          | 유형                                   | Required | Description                                                                                                                     |
| ------------------ | ------------------------------------ | -------- | ------------------------------------------------------------------------------------------------------------------------------- |
| ext_in_msg_info$10 | 생성자                                  | Required | The `$10` tag indicates that the `CommonMsgInfo` begins with `10` bits in serialization, denoting an external incoming message. |
...
| import_fee         | [VarUInteger 16](#varuinteger-n)     | 필수       | The fee for executing and delivering the message.                                                                               |
...
| created_at          | uint32                               | 필수       | UNIX timestamp representing when the message was created.                                                                       |
...
| library     | HashmapE 256 SimpleLib\* | 선택       | Currently, `StateInit` is used without libraries.                                                                                                                                                                                                                                                                                                                        |
...
| 구조           | 유형  | Required | Description                                                                                                                                   |
| ------------ | --- | -------- | --------------------------------------------------------------------------------------------------------------------------------------------- |
| addr_none$00 | 생성자 | 필수       | The `$00` tag in serialization indicates that `MsgAddressExt` starts with `00` bits. This signifies that the entire external address is `00`. |
...
| 구조               | 유형         | Required | Description                                                                                                            |
| ---------------- | ---------- | -------- | ---------------------------------------------------------------------------------------------------------------------- |
| addr_extern$01   | 생성자        | Required | The `$01` tag in serialization indicates that `MsgAddressExt` starts with `01` bits and describes an external address. |
| len              | ## 9       | Required | Represents an unsigned N-bit number, similar to `uintN`.                                                               |
| external_address | (bits len) | 필수       | A bitstring address, where the length of the address equals the previously defined `len`.                              |
...
| 구조           | 유형        | Required | Description                                                                                                                                                                                                                                                |
| ------------ | --------- | -------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| addr_std$10  | 생성자       | 필수       | The `$10` tag in serialization indicates that `MsgAddressInt` starts with `10` bits and describes an internal address.                                                                                                                                     |
| anycast      | Anycast\* | 선택       | Represents additional address data. Currently not used in ordinary internal messages.                                                                                                                                                                      |
...
| address      | (bits256) | 필수       | The smart contract account ID number.                                                                                                                                                                                                                      |
...
| 구조           | Type        | Required  | 설명                                                                                                                                                  |
| ------------ | ----------- | --------- | --------------------------------------------------------------------------------------------------------------------------------------------------- |
| addr_var$11  | Constructor | 필수        | The `$11` tag indicates that in the serialization of `MsgAddressInt`, the address starts with `11` bits, representing an internal contract address. |
| anycast      | Anycast\*   | 선택        | Additional address data is not currently used in regular internal messages.                                                                         |
...
| address      | (bits256)   | 필수        | The payload address (this could be the account ID).                                                                                                 |
...
| Structure    | 유형                      | Required | 설명                                                                                                                                  |
| ------------ | ----------------------- | -------- | ----------------------------------------------------------------------------------------------------------------------------------- |
| currencies$_ | 생성자                     | Required | The `$_` empty tag indicates that no bits are added at the beginning during the serialization of `CurrencyCollection`.              |
| grams        | (VarUInteger 16)        | 필수       | Represents the message value, expressed in nanoTons.                                                                                |
| other        | ExtraCurrencyCollection | 선택       | ExtraCurrencyCollection is a dictionary intended for additional, typically empty currencies.                                        |
...
| 구조         | 유형                | Required | Description                                                                                                                 |
| ---------- | ----------------- | -------- | --------------------------------------------------------------------------------------------------------------------------- |
| var_uint$_ | 생성자               | Required | The `var_uint$_` is an empty tag, indicating that no bits are added at the beginning of `CurrencyCollection` serialization. |
| len        | uintN             | 필수       | Specifies the bit-length used to encode the next value.                                                                     |
| value      | (uint (len \* 8)) | 선택       | An optional unsigned integer encoded in `(len * 8)` bits.                                                                   |
