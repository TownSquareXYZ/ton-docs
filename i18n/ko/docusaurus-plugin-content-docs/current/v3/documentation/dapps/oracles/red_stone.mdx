...
아래의 함수 매개변수에서 각 `feed_id`는 문자열이 `int`로 인코딩된 것을 의미합니다. 즉, 문자열의 각 문자의 16진수 값으로 구성된 값입니다. 예를 들어:
...
`feed_id`, `value`, 그리고 저장된 값의 `timestamp`를 포함합니다. 
이 메시지는 발신자에서 가져와서 처리하거나 발신자의 스토리지에 저장될 수 있습니다.
...
- RedStone 페이로드는 데이터 피드를 명시적으로 정의하여 가져와야 합니다.
  이는 **하나의 데이터 포인트**가 **하나의 데이터 패키지**에 속하게 됩니다.
