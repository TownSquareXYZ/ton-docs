...
- **확장성**: 기존의 채굴 프로세스는 많은 사용자에게 토큰을 배포할 때 리소스 집약적이고 비용이 많이 들 수 있습니다.
...
- 루트 `merkle_hash`를 계산합니다.

2. Deploy a jetton master contract:

- 컨트랙트의 저장소에 `merkle_hash`를 포함합니다.
...
- 컴파일된 파일은 '빌드/크립토/민트리스-프루프-제너레이터'로 저장됩니다.
