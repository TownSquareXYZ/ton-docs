...
| 이름              | 유형     | 설명                                                                                                                                                                                                                                                               |
| --------------- | ------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `총공급`           | `int`  | The total number of issued jettons measured in indivisible units.                                                                                                                                                                                |
| `mintable`      | `int`  | Indicates whether new jettons can be minted (-1 for mintable, 0 for non-mintable).                                                                                                                                            |
| `admin_address` | `슬라이스` |                                                                                                                                                                                                                                                                  |
| `제톤_콘텐츠`        | `셀`    | Data formatted according to [TEP-64](https://github.com/ton-blockchain/TEPs/blob/master/text/0064-token-data-standard.md), check [jetton metadata parsing page](/v3/guidelines/dapps/asset-processing/nft-processing/metadata-parsing) for more. |
| `제톤_지갑_코드`      | `셀`    |                                                                                                                                                                                                                                                                  |
...
발신자 -> 발신자의 제톤 지갑\`. 전송_ 메시지에는 다음 데이터가 포함됩니다:
...
| `금액`       | 코인     | Tthe total `ton coin` amount sent with the message.                                                                                                                                    |
| `대상`       | 주소     | The address of the new owner of the jettons                                                                                                                                                            |
| `응답_대상`    | 주소     | The wallet address used to return remaining Toncoin through the excesses message.                                                                                                      |
| 커스텀 페이로드\` | 어쩌면 셀  | Always at least 1 bit in size. Custom data used by either the sender or receiver jetton wallet for internal logic.                                                     |
| `전송_톤_금액`  | 코인     | Must be greater than 0 to send a transfer notification message with a `forward payload`. It is **part of `amount` value** and **must be lesser than `amount`**.        |
| `전송_페이로드`  | 어쩌면 셀  | Always at least 1 bit in size. If the first 32 bits equal 0x0, it is a simple message.                                                                                 |
...
이 송금에는 **수수료**와 **송금 알림 메시지**를 위해 약간의 톤 코인이 필요합니다.
...
마지막으로 `Excess 0xd53276db` 메시지를 검색하려면 `응답 대상`을 설정해야 합니다.
...
2. 메타데이터](/v3/guidelines/dapps/asset-processing/jettons#retrieving-jetton-data) 가져오기.
...
Authors are *[kosrk](https://github.com/kosrk)*, *[krigga](https://github.com/krigga)*, *[EmelyanenkoK](https://github.com/EmelyanenkoK/)* and *[tolya-yanot](https://github.com/tolya-yanot/)*.
