...
TON Connect를 사용할 때는 다양한 트랜잭션에서 사용되는 Payload의 Message Body를 구성해야 합니다. 이 페이지에서는 TON Connect SDK에서 사용할 수 있는 가장 관련성 높은 payload 예제들을 확인할 수 있습니다.

:::info
TON Connect 연결 생성의 기본 사항을 학습하는 것이 좋습니다. [통합 매뉴얼](/v3/guidelines/ton-connect/guidelines/integration-with-javascript-sdk)에서 자세히 알아보세요.
:::

## 셀과 메시지 직렬화

메시지 구축에 들어가기 전에, 메시지 본문을 구성하는 셀의 개념부터 소개하겠습니다.

### 셀이란 무엇인가?

셀은 TON 블록체인의 기본 데이터 구조입니다. 최대 `1023` 비트를 저장할 수 있고 다른 셀에 대한 최대 `4`개의 참조를 보유할 수 있어 더 복잡한 데이터 구조를 저장할 수 있습니다.
`@ton/core`와 `@ton-community/assets-sdk` 같은 라이브러리들이 셀을 효율적으로 다루는 방법을 제공합니다.

셀에 대해 [여기](/v3/concepts/dive-into-ton/ton-blockchain/cells-as-data-storage)에서 더 자세히 알아볼 수 있습니다.

### 셀 생성하기

셀을 만들려면 `beginCell()` 함수를 사용합니다. 셀이 "열려있는" 동안 `store...()` 함수로 다양한 데이터 타입을 저장할 수 있습니다.
작업이 끝나면 `endCell()` 함수로 셀을 닫습니다.
...
셀에서 데이터를 읽거나 파싱하려면 `beginParse()` 함수를 호출합니다. 비슷한 `load...()` 함수를 사용해 저장된 순서대로 데이터를 읽습니다:
...
각 셀은 1023비트로 제한됩니다. 이를 초과하면 오류가 발생합니다:
...
더 많은 데이터를 저장하기 위해 셀은 다른 셀을 참조할 수 있습니다. storeRef() 함수를 사용하여 중첩된 셀을 만들 수 있습니다:
...
참조된(중첩된) 셀을 로드하려면 `loadRef()`를 사용합니다:
...
셀은 선택적 값(null일 수 있는)을 저장할 수 있습니다. 이는 `storeMaybe...()` 함수를 사용하여 저장됩니다:
...
해당하는 loadMaybe...() 함수를 사용하여 선택적 값을 파싱할 수 있습니다. 반환된 값은 null일 수 있으므로 null 체크를 잊지 마세요!
...
### 더 간단한 직렬화와 역직렬화를 위한 assets sdk 사용하기

셀을 수동으로 다루는 것은 번거로울 수 있으므로, `@ton-community/assets-sdk`는 메시지를 직렬화하고 역직렬화하는 편리한 메서드를 제공합니다.

`@ton-community/assets-sdk`를 사용하면 더 읽기 쉽고 오류가 적습니다.
...
## TON Connect JS SDK 예제들

### 트랜잭션 템플릿

개발자가 어떤 수준의 작업을 해결하든, 일반적으로 `@tonconnect/sdk` 또는 `@tonconnect/ui`의 connector 엔티티를 사용해야 합니다.
`@tonconnect/sdk`와 `@tonconnect/ui`를 기반으로 만든 예제입니다:
...
TON Connect SDK는 메시지 전송을 위한 래퍼를 포함하고 있어, 두 지갑 간의 일반적인 Toncoin 전송을 payload가 없는 기본 트랜잭션으로 쉽게 준비할 수 있습니다.

TON Connect JS SDK를 사용한 일반적인 TON 전송은 다음과 같이 실행할 수 있습니다:
...
:::tip
[TON 스마트 컨트랙트 주소](/v3/documentation/smart-contracts/addresses)에 대해 자세히 알아보세요.
:::

특정 커스텀 트랜잭션의 경우, 특정 payload를 정의해야 합니다.

### 코멘트가 있는 전송

가장 간단한 예제는 코멘트가 있는 payload를 추가하는 것입니다. [이 페이지](/v3/documentation/smart-contracts/message-management/internal-messages#simple-message-with-comment)에서 자세한 내용을 확인하세요.
트랜잭션 전에 [@ton/ton](https://github.com/ton-org/ton) JavaScript 라이브러리를 통해 `body` [cell](/v3/documentation/data-formats/tlb/cell-boc)을 준비해야 합니다.
...
Jetton 전송을 위한 `body`는 ([TEP-74](https://github.com/ton-blockchain/TEPs/blob/master/text/0074-jettons-standard.md#1-transfer)) 표준을 기반으로 합니다. 서로 다른 토큰 간에 소수점 자릿수가 다를 수 있다는 점에 유의하세요: 예를 들어, USDT는 6자리(1 USDT = 1 _ 10\*\*9)를 사용하고, TON은 9자리(1 TON = 1 _ 10\*\*9)를 사용합니다.

:::info
`ton-connect`와 함께 `assets-sdk` 라이브러리의 기본 제공 메서드를 사용할 수 있습니다.
:::
...
다음으로 이 body를 sender의 jettonWalletContract로 보내는 트랜잭션을 실행합니다:
...
- `validUntil` - 메시지가 유효한 UNIX 시간
- `jettonWalletAddress` - JettonMaster와 Wallet 컨트랙트를 기반으로 정의된 JettonWallet 주소
- `balance` - 나노톤 단위의 Toncoin 금액(가스비 지불용)
- `body` - jettonContract용 payload

<details>
    <summary>Jetton Wallet 상태 초기화 및 주소 준비 예제</summary>
...
:::tip
참고: 브라우저의 경우 `Buffer`용 폴리필을 설정해야 합니다.
:::
...
또는 내장된 메서드가 있는 Jetton Contract를 사용할 수 있습니다:
...
### Jetton Transfer with Comment (코멘트가 있는 Jetton 전송)

<Tabs groupId="Jetton Transfer with Comment">
<TabItem value="@ton/ton" label="@ton/ton">

일반적인 전송 `body`에 추가로 코멘트를 직렬화하고 이를 `forwardPayload`에 포함시켜야 합니다. 토큰마다 소수점 자릿수가 다를 수 있다는 점에 유의하세요(예: USDT는 6자리, TON은 9자리).
...
Jetton Burn의 `body`는 [TEP-74](https://github.com/ton-blockchain/TEPs/blob/master/text/0074-jettons-standard.md#1-transfer) 표준을 기반으로 합니다. 주의할 점은 토큰마다 소수점 자릿수가 다를 수 있다는 것입니다. 예를 들어, USDT는 소수점 6자리(1 USDT = 1 _ 10\*\*6)를 사용하며, TON은 소수점 9자리(1 TON = 1 _ 10\*\*9)를 사용합니다.
...
- `jettonWalletAddress` - JettonMaster와 Wallet 컨트랙트를 기반으로 정의된 Jetton Wallet 컨트랙트 주소
- `amount` - 나노톤 단위의 Toncoin 금액(가스비 지불용)
- `body` - `burn#595f07bc` op 코드가 있는 jetton wallet용 payload
...
<TabItem value="@ton/ton" label="@ton/ton">
일반적으로 메시지 `body`는 다음과 같이 구성되어야 합니다:
...
`WALLET_DST` - Address - NFT 초과분을 받기 위한 초기 NFT 소유자의 주소
`NEW_OWNER_WALLET`의 새 소유자에게 `NFTitem`을 전송합니다.
...
- `NFTitem` - Address - 새 소유자 `NEW_OWNER_WALLET`에게 전송하고자 하는 NFT 아이템 스마트 컨트랙트의 주소
- `balance` - 나노톤 단위의 Toncoin 금액(가스비 지불용)
- `body` - NFT 컨트랙트용 payload
...
또는 내장된 메서드가 있는 NFT Contract를 사용할 수 있습니다:
...
### NFT Sale (GetGems) (NFT 판매)

여기서는 [nft-fixprice-sale-v3r3](https://github.com/getgems-io/nft-contracts/blob/main/packages/contracts/sources/nft-fixprice-sale-v3r3.fc) 컨트랙트에 따라 GetGems 마켓플레이스에서의 판매를 위한 메시지와 트랜잭션 준비 예제를 보여줍니다.

GetGems Sale Contract에 NFT를 올리기 위해서는 NFT를 특별한 NFT Sale Contract로 전송할 특수한 메시지 본문 `transferNftBody`를 준비해야 합니다.
...
메시지에는 많은 단계가 필요하므로 전체 알고리즘은 방대하며 다음에서 찾을 수 있습니다:

<details>
    <summary>NFT Sale 메시지 본문 생성을 위한 전체 알고리즘 보기</summary>
...
준비된 `transferNftBody`는 적어도 `1.08` TON과 함께 NFT Item Contract로 전송되어야 하며, 이는 성공적인 처리를 위해 필요합니다. 초과분은 발신자의 지갑으로 반환됩니다.
...
### NFT Buy (GetGems) (NFT 구매)

<Tabs groupId="NFT Buy tabs">
<TabItem value="@ton/ton" label="@ton/ton">

[nft-fixprice-sale-v3r3](https://github.com/getgems-io/nft-contracts/blob/main/packages/contracts/sources/nft-fixprice-sale-v3r3.fc) 판매 컨트랙트에서 NFT를 구매하는 과정은 payload 없이 일반적인 전송으로 수행될 수 있습니다. 유일하게 중요한 것은 다음과 같이 계산되는 정확한 TON 금액입니다:
...
Python 예제들은 [PyTonConnect](https://github.com/XaBbl4/pytonconnect)와 [pytoniq](https://github.com/yungwine/pytoniq)를 사용합니다.
...
:::tip
예제 [소스코드](https://github.com/yungwine/ton-connect-examples/blob/master/main.py)를 참조하세요.
:::

### Regular TON Transfer (일반 TON 전송)
...
### Transfer With Comment (코멘트가 있는 전송)

먼저 다음 함수를 통해 코멘트가 있는 메시지를 구현합니다:
...
### Jetton Transfer (Jetton 전송)

Jetton 전송 트랜잭션을 구축하는 함수의 예시입니다. 토큰마다 소수점 자릿수가 다를 수 있다는 점에 유의하세요: 예를 들어, USDT는 6자리(1 USDT = 1 _ 10\*\*9), TON은 9자리(1 TON = 1 _ 10\*\*9)를 사용합니다.
...
### Jetton Burn (Jetton 소각)

Jetton 소각 트랜잭션을 구축하는 함수의 예시입니다. 토큰마다 소수점 자릿수가 다를 수 있음을 기억하세요.
...
GetGems 마켓플레이스에서 판매하기 위한 메시지와 트랜잭션 준비의 예시입니다. [nft-fixprice-sale-v3r3](https://github.com/getgems-io/nft-contracts/blob/main/packages/contracts/sources/nft-fixprice-sale-v3r3.fc) 컨트랙트를 따릅니다.

특별한 NFT Sale Contract에 NFT를 전송할 특수 메시지 본문 `transferNftBody`를 준비해야 합니다.

<details>
<summary>NFT Sale Body 생성 예시</summary>
...
[nft-fixprice-sale-v3r3](https://github.com/getgems-io/nft-contracts/blob/main/packages/contracts/sources/nft-fixprice-sale-v3r3.fc) 판매 컨트랙트의 NFT 구매 과정은 payload 없이 일반 전송으로 진행할 수 있습니다. 중요한 것은 `buyAmount = Nftprice TON + 1.0 TON` 공식으로 계산되는 정확한 TON 금액뿐입니다.
...
Go 예제는 [tonconnect](https://github.com/cameo-engineering/tonconnect)와 [tonutils-go](https://github.com/xssnick/tonutils-go)를 사용합니다.
...
:::tip
[tonconnect](https://github.com/cameo-engineering/tonconnect/blob/master/examples/basic/main.go)와 [tonutils-go](https://github.com/xssnick/tonutils-go?tab=readme-ov-file#how-to-use) 예제를 참조하세요.
:::

여기서는 tonconnect 세션을 생성하고 메시지로 구성된 트랜잭션을 보내는 방법을 확인할 수 있습니다.
...
이후 예제에서는 메시지와 트랜잭션만 생성합니다.

### Regular TON Transfer (일반 TON 전송)

일반 TON 전송 메시지를 구축하는 함수의 예시:
...
Jetton 전송 메시지 예시. 토큰마다 소수점 자릿수가 다를 수 있음을 유의하세요(USDT: 6자리, TON: 9자리):
...
GetGems Sale Contract에 NFT를 올리기 위해서는 특별한 NFT Sale Contract로 NFT를 전송할 `transferNftBody` 특수 메시지 본문을 준비해야 합니다:
...
메시지에는 많은 단계가 필요하므로 전체 알고리즘을 보려면 다음을 확인하세요:

<details>
    <summary>NFT Sale 메시지 본문 생성을 위한 전체 알고리즘</summary>
...
[nft-fixprice-sale-v3r3](https://github.com/getgems-io/nft-contracts/blob/main/packages/contracts/sources/nft-fixprice-sale-v3r3.fc) 판매 컨트랙트의 NFT 구매 과정은 payload 없이 일반 전송으로 수행할 수 있습니다. 중요한 것은 다음 공식으로 계산되는 정확한 TON 금액입니다: 
`buyAmount = Nftprice TON + 1.0 TON`.
...
- JavaScript 예제 제공: [@aSpite](https://t.me/aspite)
- Python 예제 제공: [@yunwine](https://t.me/yungwine)
- Go 예제 제공: [@gleb498](https://t.me/gleb498)

## 참고 자료

- [TON Connect SDKs](/v3/guidelines/ton-connect/guidelines/developers)
- [TON Connect - 메시지 전송](/v3/guidelines/ton-connect/guidelines/sending-messages)
- [스마트 컨트랙트 개발 - 메시지 전송 (Low Level)](/v3/documentation/smart-contracts/message-management/sending-messages)
- [TON Jetton 처리](/v3/guidelines/dapps/asset-processing/jettons)
- [TON의 NFT 처리](/v3/guidelines/dapps/asset-processing/nft-processing/nfts)
