...
| info      | [CommonMsgInfo](#commonmsginfo) | 必需      | Contains detailed message properties such as destination and value. It is always stored in the message's root cell.             |   |
| init      | [StateInit](#stateinit-tl-b)    | 可选      | The general structure is used in TON to initialize new contracts. It can be placed in either a cell reference or the root cell. |   |
| body      | X                               | 必需      | Message payload. It can be placed in either a cell reference or the root cell.                                                  |   |
...
| created_at     | uint32                                    | 必需       | UNIX timestamp indicating when the message was created.                                                                         |
...
| ------------------- | ------------------------------------ | -------- | ------------------------------------------------------------------------------------------------------------------------------- |
| ext_out_msg_info$10 | Constructor                          | Required | The `$10` tag indicates that the `CommonMsgInfo` begins with `10` bits in serialization, denoting an external incoming message. |
...
| 结构           | 类型           | Required | Description                                                                                                                                                                                                                                                |
| ------------ | ------------ | -------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| addr_std$10  | 构造函数         | Required | The `$10` tag in serialization indicates that `MsgAddressInt` starts with `10` bits and describes an internal address.                                                                                                                                     |
...
| Structure    | Type        | Required  | 描述                                                                                                                                                                                         |
| ------------ | ----------- | --------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| addr_std$10  | Constructor | 必需        | The `$11` tag indicates that in the serialization of `MsgAddressInt`, the address starts with `11` bits, representing an internal contract address.                                        |
| anycast      | Anycast\*   | 可选        | Additional address data is not currently used in regular internal messages.                                                                                                                |
| workchain_id | int8        | Required  | Same as `uintN` – represents an unsigned N-bit number.                                                                                                                                     |
| address      | (bits256)   | Required  | The WorkChain is where the smart contract for the destination address is located. Currently, this is always set to zero.                                                                   |
| address      | (bits256)   | 必需        | The payload address (this could be the account ID).                                                                                                                                        |
