...
| AWS            | `i4i.8xlarge`                   | `32 vCPUs`              | `256GB` | `2 x 3,750 AWS Nitro SSD (fixed)` | `Up to 25 Gbps` | Bind an Elastic IP                      | `64 TB/ماه`   |
...
- `-d` - **mytonctrl** یک [dump](https://dump.ton.org/) از آخرین وضعیت بلاکچین دانلود خواهد کرد.
  این کار زمان همگام‌سازی را به شدت کاهش می‌دهد.
- `-c <path>` - اگر می‌خواهید برای همگام‌سازی از liteservers عمومی استفاده نکنید. _(الزامی نیست)_
...
برای تمام انواع نودها، بخش **وضعیت اعتبارسنج محلی** باید ظاهر شود.
در غیر این صورت، [قسمت رفع اشکال](/v3/guidelines/nodes/nodes-troubleshooting#status-command-displays-without-local-node-section) و [لاگ‌های نود را بررسی کنید](/v3/guidelines/nodes/running-nodes/full-node#check-the-node-logs).
:::
