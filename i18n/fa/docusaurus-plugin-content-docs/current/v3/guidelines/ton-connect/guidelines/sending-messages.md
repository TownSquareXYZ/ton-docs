...
- چگونه از DApp به بلاکچین پیام ارسال کنیم
- چگونه چندین پیام را در یک تراکنش ارسال کنیم
- چگونه با استفاده از TON Connect قرارداد را واکشی کنیم

## صفحه زمین بازی

ما از [TON Connect SDK](https://github.com/ton-connect/sdk/tree/main/packages/sdk) سطح پایین برای جاوااسکریپت استفاده خواهیم کرد. ما در کنسول مرورگر بر روی صفحه‌ای که کیف پول قبلاً متصل شده است آزمایش خواهیم کرد. این صفحه نمونه است:
...
می‌توانید آن را آزادانه در کنسول مرورگر خود جای‌گذاری کرده و اجرا کنید.
...
- کیف‌پول‌های استاندارد ([v3](/v3/documentation/smart-contracts/contracts-specs/wallet-contracts#wallet-v3)/[v4](/v3/documentation/smart-contracts/contracts-specs/wallet-contracts#wallet-v4)): ۴ پیام خروجی؛
- کیف‌پول‌های فشار بالا: ۲۵۵ پیام خروجی (نزدیک به محدودیت‌های بلاکچین).
...
خواهید دید که این دستور چیزی را به کنسول چاپ نمی‌کند، `null` یا `undefined`، همان‌طور که عملکردهای بازگشتی هیچ چیز انجام نمی‌دهند. این بدان معناست که `connector.sendTransaction` بلافاصله خارج نمی‌شود.
...
عملکرد خارج خواهد شد و خروجی از بلاکچین چاپ می‌شود:
...
## ارسال تراکنش‌های پیچیده

### سریالیز cells

Before we proceed, let's talk about the format of the messages we will send.

- **payload** (string base64, optional): BOC خام تک-cell که در Base64 کد شده است.
 - We will use it to store text comments on transfer
- **stateInit** (string base64, optional): BOC خام تک-cell که در Base64 کد شده است.
...
پس از تأیید، می‌توانیم تراکنشمان کامل را در [tonscan.org](https://tonscan.org/tx/pCA8LzWlCRTBc33E2y-MYC7rhUiXkhODIobrZVVGORg=) ببینیم.

## اگر کاربر درخواست تراکنش را رد کند، چه اتفاقی می‌افتد؟
