...
قبل از ورود به ساخت پیام‌ها، بیایید مفهوم سلول‌ها که بدنه پیام‌ها از آنها ساخته می‌شوند را معرفی کنیم.
...
### مقادیر بزرگتر داده‌ها

هر cell یک محدودیت 1023 بیتی دارد. اگر این مقدار را تجاوز کنید، خطا روی می‌دهد:
...
برای بارگذاری یک سلول ارجاع شده (تو در تو)، از `loadRef()` استفاده کنید:
...
دستکاری دستی سلول‌ها می‌تواند خسته‌کننده باشد، بنابراین `@ton-community/assets-sdk` متدهای مناسبی برای سریال‌سازی و دسریال‌سازی پیام‌ها ارائه می‌دهد.

استفاده از `@ton-community/assets-sdk` خوانایی بیشتری دارد و کمتر مستعد خطا است.
...
مهم نیست که یک توسعه‌دهنده در حال حل کردن چه سطحی از وظایف است، معمولاً لازم است از موجودیت کانکتور در `@tonconnect/sdk` یا `@tonconnect/ui` استفاده کند.
نمونه‌هایی که بر اساس `@tonconnect/sdk` و `@tonconnect/ui` ایجاد شده‌اند:
...
SDKهای TON Connect شامل بسته‌هایی برای ارسال پیام هستند که آماده‌سازی انتقال‌های معمولی Toncoin بین دو کیف پول به عنوان تراکنش پیش‌فرض بدون بارگذاری را آسان می‌سازد.
...
یک انتقال معمولی TON با استفاده از SDKهای TON Connect JS می‌تواند به شکل زیر اجرا شود:
...
بدنه تراکنش با موارد زیر ایجاد می‌شود:
...
:::info
شما می‌توانید کتابخانه `assets-sdk` را با متدها به صورت پیش‌فرض استفاده کنید (حتی با `ton-connect`)
:::
...
- `validUntil` - زمان در یونیکس تا زمانی که پیام معتبر است
- `jettonWalletAddress` - آدرس، آدرس JettonWallet، که بر اساس قراردادهای JettonMaser و کیف پول تعریف شده است
...
پیام در درخواست زیر قرار می‌گیرد:
...
- `jettonWalletAddress` - آدرس قرارداد کیف پول Jetton، که بر اساس قراردادهای JettonMaser و کیف پول تعریف شده است
- `amount` - شماره صحیح، مقدار Toncoin برای پرداخت های گاز در nanotons.
- `body` - بارگذاری برای کیف پول jetton با کد عملیات `burn#595f07bc`
...
<TabItem value="@ton/ton" label="@ton/ton">
پیغام `body` به طور معمول باید به روش زیر انجام شود:
...
`WALLET_DST` - آدرس - آدرس مالک اولیه NFT برای دریافت اضافی
انتقال `NFTitem` به مالک جدید `NEW_OWNER_WALLET`.
...
- `balance` - Integer, the amount of Toncoin used for gas payments in nanotons.
- `body` - بار مفید برای قرارداد NFT
...
اینجا یک مثال از آماده‌سازی پیام و معامله برای فروش در بازار GetGems طبق قرارداد [nft-fixprice-sale-v3r3](https://github.com/getgems-io/nft-contracts/blob/main/packages/contracts/sources/nft-fixprice-sale-v3r3.fc) آورده شده است.
...
<details>
    <summary>نمایش کل الگوریتم برای ایجاد بدنه پیام فروش NFT</summary>
...
نمونه‌های پایتون از [PyTonConnect](https://github.com/XaBbl4/pytonconnect) و [pytoniq](https://github.com/yungwine/pytoniq) استفاده می‌کنند.
...
:::tip
مثال‌ها را بخوانید [منبع](https://github.com/yungwine/ton-connect-examples/blob/master/main.py).
:::
...
ابتدا، با استفاده از تابع زیر پیامی با نظر پیاده‌سازی کنید:
...
بدنه نهایی تراکنش برای انتقال با نظر:
...
:::tip
بیشتر درباره [آدرس‌های قرارداد هوشمند TON](/v3/documentation/smart-contracts/addresses) بیاموزید.
:::
...
نمونه‌ای از عملکرد برای تراکنش انتقال NFT:
...
<details>
<summary>نمونه‌ای از ایجاد بدنه فروش NFT</summary>
...
روند خرید NFT برای قرارداد فروش [nft-fixprice-sale-v3r3](https://github.com/getgems-io/nft-contracts/blob/main/packages/contracts/sources/nft-fixprice-sale-v3r3.fc) می‌تواند با انتقال معمولی بدون محموله انجام شود، تنها چیز مهم مقدار دقیق توان است که به صورت زیر محاسبه می‌شود:
`buyAmount = Nftprice TON + 1.0 TON`.
...
مثال‌های Go از [tonconnect](https://github.com/cameo-engineering/tonconnect) و [tonutils-go](https://github.com/xssnick/tonutils-go) استفاده می‌کنند.
...
:::tip
مثال‌های [tonconnect](https://github.com/cameo-engineering/tonconnect/blob/master/examples/basic/main.go) و [tonutils-go](https://github.com/xssnick/tonutils-go?tab=readme-ov-file#how-to-use) را بخوانید.
:::
...
نمونه‌ای از عملکرد برای ساخت پیام انتقال معمولی TON:
...
نمونه‌ای از عملکرد برای ساخت پیام انتقال با نظر:
...
نمونه‌ای از عملکرد برای پیام انتقال NFT:
...
<details>
    <summary>الگوریتم کامل برای ایجاد بدنه پیام فروش NFT را نشان دهید</summary>
...
- [پردازش NFT روی TON](/v3/guidelines/dapps/asset-processing/nft-processing/nfts)
