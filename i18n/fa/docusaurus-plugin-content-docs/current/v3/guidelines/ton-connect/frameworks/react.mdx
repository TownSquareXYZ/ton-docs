...
# اتصال TON برای React

SDK توصیه شده برای برنامه‌های React عبارت است از [UI React SDK](/v3/guidelines/ton-connect/guidelines/developers#ton-connect-react). این یک مولفه React است که راهی در سطح بالا برای تعامل با TON Connect ارایه می‌دهد.
...
پس از ایجاد فایل مانیفست، TonConnectUIProvider را به ریشه برنامه مینی خود وارد کرده و URL مانیفست را وارد کنید:
...
اگر می‌خواهید کاربر را بعد از اتصال کیف پول به یک [برنامه مینی تلگرام](/v3/guidelines/dapps/tma/overview) منتقل کنید، می‌توانید عنصر `TonConnectUIProvider` را سفارشی کنید:
...
[نمونه را در GitHub باز کنید](https://github.com/ton-connect/demo-dapp-with-wallet/blob/master/src/App.tsx)
...
اگر می‌خواهید در برنامه React خود از برخی ویژگی‌های سطح پایین SDK رابط کاربری TON Connect استفاده کنید، می‌توانید از هوک‌های بسته `@tonconnect/ui-react` استفاده کنید.
...
از این هوک برای دسترسی به توابع باز و بستن پنجره مدل استفاده کنید. هوک یک شیء با وضعیت مدل کنونی و روش‌های باز و بستن مدل را باز می‌گرداند.
...
علاوه بر این، می‌توانید به جزئیات دقیق‌تری از کیف پول متصل شده همچون نام، تصویر و سایر ویژگی‌های آن دسترسی داشته باشید (نگاه کنید به [رابط WalletInfo](https://ton-connect.github.io/sdk/types/_tonconnect_sdk.WalletInfo.html)).
...
[مشاهده اطلاعات بیشتر درباره‌ی روش‌های نمونه TonConnectUI](https://github.com/ton-connect/sdk/tree/main/packages/ui#send-transaction)
...
برای اطمینان از اینکه کاربر واقعاً مالک آدرس اعلام شده است، می‌توانید از `ton_proof` استفاده کنید.

از تابع `tonConnectUI.setConnectRequestParameters` برای تنظیم پارامترهای درخواست اتصال خود استفاده کنید. می‌توانید از آن برای موارد زیر استفاده کنید:

- وضعیت بارگذاری: وضعیت بارگذاری را در حین انتظار برای پاسخ از بک‌اند خود نمایش دهید.
- وضعیت آمادگی با tonProof: وضعیت را به «آماده» تنظیم کنید و مقدار tonProof را شامل کنید.
- اگر خطایی رخ دهد، بارگذار را حذف کنید و درخواست اتصال را بدون پارامترهای اضافی ایجاد کنید.
...
زمانی که کیف پول متصل شد، می‌توانید نتیجه `ton_proof` را در شئ `wallet` پیدا کنید:
...
برای قطع اتصال کیف پول با این متد فراخوانی کنید:
