...
اگر می‌خواهید کاربر را بعد از اتصال کیف‌پول به یک [اپلیکیشن مینی تلگرام](/v3/guidelines/dapps/tma/overview) هدایت کنید، می‌توانید عنصر `TonConnectUIProvider` را سفارشی‌سازی کنید:
...
[در مستندات SDK بیشتر بخوانید](https://github.com/ton-connect/sdk/tree/main/packages/ui#use-inside-twa-telegram-web-app)
...
از این هوک برای دسترسی به توابع باز و بستن پنجره مدال استفاده کنید. هوک یک شی با حالت کنونی مدال و روش‌های باز و بسته کردن مدال بازمی‌گرداند.
...
از آن برای دسترسی به نمونه `TonConnectUI` و تابع به‌روزرسانی گزینه‌های رابط کاربری استفاده کنید.
...
[بیشتر درباره روش‌های نمونه TonConnectUI بخوانید](https://github.com/ton-connect/sdk/tree/main/packages/ui#send-transaction)
...
:::tip
نحوه امضا و تأیید پیام‌ها را بفهمید: [امضا و تأیید](/v3/guidelines/ton-connect/guidelines/verifying-signed-in-users)
:::

Use the `tonConnectUI.setConnectRequestParameters` function to pass your connect request parameters.

این تابع یک پارامتر می‌پذیرد:
...
برای قطع اتصال کیف پول فراخوانی شود:
...
اگر با مشکلاتی مواجه هستید که انیمیشن‌ها در محیط شما کار نمی‌کنند، ممکن است به دلیل عدم پشتیبانی از API وب انیمیشن‌ها باشد. برای حل این مشکل، می‌توانید از polyfill `web-animations-js` استفاده کنید.

#### استفاده از npm

برای نصب polyfill، دستور زیر را اجرا کنید:
...
در غیر این صورت، می‌توانید با اضافه کردن تگ اسکریپت زیر به HTML خود، polyfill را از طریق CDN شامل کنید:
