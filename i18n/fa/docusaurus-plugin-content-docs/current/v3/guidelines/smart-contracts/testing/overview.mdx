...
1. یک پروژه جدید Blueprint ایجاد کنید:

```bash
npm create ton@latest MyProject
```

2. یک تست را اجرا کنید:
...
1. یک موجودیت `Contract` خاص تعریف‌شده با استفاده از `blockchain.openContract()` ایجاد کنید.
2. Describe the actions your `Contract` should perform and save the execution result in the `res` variable.
3. خواص را با استفاده از تابع `expect()` و مچ‌کننده `toHaveTransaction()` بررسی کنید.
...
| موفقیت  | boolean? | Custom Sandbox flag that defines the resulting status of a certain transaction. True if both the compute and the action phases succeeded. Otherwise, False. |
...
از آموزه‌های ارزشمند جامعه در TON بیشتر درباره تست‌گیری بیاموزید:

- [درس <b>۲</b>: تست‌گیری FunC برای یک قرارداد هوشمند](https://github.com/romanovichim/TonFunClessons_Eng/blob/main/lessons/smartcontract/2lesson/secondlesson.md)
- [TON Hello World part 4: Step-by-step guide for testing your first smart contract](https://helloworld.tonstudio.io/04-testing/)
- [خط لوله قرارداد هوشمند TON](https://dev.to/roma_i_m/ton-smart-contract-pipeline-write-simple-contract-and-compile-it-4pnh)
...
- [طرح‌ اولیه](/v3/documentation/smart-contracts/getting-started/javascript)
