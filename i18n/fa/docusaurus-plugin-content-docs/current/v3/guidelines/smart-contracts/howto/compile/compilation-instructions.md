...
احتمالاً نرم‌افزار بر روی اکثر سیستم‌های لینوکس به درستی کامپایل و کار می‌کند. باید روی macOS و حتی Windows هم کار کند.
...
3. فرض کنید که درخت سورس را به دایرکتوری `~/ton` که `~` دایرکتوری خانگی شماست دریافت کرده‌اید و یک دایرکتوری خالی `~/ton-build` را ایجاد کرده‌اید:

```bash
mkdir ton-build
```

سپس در یک ترمینال لینوکس یا MacOS موارد زیر را اجرا کنید:
...
با نصب بسته‌های سیستمی مورد نیاز، سیستم را آماده کنید
...
از clang تازه نصب شده استفاده کنید.
...
اکنون می‌توانید TON را کامپایل کنید
