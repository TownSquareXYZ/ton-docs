...
## کار با آدرس‌های قراردادها

### چگونه می‌توان آدرس‌ها را (کاربرپسند \<-\> خام) تبدیل، تجمیع، و از رشته‌ها استخراج کرد؟
...
|  شروع آدرس | فرمت باینری | قابل جهش |  فقط تستنت |
| :--------: | :---------: | :------: | :--------: |
| E...       |  000100.01  |  بله     | نه         |
| U...       |  010100.01  |  نه      | نه         |
| k...       |  100100.01  |  بله     | بله        |
...
### نوشتن توضیحات: رشتهٔ طولانی در قالب snake
...
For example, using Tonviewer, you can get the jetton master contract address (e.g., `EQBynBO23ywHy_CgarY9NK9FTz0yDsG82PtcbSTQgGoXwiuA`). If we [go to this address](https://tonviewer.com/EQBynBO23ywHy_CgarY9NK9FTz0yDsG82PtcbSTQgGoXwiuA?section=method) and open the Methods tab, we can see that there is already a `get_jetton_data` method there. By calling it, we can get the hex form of the cell with the jetton wallet code:
...
تمام مثال را [اینجا](/example-code-snippets/pythoniq/jetton-offline-address-calc-wrapper.py) بخوانید.

</TabItem>
</Tabs>

Major tokens typically use a standard implementation of the [ TEP-74 standard](https://github.com/ton-blockchain/token-contract/blob/main/ft/jetton-wallet.fc), except the new [Jetton-with-governance contracts](https://github.com/ton-blockchain/stablecoin-contract) for centralized stablecoins. The difference is [the presence of a wallet status field and the absence of a code cell in the vault](https://github.com/ton-blockchain/stablecoin-contract/blob/7a22416d4de61336616960473af391713e100d7b/contracts/jetton-utils.fc#L3-L12).

### چگونه می‌توان پیام برای انتقال jetton با نظر را ساخت?
...
البته، همیشه می‌توان محاسبات را در واحدهای تقسیم‌ناپذیر انجام داد.
:::
...
:::info
توسط استاندارد NFT برای /ton-blockchain /token-contract مشخص نشده است
:::
...
در مرحله بعد، باید هزینه کلی تراکنش را به‌درستی محاسبه کنیم. مقدار <b>۰٫۰۱۵</b> از طریق آزمایش به دست آمده است، اما ممکن است برای هر سناریو متفاوت باشد. این بیشتر به محتوای NFT بستگی دارد، زیرا افزایش اندازه محتوا باعث افزایش **هزینه ارسال** (هزینه تحویل) می‌شود.

### چگونه مالک قرارداد هوشمند یک مجموعه را تغییر دهیم?
...
### چگونه محتوای قرارداد هوشمند یک مجموعه را تغییر دهیم?
...
### چگونه پیام مبادله‌ای به DEX (DeDust) ارسال کنیم?
...
- [مستندات DeDust](https://docs.dedust.io/).
...
این طرح برای بدنه انتقال به **خزانه** toncoin است.

ابتدا، نیاز دارید تا آدرس‌های **خزانه** jettons که مبادله خواهید کرد یا آدرس **خزانه** toncoin را بدانید. این کار از طریق متد دریافت `get_vault_address` قرارداد [**Factory**](https://docs.dedust.io/reference/factory) قابل انجام است. به‌عنوان یک آرگومان شما باید یک slice مطابق با طرح‌واره را ارائه دهید:
...
همچنین برای خود مبادله نیز به آدرس **استخر** - به‌دست‌آمده از متد دریافت `get_pool_address`. به‌عنوان آرگومان‌ها - slice دارایی‌ها مطابق با طرح‌واره بالا. در پاسخ، هر دو متد یک slice از آدرس **خزانه** / **استخر** درخواست‌شده را برمی‌گردانند.
...
در زیر طرح‌واره TL-B که DeDust برای مفهوم Asset معرفی کرده است آمده است.
...
ما همچنین نیاز به آوردن DeDust SDK داریم.

```bash
npm install --save @dedust/sdk
```

اکنون باید برخی از objects را مقداردهی اولیه کنیم.
...
برای Swap توکن X با Y، فرآیند یکسان است، به عنوان مثال، ما مقداری از توکن X را به Vault X می‌فرستیم، Vault X
دارایی ما را دریافت می‌کند، آن را نگه می‌دارد، و به Pool (X, Y) اطلاع می‌دهد که این آدرس برای Swap درخواست داده است، اکنون Pool براساس
محاسبه به Vault دیگری اطلاع می‌دهد، در اینجا Vault Y معادل Y را به کاربری که Swap را درخواست کرده است، آزاد می‌کند.

تفاوت بین دارایی‌ها فقط در روش انتقال است برای مثال، برای jettons، آن‌ها را با استفاده از یک پیام انتقال به Vault انتقال می‌دهیم و یک forward_payload خاص را پیوست می‌کنیم، اما برای سکه بومی، یک پیام Swap به Vault می‌فرستیم و مقدار متناظر TON را ضمیمه می‌کنیم.

این شِما برای TON و jetton:

```tlb
swap#ea06185d query_id:uint64 amount:Coins _:SwapStep swap_params:^SwapParams = InMsgBody;
```

بنابراین هر Vault و Pool متناظر برای Swaps خاصی طراحی شده‌اند و دارای یک API اختصاصی هستند که برای دارایی‌های خاص سفارشی شده است.

این Swap با jetton SCALE بود. فرآیند Swap jetton با jetton یکسان است، تنها تفاوت این است که باید بار Payload توصیف شده در شِما TL-B را فراهم کنیم.
...
لیست تراکنش‌های یک حساب را می‌توان از طریق روش API `getTransactions` دریافت کرد. این یک آرایه از شی‌های `Transaction` را برمی‌گرداند، که هر مورد دارای ویژگی‌های زیادی است. با این حال، فیلدهایی که بیشتر استفاده می‌شوند عبارتند از:

- فرستنده، بدنه و مقدار پیام که این تراکنش را آغاز کرده است
- هش و زمان منطقی (LT) تراکنش

فیلدهای _Sender_ و _Body_ ممکن است برای تعیین نوع پیام (انتقال عادی، انتقال jetton، انتقال nft و غیره) استفاده شوند.

در زیر یک مثال آورده شده است که چگونه می‌توانید ۵ تراکنش اخیر را در هر حساب بلاک‌چین به‌دست آورید، آن‌ها را بر اساس نوع تجزیه و تحلیل کرده و در یک حلقه نمایش دهید.
...
### چگونه میتوان تراکنشی برای یک نتیجه خاص از TON Connect پیدا کرد?
...
آماده‌سازی تابع `retry` برای تلاش ها به منظور گوش دادن به بلاکچین:
...
### چگونه می‌توان تراکنش یا هش پیام را پیدا کرد?
