...
ما فعالانه به دنبال بازخورد درباره فرآیند نصب هستیم. اگر سوالی یا پیشنهادی دارید، لطفاً [با ما تماس بگیرید](https://t.me/Alexgton).
...
| نام قالب                    | قالب                                                                       | مثال                                                                  | Description                                                                     |
| :-------------------------- | :------------------------------------------------------------------------- | :-------------------------------------------------------------------- | ------------------------------------------------------------------------------- |
...
2. [mnemonic2key](https://github.com/ton-blockchain/mnemonic2key) را کلون کرده و نصب کنید:
   ...
   npm install
   ```
3. دستور زیر را اجرا کنید و `word1`, `word2`... را با عبارت یادآور خود و `address` را با آدرس قرارداد Wallet خود جایگزین کنید:
   ```
...
8. اکنون می‌توانید با استفاده از دستور `mg` وجوه ارسال کنید. برای مشاهده مستندات کمک، `mg` را وارد کنید.
   به خاطر داشته باشید که هنگام اجرای دستورات، جای‌نامها (کلمات داخل `< >`) را با مقادیر واقعی خود جایگزین کنید.
...
| ترکیب‌های مختلفی از آرگومان‌های زیر | `el --json adnl pubkey wallet past` | Full ADNL, Pubkey, Wallet, and past election entries in JSON representation. |
...
| ترکیب‌های مختلفی از آرگومان‌های زیر | `vl --json adnl pubkey wallet past` | Full ADNL, Pubkey, Wallet, and past validator entries in JSON representation. |
...
| نام آرگومان نود    | توضیحات                                                                                                        | مقدار پیش‌فرض                                                                         |
| ------------------ | -------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------- |
...
| `archive-ttl`      | ttl بلوک‌هایی که نود ذخیره می‌کند. _برای اجبار نود به عدم ذخیره بلوک‌های آرشیوی از مقدار `86400` استفاده کنید_ | 2592000 اگر حالت liteserver در طول فرآیند نصب فعال شده باشد، در غیر این صورت 86400    |
