...
| ساختار    | Type                            | الزامی  | Description                                                                                                                     |   |
| --------- | ------------------------------- | ------- | ------------------------------------------------------------------------------------------------------------------------------- | - |
...
| info      | [CommonMsgInfo](#commonmsginfo) | الزامی  | Contains detailed message properties such as destination and value. It is always stored in the message's root cell.             |   |
| init      | [StateInit](#stateinit-tl-b)    | اختیاری | The general structure is used in TON to initialize new contracts. It can be placed in either a cell reference or the root cell. |   |
| body      | X                               | الزامی  | Message payload. It can be placed in either a cell reference or the root cell.                                                  |   |
...
| ساختار                 | Type                                      | Required | Description                                                                                                                     |
| ---------------------- | ----------------------------------------- | -------- | ------------------------------------------------------------------------------------------------------------------------------- |
...
| مقدار                  | [CurrencyCollection](#currencycollection) | Required | Describes the currency details, including the total funds transferred with the message.                                         |
| کارمزد_ای‌اچ‌آر        | [VarUInteger 16](#varuinteger-n)          | Required | Fee for delivering the message using hypercube routing.                                                                         |
| کارمزد_فوروارد         | [VarUInteger 16](#varuinteger-n)          | Required | Validators set the fee for forwarding the message.                                                                              |
| created_lt             | uint64                                    | Required | Logic time of the message, used by validators to order actions in smart contracts.                                              |
| created_at             | uint32                                    | الزامی   | UNIX timestamp indicating when the message was created.                                                                         |
...
| Structure          | نوع                                  | Required | Description                                                                                                                     |
| ------------------ | ------------------------------------ | -------- | ------------------------------------------------------------------------------------------------------------------------------- |
| ext_in_msg_info$10 | سازنده                               | Required | The `$10` tag indicates that the `CommonMsgInfo` begins with `10` bits in serialization, denoting an external incoming message. |
| ihr_disabled       | بولی                                 | Required | Flag indicating whether hyper routing is disabled. Currently, it is always set to `true`.                                       |
...
| import_fee         | [VarUInteger 16](#varuinteger-n)     | ضروری    | The fee for executing and delivering the message.                                                                               |
...
| src                 | [MsgAddressInt](#msgaddressint-tl-b) | Required | The sender’s smart contract address.                                                                                            |
| مقصد                | [MsgAddressExt](#msgaddressext-tl-b) | Required | The external destination address for the message.                                                                               |
...
| created_at          | uint32                               | ضروری    | UNIX timestamp representing when the message was created.                                                                       |
...
| ساختار       | نوع                      | ضروری    | Description                                                                                                                                                                                                                                                                                                                                                              |
| ------------ | ------------------------ | -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| عمق تقسیم    | (## 5)                   | Optional | A parameter for high-load contracts that defines the behavior of splitting into multiple instances across different shards. Currently, `StateInit` is used without it.                                                                                                                                                                                                   |
| اختصاصی      | TickTock\*               | Optional | A structure used for invoking smart contracts in every new blockchain block. Available only in the MasterChain and not typically used in regular user contracts.                                                                                                                                                                                                         |
| کد           | Cell                     | Optional | The contract's serialized code.                                                                                                                                                                                                                                                                                                                                          |
| داده         | Cell                     | Optional | The initial data for the contract.                                                                                                                                                                                                                                                                                                                                       |
| کتابخانه     | HashmapE 256 SimpleLib\* | اختیاری  | Currently, `StateInit` is used without libraries.                                                                                                                                                                                                                                                                                                                        |
...
| ساختار       | نوع        | Required | Description                                                                                                                                   |
| ------------ | ---------- | -------- | --------------------------------------------------------------------------------------------------------------------------------------------- |
| addr_none$00 | سازنده     | ضروری    | The `$00` tag in serialization indicates that `MsgAddressExt` starts with `00` bits. This signifies that the entire external address is `00`. |
...
| ساختار           | نوع        | Required | Description                                                                                                            |
| ---------------- | ---------- | -------- | ---------------------------------------------------------------------------------------------------------------------- |
| addr_extern$01   | سازنده     | Required | The `$01` tag in serialization indicates that `MsgAddressExt` starts with `01` bits and describes an external address. |
| طول              | ## 9       | Required | Represents an unsigned N-bit number, similar to `uintN`.                                                               |
| آدرس_خارجی       | (طول بیت)  | ضروری    | A bitstring address, where the length of the address equals the previously defined `len`.                              |
...
| ساختار        | نوع          | Required | Description                                                                                                                                                                                                                                                |
| ------------- | ------------ | -------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| addr_std$10   | سازنده       | ضروری    | The `$10` tag in serialization indicates that `MsgAddressInt` starts with `10` bits and describes an internal address.                                                                                                                                     |
| پخش هر        | Anycast\*    | اختیاری  | Represents additional address data. Currently not used in ordinary internal messages.                                                                                                                                                                      |
| شناسه_کارِچین | int8         | Required | The WorkChain where the smart contract of the destination address is located. Currently, this value always equals zero.                                                                                                                                    |
| آدرس          | (bits256)    | ضروری    | The smart contract account ID number.                                                                                                                                                                                                                      |
...
| ساختار        | Type         | Required  | توضیحات                                                                                                                                             |
| ------------- | ------------ | --------- | --------------------------------------------------------------------------------------------------------------------------------------------------- |
| addr_var$11   | Constructor  | ضروری     | The `$11` tag indicates that in the serialization of `MsgAddressInt`, the address starts with `11` bits, representing an internal contract address. |
| پخش هر        | Anycast\*    | اختیاری   | Additional address data is not currently used in regular internal messages.                                                                         |
| طول_آدرس      | ## 9         | Required  | Same as `uintN` – represents an unsigned N-bit number.                                                                                              |
| شناسه_کارِچین | int32        | Required  | The WorkChain is where the smart contract for the destination address is located. Currently, this is always set to zero.                            |
| آدرس          | (bits256)    | ضروری     | The payload address (this could be the account ID).                                                                                                 |

## نوع‌های پایه‌ای استفاده شده
...
| Structure      | نوع                     | Required | توضیحات                                                                                                                             |
| -------------- | ----------------------- | -------- | ----------------------------------------------------------------------------------------------------------------------------------- |
| واحدهای پولی$_ | سازنده                  | Required | The `$_` empty tag indicates that no bits are added at the beginning during the serialization of `CurrencyCollection`.              |
| گرم‌ها         | (VarUInteger ۱۶)        | ضروری    | Represents the message value, expressed in nanoTons.                                                                                |
| دیگر           | ExtraCurrencyCollection | اختیاری  | ExtraCurrencyCollection is a dictionary intended for additional, typically empty currencies.                                        |
...
| ساختار     | نوع               | Required | Description                                                                                                                 |
| ---------- | ----------------- | -------- | --------------------------------------------------------------------------------------------------------------------------- |
| var_uint$_ | سازنده            | Required | The `var_uint$_` is an empty tag, indicating that no bits are added at the beginning of `CurrencyCollection` serialization. |
| len        | uintN             | ضروری    | Specifies the bit-length used to encode the next value.                                                                     |
| مقدار      | (uint (len \* ۸)) | اختیاری  | An optional unsigned integer encoded in `(len * 8)` bits.                                                                   |

## مثال پیام

### پیام داخلی معمولی func
...
### پیام معمولی func به صورت کوتاه
