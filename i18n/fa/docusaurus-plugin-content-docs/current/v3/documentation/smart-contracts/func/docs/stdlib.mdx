---
toc_min_heading_level: ۲
toc_max_heading_level: ۶
---

import Feedback from "@site/src/components/Feedback";

# کتابخانه استاندارد FunC
...
لیست‌ها می‌توانند به صورت کورتژهای ۲ عنصری تو در تو نمایش داده شوند. لیست خالی معمولاً به عنوان مقدار TVM `null` نمایش داده می‌شود (می‌تواند با صدا زدن `null()` به دست آید). به عنوان مثال، کورتژ `(1, (2, (3, null)))` لیست `[1, 2, 3]` را نمایش می‌دهد. عناصر یک لیست می‌توانند از انواع مختلف باشند.
...
یک عنصر را به ابتدای یک لیست سبک Lisp اضافه می‌کند.
...
سر یک لیست سبک Lisp را بر می‌گرداند.
...
دُم یک لیست سبک Lisp را بر می‌گرداند.

### عملیات اولیه کورتژهای دیگر
...
اولین عنصر یک کورتژ را بر می‌گرداند.
...
دومین عنصر یک کورتژ را بر می‌گرداند.
...
سومین عنصر یک کورتژ را بر می‌گرداند.
...
چهارمین عنصر یک کورتژ را بر می‌گرداند.
...
اولین عنصر یک زوج را بر می‌گرداند.
...
دومین عنصر یک زوج را بر می‌گرداند.
...
اولین عنصر یک سه‌گانه را بر می‌گرداند.
...
دومین عنصر یک سه‌گانه را بر می‌گرداند.
...
سومین عنصر یک سه‌گانه را بر می‌گرداند.

## عملیات اولیه مخصوص دامنه

### استخراج اطلاعات از c7
...
زمان منطقی تراکنش جاری را بر می‌گرداند.
...
مقدار پارامتر تنظیمات جهانی با اندیس عدد صحیح `i` را به صورت `cell` یا مقدار `null` بر می‌گرداند.
...
### ذخیره و بارگذاری ذخیره‌سازی ماندگار
...
سلول `c` را به عنوان داده‌های قرارداد ماندگار تنظیم می‌کند. می‌توانید با استفاده از این پریمیتیو، ذخیره‌سازی قرارداد ماندگار را به‌روزرسانی کنید.

### پریمیتیوهای ادامه‌دار
...
1. Insufficient Toncoins:
   ...
   - فقط کافی برای پرداخت یک پیام خارجی خروجی وجود ندارد.
...
### ابتدایی‌های تولید کننده اعداد تصادفی
...
- [تولید اعداد تصادفی](/v3/guidelines/smart-contracts/security/random-number-generation)
...
یک عدد صحیح شبه تصادفی جدید `z` در بازه `0..range−1` تولید می‌کند (یا `range..−1` اگر `range < 0` باشد). به طور دقیق‌تر، یک مقدار تصادفی بدون علامت `x` مانند `random` تولید می‌شود؛ سپس `z := x * range / 2^256` محاسبه می‌شود.
...
یک عدد صحیح ۲۵۶-بیتی بدون علامت `x` را به یک بذر تصادفی `r` مخلوط می‌کند با تنظیم بذر تصادفی به sha256 از الحاق دو رشته ۳۲-بایتی: اول با نمایش big-endian بذر قدیمی `r` و دوم با نمایش big-endian `x`.
...
- `addr_none` توسط `t = (0)` نمایش داده می‌شود، یعنی یک زوج که دقیقا شامل یک عدد صحیح برابر با صفر است.
- `addr_extern` توسط `t = (1, s)` نمایش داده می‌شود، جایی که slice `s` شامل فیلد `external_address` است. به عبارت دیگر، `t` یک زوج است (یک زوج شامل دو مدخل)، که شامل یک عدد صحیح برابر با یک و slice `s` است.
- `addr_std` توسط `t = (2, u, x, s)` نمایش داده می‌شود، جایی که `u` یا `null` است (اگر `anycast` وجود نداشته باشد) یا یک slice `s'` که شامل `rewrite_pfx` است (اگر `anycast` موجود باشد). بعد، عدد صحیح `x` به عنوان `workchain_id` است و slice `s` شامل آدرس است.
- `addr_var` توسط `t = (3, u, x, s)` نمایش داده می‌شود، جایی که `u`، `x`، و `s` همان معنی را که برای `addr_std` دارند.
...
اولین ارجاع را از یک slice بارگذاری می‌کند.
...
اولین ارجاع را از یک slice پیش‌بارگذاری می‌کند.
...
یک عدد صحیح `len`-بیتی با علامت را از یک slice بارگذاری می‌کند.
...
یک عدد صحیح بدون علامت به طول `len`-بیت را از یک قطعه بارگذاری می‌کند.
...
یک عدد صحیح علامت‌دار به طول `len`-بیت را از یک قطعه پیش‌بارگذاری می‌کند.
...
یک عدد صحیح بدون علامت به طول `len`-بیت را از یک قطعه پیش‌بارگذاری می‌کند.
...
یک دیکشنری `D` را از قطعه `s` پیش‌بارگذاری می‌کند.
...
دیکشنری را به عنوان `load_dict` بارگذاری می‌کند اما فقط باقی‌مانده قطعه را برمی‌گرداند.

### ابتدائیات اندازه قطعه
...
تعداد ارجاع‌ها در قطعه `s` را برمی‌گرداند.
...
تعداد بیت‌های داده در قطعه `s` را برمی‌گرداند.
...
هم تعداد بیت‌های داده و هم تعداد ارجاعات در `s` را برمی‌گرداند.
...
بررسی می‌کند که قطعه `s` خالی است (یعنی شامل هیچ بیت داده و ارجاع به سلول نمی‌شود).
...
بررسی می‌کند که آیا قطعه `s` هیچ بیت داده‌ای ندارد.
...
بررسی می‌کند که آیا قطعه `s` هیچ ارجاعی ندارد.
...
یک `builder` خالی جدید ایجاد می‌کند.
...
بررسی می‌کند آیا یک لغت‌نامه خالی است یا نه. معادل `cell_null?`.

## ابتدای لغت‌نامه‌های پیشوند
...
کمینه‌ی دو عدد صحیح `x` و `y` را محاسبه می‌کند.
...
بیشینه‌ی دو عدد صحیح `x` و `y` را محاسبه می‌کند.
