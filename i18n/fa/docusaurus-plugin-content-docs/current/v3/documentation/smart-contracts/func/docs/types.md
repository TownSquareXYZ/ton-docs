...
FunC از استنباط نوع پشتیبانی می‌کند. انواع `_` و `var` به عنوان "holes" نوعی نمایش داده می‌شوند که در طول بررسی نوع بعداً می‌توان با یک نوع واقعی پر شوند. به‌عنوان مثال، `var x = ۲;` تعریفی از متغیر `x` معادل `۲` است. بررسی‌کننده نوع می‌تواند استنباط کند که `x` دارای نوع `int` است، زیرا `۲` دارای نوع `int` است و سمت چپ و راست انتساب باید نوع برابر داشته باشند.
...
نوع به فرم `(A)` توسط بررسی‌کننده نوع به عنوان همان نوع `A` در نظر گرفته می‌شود.
...
یک تابع چندریختی است که یک مقدار (ورودی تکی از استک) را می‌گیرد و دو نسخه از این مقدار را برمی‌گرداند. `duplicate(6)` مقادیر `6 6` را تولید می‌کند و `duplicate([])` دو نسخه `[] []` از یک تاپل خالی تولید می‌کند.
