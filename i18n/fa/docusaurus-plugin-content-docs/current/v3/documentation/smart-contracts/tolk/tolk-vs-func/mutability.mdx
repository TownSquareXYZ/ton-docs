---
title: "Tolk در برابر FunC: قابلیت تغییر"
---

import Feedback from '@site/src/components/Feedback';

# قابلیت تغییر در Tolk در مقایسه با توابع tilda در FunC

:::tip خلاصه

- هیچ متد `~` tilda وجود ندارد
- `cs.loadInt(32)` یک Slice را تغییر می‌دهد و یک integer برمی‌گرداند
- `b.storeInt(x, 32)` یک builder را تغییر می‌دهد
- `b = b.storeInt()` also works since it not only modifies but returns
- chained methods work identically to JS; they return `self`
- همه چیز به دقت همان‌طور که انتظار می‌رود، مانند JS کار می‌کند
- no runtime overhead, the same Fift instructions
- متدهای سفارشی به آسانی ایجاد می‌شوند
- tilda `~` در Tolk به هیچ وجه وجود ندارد
  :::
...
هدف این است که فراخوانی‌ها مشابه JS و زبان‌های دیگر باشند:
...
<h3 className="cmp-func-tolk-header">
  به‌طور پیش‌فرض، تمام پارامترها توسط مقدار کپی می‌شوند (مشابه FunC)
</h3>
...
برای slices و هر نوع دیگر نیز همین‌طور:
...
این یک تعمیم است. یک تابع ممکن است چندین پارامتر تغییر داشته باشد:
...
با ترکیب `mutate` و `self`، ما به روش‌های تغییرپذیر دست می‌یابیم.

<h3 className="cmp-func-tolk-header">
  `mutate self` یک روش است که از طریق dot فراخوانی می‌شود و یک شیء را تغییر می‌دهد.
</h3>
...
<h3 className="cmp-func-tolk-header">
  `return self` باعث می‌شود یک روش زنجیره‌ای شود.
</h3>
...
وقتی یک تابع دارای پارامترهای `mutate` است، در واقع آنها را به‌طور ضمنی برمی‌گرداند و به‌طور ضمنی به آرگومان‌ها اختصاص می‌دهد. مثال بهتر توضیح می‌دهد:
...
بنابراین، یک تابع `asm` باید `self'` را قبل از مقدار بازگشتی‌اش بر روی پشته قرار دهد:
...
<h3 className="cmp-func-tolk-header">
  آیا من به `@inline` برای توابع/متدهای ساده نیاز دارم؟
</h3>
