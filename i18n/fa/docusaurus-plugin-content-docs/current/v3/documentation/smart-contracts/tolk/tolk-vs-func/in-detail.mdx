...
# Tolk در برابر FunC: به‌صورت جزئیات

A huge list is below. Will anyone have enough patience to read it up to the end?..

:::tip یک نسخه فشرده وجود دارد
اینجا: [Tolk vs FunC: به اختصار](/v3/documentation/smart-contracts/tolk/tolk-vs-func/in-short)
:::
...
<h3 className=\"cmp-func-tolk-header\">
  ✅ `2+2` برابر است با ۴، نه یک شناسه. شناسه‌ها فقط می‌توانند ترکیبی از حروف و عدد باشند
</h3>
...
در Tolk، فضاها ضروری نیستند. `2+2` همان طور که انتظار می‌رود ۴ است. `3+~x` به شکل `3 + (~ x)` می‌باشد، و غیره.
...
<h3 className="cmp-func-tolk-header">
  ✅ ناپاک به طور پیش‌فرض، کامپایلر فراخوانی‌های توابع کاربر را حذف نخواهد کرد
</h3>
...
در Tolk، همه توابع به طور پیش‌فرض ناپاک هستند. می‌توانید یک تابع را با حاشیه‌نویسی خالص علامت‌گذاری کنید،
و سپس عملیات ناپاک در بدنه آن ممنوع است (استثناءها، تغییرات جهانی، فراخوانی توابع ناپاک و غیره).

<h3 className="cmp-func-tolk-header">
  ✅ سینتکس جدید توابع: کلیدواژه `fun`، ویژگی‌های `@`، نوع‌ها در سمت راست (مانند در TypeScript، Kotlin، Python و غیره)
</h3>
...
نوع‌های متغیرها - همچنین در سمت راست:
...
تعدیل‌کننده‌ها `inline` و دیگران — با حاشیه‌نویسی:
...
پیاده‌سازی `asm` — مانند FunC، اما با تراز درست، بهتر به نظر می‌رسد:
...
همچنین یک ویژگی `@deprecated` وجود دارد که بر کامپایل تأثیری نمی‌گذارد، اما برای انسان و IDE است.

<h3 className="cmp-func-tolk-header">
  ✅ `get` به جای `method_id`
</h3>
...
هر دو `get methodName()` و `get fun methodName()` قابل قبول هستند.

برای `method_id(xxx)` (در عمل نادر اما معتبر)، یک ویژگی وجود دارد:
...
<h3 className="cmp-func-tolk-header">
  ✅ الزامی بودن اعلام نوع پارامترها (اگرچه برای محلی‌ها اختیاری است)
</h3>
...
برای متغیرهای محلی، نوع‌ها نیز اختیاری هستند:
...
<h3 className="cmp-func-tolk-header">
  ✅ در یک حوزه یکسان مجاز به تغییر اعلان متغیرها نیست
</h3>
...
به همین دلیل، باز تخصیص جزئی مجاز نیست:
...
توجه داشته باشید که این موضوع برای `loadUint()` و دیگر متدها مشکلی ایجاد نمی‌کند. در FunC، آنها یک شی تغییر یافته را برمی‌گرداندند، بنابراین الگوی `var (cs, int value) = cs.load_int(۳۲)` بسیار رایج بود. در Tolk، چنین متدهایی یک شی را تغییر می‌دهند: `var value = cs.loadInt(۳۲)`، بنابراین احتمالاً تغییر اعلان مورد نیاز نخواهد بود.
...
- `self`، برای ایجاد متدهای زنجیره‌ای، در زیر توضیح داده شده؛ در واقع این یک نوع نیست، می‌تواند تنها به‌جای نوع بازگشتی تابع ظاهر شود
...
<h3 className="cmp-func-tolk-header">
  ✅ نام‌گذاری دیگری برای recv_internal / recv_external
</h3>
...
همه نوع‌های پارامتر و ترتیب آنها به همان صورت برای تغییر نام می‌مانند، تنها نامگذاری تغییر می‌کند. `fun main` نیز در دسترس است.

<h3 className="cmp-func-tolk-header">
  ✅ #include → import. واردات سخت‌گیرانه
</h3>
...
<h3 className="cmp-func-tolk-header">
  ✅ #pragma → گزینه‌های کامپایلر
</h3>

در FunC، ویژگی‌های "تجربی" مانند `allow-post-modifications` توسط یک پراگما در فایل‌های .fc فعال شدند (که منجر به مشکلاتی شد هنگامی که برخی فایل‌ها آن را شامل می‌شدند و برخی نه). در واقع، این یک پراگما برای یک فایل نیست، بلکه یک گزینه کامپایل است.

در Tolk، همه پراگماها حذف شدند. `allow-post-modification` و `compute-asm-ltr` به منابع Tolk ادغام شدند (گویی همیشه در FunC فعال بودند). به جای پراگماها، اکنون امکان ارائه گزینه‌های تجربی وجود دارد.

تا به حال، یک گزینه تجربی معرفی شده است - `remove-unused-functions`، که نمادهای استفاده‌نشده را در خروجی Fift نادیده می‌گیرد.
...
ایجاد مقادیر null و بررسی متغیرها از نظر null اکنون بسیار زیبا به نظر می‌رسد.
...
<h3 className="cmp-func-tolk-header">
  ✅ کلمات کلیدی `throw` و `assert`
</h3>

Tolk dramatically simplifies working with exceptions.

اگر FunC دارای `throw()`، `throw_if()`، `throw_arg_if()` و به همان شکل unless باشد، Tolk فقط دو کلمه کلیدی دارد: `throw` و `assert`.
...
توجه داشته باشید که `!شرط` ممکن است زیرا NOT منطقی در دسترس است، به پایین نگاه کنید.

یک نحو طولانی (طولانی) برای `assert(condition, excNo)` وجود دارد:
...
همچنین، Tolk مبادله می‌کند `catch` آرگومان‌ها: این `catch (excNo, arg)` است، هر دو اختیاری هستند (چون arg به احتمال زیاد خالی است).
...
<h3 className="cmp-func-tolk-header">
  ✅ `انجام بده ... تا زمانی که` → `انجام بده ... در حالی که`
</h3>
...
توجه داشته باشید که `!condition` ممکن است زیرا NOT منطقی در دسترس است، به زیر مراجعه کنید.

<h3 className="cmp-func-tolk-header">
  ✅ اولویت عملگرها با C++ / JavaScript یکسان شد
</h3>
...
منجر به خطای کامپایل خواهد شد (مشابه gcc/clang):
...
همچنین، خطاهایی برای یک اشتباه رایج در اپراتورهای شیفت بیتی اضافه کرده‌ام: `a << ۸ + ۱` معادل `a << ۹` است، که احتمالاً غیرمنتظره است.
...
عملگرهای `~% ^% /% ~/= ^/= ~%= ^%= ~>>= ^>>=` دیگر وجود ندارند.

<h3 className="cmp-func-tolk-header">
  ✅ متغیرهای تغییرناپذیر، توسط `val` اعلام شدند
</h3>

مانند Kotlin: `var` برای متغیرهای قابل تغییر، `val` برای تغییرناپذیر، به صورت انتخابی با یک نوع. در FunC معادلی برای `val` وجود ندارد.
...
پارامترهای یک تابع قابل تغییر هستند، اما از آنجایی که به صورت مقداری کپی می‌شوند، آرگومان‌های فراخوانی تغییری نمی‌کنند. دقیقاً مانند FunC، فقط برای وضوح بیشتر.
...
در Tolk، یک تابع می‌تواند پارامترهای `mutate` را اعلام کند. این یک تعمیم برای توابع ~ FunC تیلدا است، به زیر مراجعه کنید.

<h3 className="cmp-func-tolk-header">
  ✅ گزینه‌های خط فرمان حذفی حذف شدند
</h3>
...
بیش از حد کافی است. از `-v` برای نمایش نسخه و خروج استفاده کنید. از `-h` برای دیدن تمام نشانگر‌های خط فرمان موجود استفاده کنید.

فقط یک فایل ورود می‌تواند پاس داده شود، بقیه باید `import` شوند.

<h3 className="cmp-func-tolk-header">
  ✅ توابع stdlib به اسامی ~~طولانی~~ واضح، به سبک camelCase تغییر نام یافتند
</h3>
...
"stdlib.fc" سابق به فایل‌های متعددی تقسیم شد: common.tolk، tvm-dicts.tolk و دیگران.

اینجا ادامه دهید: [Tolk vs FunC: کتابخانه استاندارد](/v3/documentation/smart-contracts/tolk/tolk-vs-func/stdlib).

<h3 className="cmp-func-tolk-header">
  ✅ stdlib اکنون جاسازی شده است، و از GitHub بارگیری نمی‌شود
</h3>
...
<table className="cmp-func-tolk-table">
  ...
  <tr>
    <td>
      <ol style={{margin: 0}}>
        <li>دانلود stdlib.fc از GitHub</li>
        <li>ذخیره در پروژه خود</li>
        <li>`#include "stdlib.fc";`</li>
        <li>استفاده از توابع استاندارد</li>
      </ol>
    </td>
    <td>
      <ol style={{margin: 0}}>
...
در Tolk، stdlib بخشی از توزیع است. کتابخانه استاندارد غیر قابل تفکیک است، زیرا نگهداری سه‌گانه "زبان، کامپایلر، stdlib" با هم تنها روش صحیح برای حفظ چرخه انتشار است.

این به این صورت کار می‌کند. کامپایلر Tolk می‌داند که چگونه یک کتابخانه استاندارد را پیدا کند. اگر کاربر یک بسته apt نصب کرده باشد، منابع stdlib نیز دانلود شده و روی هارد دیسک موجودند، بنابراین کامپایلر آن‌ها را از طریق مسیرهای سیستم پیدا می‌کند. اگر کاربر از یک پوشش WASM استفاده کند، از طریق tolk-js ارائه می‌شوند. و به همین ترتیب.

کتابخانه استاندارد به چندین فایل تقسیم شده است: `common.tolk` (متداول‌ترین توابع)، `gas-payments.tolk` (محاسبه هزینه‌های gas)، `tvm-dicts.tolk` و سایر فایل‌ها. توابع از `common.tolk` همیشه در دسترس هستند (کامپایلر به طور ضمنی آن را وارد می‌کند). فایل‌های دیگر باید به صورت واضح وارد شوند:
...
به قانون "آنچه را که استفاده می‌کنید وارد کنید" توجه کنید، این قانون به فایل‌های `@stdlib/...` نیز اعمال می‌شود (با تنها استثنای "common.tolk").

پلاگین JetBrains IDE به طور خودکار پوشه stdlib را کشف می‌کند و واردات لازم را هنگام تایپ وارد می‌کند.

<h3 className="cmp-func-tolk-header">
  ✅ عملگرهای منطقی `&& ||`، عدم منطقی `!`
</h3>

در FunC، تنها عملگرهای بیتی `~ & | ^` وجود دارند. توسعه‌دهندگانی که اولین قدم‌ها را بر می‌دارند و فکر می‌کنند "خوب، بدون منطقی، من از بیتی به همان روش استفاده خواهم کرد"، اغلب اشتباه می‌کنند، زیرا رفتار عملگرها کاملاً متفاوت است:
...
<table className="cmp-func-tolk-table">
  ...
  <tbody>
  <tr><td colSpan={2}>گاهی اوقات، مشابه:</td></tr>
  <tr>
  ...
  <tr><td colSpan={2}>اما به طور کلی، نه:</td></tr>
  <tr>
    <td><code>{'1 & 2 = 0'}</code></td>
    <td><code>{'1 && 2 = -1 (صحیح)'}</code></td>
  </tr>
  </tbody>
</table>
...
<table className="cmp-func-tolk-table different-col-widths">
  <thead>
  <tr>
    <th><code>شرط & f()</code></th>
    <th><code>شرط && f()</code></th>
  </tr>
  </thead>
  <tbody>
  <tr>
    <td><code className="inline">f()</code> همیشه فراخوانی می‌شود</td>
    <td><code className="inline">f()</code> تنها اگر <code className="inline">شرط</code> برقرار باشد فراخوانی می‌شود</td>
  </tr>
  </tbody>
</table>
...
<table className="cmp-func-tolk-table different-col-widths">
  ...
  <tr>
    <td><code className="inline">f()</code> همیشه فراخوانی می‌شود</td>
    <td><code className="inline">f()</code> تنها اگر <code className="inline">شرط</code> نادرست باشد فراخوانی می‌شود</td>
  </tr>
  </tbody>
</table>

Tolk عملگرهای منطقی را پشتیبانی می‌کند. آن‌ها دقیقاً همان‌طور که به آن عادت کرده‌اید عمل می‌کنند (ستون سمت راست). در حال حاضر، `&&` و `||` گاهی اوقات کد Fift غیر بهینه تولید می‌کنند، اما در آینده، کامپایلر Tolk در این مورد هوشمندتر خواهد شد. این امر قابل اغماض است، به سادگی مانند سایر زبان‌ها از آن‌ها استفاده کنید.
...
<table className="cmp-func-tolk-table different-col-widths">
  ...
  <tr>
    <td><code dangerouslySetInnerHTML={{__html: 'if (~ found?) {<br>    if (cs~load_int(۳۲) == ۰) {<br>        ...<br>    }<br>}'}}></code></td>
    <td><code dangerouslySetInnerHTML={{__html: 'if (!found && cs.loadInt(۳۲) == ۰) {<br>    ...<br>}'}}></code></td>
  </tr>
  <tr>
...
<h3 className="cmp-func-tolk-header">
  ✅ روش‌های «~» حذف شده‌اند، و به جای آن از کلیدواژه «mutate» استفاده می‌شود
</h3>

این تغییر به قدری بزرگ است که در صفحه جداگانه‌ای توصیف شده است: [قابلیت تغییر Tolk](/v3/documentation/smart-contracts/tolk/tolk-vs-func/mutability).

<hr />

<h3>مقایسه مصرف گاز Tolk و FunC</h3>
