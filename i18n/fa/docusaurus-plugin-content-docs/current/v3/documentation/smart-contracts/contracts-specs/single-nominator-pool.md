...
- Even if the *Validator* wallet is compromised, *Owner* can tell *SingleNominator* to change the validator address. This will prevent the attacker from interacting with *SingleNominator* further. There is no race condition here; *Owner* will always take precedence.
...
The standard [nominator pool](https://github.com/ton-blockchain/nominator-pool) can't prevent all attack scenarios - a malicious validator operator could potentially steal from nominators. This risk doesn't exist with *SingleNominator* since both the *Owner* and *Validator*  are controlled by the same entity.
...
## پیام‌های مخصوص صاحب حساب

صاحب حساب انتخاب‌کننده می‌تواند ۴ عملیات انجام دهد:
...
- single-nominator-addr آدرس انتخاب‌کننده واحد است که صاحب حساب می‌خواهد از آن برداشت کند.
...
- single-nominator-addr آدرس تنها اعتبارسنج است.
