...
یک تراکنش در تون شامل موارد زیر است:
...
- موفقیت، کد خروج ۰ یا ۱
- شکست، `aborted: true` بدون اجرا
- شکست، [کد خروج](/v3/documentation/tvm/tvm-exit-codes)، `aborted: true`
...
برای حالت موجود در تصویر، به این صورت می‌شود: `in_msg_lt < tx0_lt < out_msg_lt`
...
اگر `msg1_lt < msg2_lt` => `tx1_lt < tx2_lt`.
...
1. اولین ترتیب ممکن `tx1'_lt < tx2'_lt` است:
...
2. دومین ترتیب ممکن `tx2'_lt < tx1'_lt` است:
...
ساختار غیرهمزمان زنجیره بلوک TON چالش‌هایی را برای تضمین تحویل پیام‌ها ایجاد می‌کند. زمان منطقی به تعیین ترتیب رویدادها و تراکنش‌ها کمک می‌کند، اما به دلیل مسیرهای متفاوت در زنجیره‌های شارد، ترتیب تحویل پیام‌ها بین چندین قرارداد هوشمند تضمین نمی‌شود. علی‌رغم این پیچیدگی‌ها، TON تضمین می‌کند که پیام‌های داخلی تحویل می‌شوند و اعتماد به شبکه را حفظ می‌کند. توسعه‌دهندگان باید به این جزئیات تطابق شوند تا بتوانند از پتانسیل کامل TON در ساخت برنامه‌های مرکززدایی خلاقانه بهره‌برداری کنند.
