...
- `transfer_id` - int256 تصادفی، همان برای همه messageParts در طول یک انتقال داده است.
- `fec_type` برابر است با `fec.raptorQ`.
  - `data_size` = ۱۵۶
  - `symbol_size` = ۷۶۸
  - `symbols_count` = ۱
- `part` در مورد ما همیشه ۰ است، می‌تواند برای انتقال‌هایی که به محدودیت اندازه می‌رسند استفاده شود.
- `total_size` = ۱۵۶. اندازه داده‌های انتقال ما.
- `seqno` - برای اولین بسته برابر با ۰ است و برای هر بسته بعدی ۱ افزایش خواهد داشت، به عنوان پارامتر برای رمزگشایی و رمزگذاری نماد استفاده می‌شود.
- `data` - نماد رمزگذاری شده ما، به اندازه ۷۶۸ بایت.

پس از سریال‌سازی `rldp.messagePart`، آن را در `adnl.message.custom` بسته‌بندی کرده و از طریق ADNL UDP ارسال کنید.
