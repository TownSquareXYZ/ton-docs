...
هر بسته ADNL TCP، به جز handshake، دارای ساختار زیر است:

- 4 bytes of packet size in little-endian (N)
- ۳۲ بایت nonce (بایت‌های تصادفی برای محافظت در برابر حملات بررسی مجموع)
- (N - 64) بایت payload
- ۳۲ بایت checksum SHA256 از nonce و payload
...
کلاینت ۱۶۰ بایت تصادفی تولید می‌کند که برخی از آن‌ها توسط طرفین به عنوان پایه برای رمزنگاری AES استفاده خواهند شد.

Two permanent AES-CTR ciphers are created, which the parties will use to encrypt/decrypt messages after the handshake.

- رمز A - کلید ۰ - ۳۱ بایت، iv ۶۴ - ۷۹ بایت
- رمز B - کلید ۳۲ - ۶۳ بایت، iv ۸۰ - ۹۵ بایت

The ciphers are utilized in the following order:

- رمز A توسط سرور برای رمزگذاری پیام‌هایی که ارسال می‌کند استفاده می‌شود.
- Cipher A is used by the client to decrypt messages it receives.
- رمز B توسط کلاینت برای رمزگذاری پیام‌هایی که ارسال می‌کند استفاده می‌شود.
- Cipher B is used by the server to decrypt messages it receives.

برای برقراری اتصال، کلاینت باید یک بسته handshake ارسال کند که شامل موارد زیر باشد:

- [32 bytes] **Server key ID** [[see details here]](#getting-key-id)
- [۳۲ بایت] **کلید عمومی ما ed25519**
- [۳۲ بایت] **هش SHA256 از ۱۶۰ بایت ما**
...
از این لحظه به بعد، اتصال برقرار شده محسوب می‌شود.
...
بهترین راه حل این است که هر ۵ ثانیه یک بسته پینگ ارسال شود. این برای حفظ اتصال در زمانی که هیچ داده‌ای منتقل نمی‌شود ضروری است، در غیر این صورت سرور ممکن است اتصال را قطع کند.
...
- ۳۲ بایت nonce -> ۳۲ بایت تصادفی
- ۴ بایت شناسه طرح TL -> **9a2b084d**
- 8 bytes of request-id -> random uint64 number
- ۳۲ بایت checksum SHA256 از nonce و payload

بسته خود را ارسال می‌کنیم و منتظر [tcp.pong](https://github.com/ton-blockchain/ton/blob/ad736c6bc3c06ad54dc6e40d62acbaf5dae41584/tl/generate/scheme/ton_api.tl#L23) می‌مانیم، `random_id` برابر با چیزی خواهد بود که در بسته پینگ ارسال کرده‌ایم.
...
- ۴ بایت از اندازه بسته در قالب لیتل اندین -> ۶۴ + (۴+۳۲+(۱+۴+(۱+۴+۳)+۳)) = **۱۱۶**
- ۳۲ بایت نوآنس -> ۳۲ بایت تصادفی
- ۴ بایت از ID طرح ADNLQuery -> **7af98bb4**
- ۳۲ بایت `query_id:int256` -> ۳۲ بایت تصادفی
  - ۱ بایت اندازه آرایه -> **۱۲**
  - ۴ بایت از ID طرح LiteQuery -> **df068c79**
    - ۱ بایت اندازه آرایه -> **۴**
    - ۴ بایت از ID طرح getMasterchainInfo -> **2ee6b589**
    - ۳ بایت صفر برای پرکن (هم‌ترازی به ۸)
  - ۳ بایت صفر برای پرکن (هم‌ترازی به ۱۶)
- ۳۲ بایت از چک‌سام SHA256 از نوآنس و بار مفید

مثال بسته به صورت هگز:
...
در پاسخ، انتظار داریم [liteServer.masterchainInfo](https://github.com/ton-blockchain/ton/blob/ad736c6bc3c06ad54dc6e40d62acbaf5dae41584/tl/generate/scheme/lite_api.tl#L30)، شامل last:[ton.blockIdExt](https://github.com/ton-blockchain/ton/blob/ad736c6bc3c06ad54dc6e40d62acbaf5dae41584/tl/generate/scheme/tonlib_api.tl#L51) state_root_hash:int256 و init:[tonNode.zeroStateIdExt](https://github.com/ton-blockchain/ton/blob/ad736c6bc3c06ad54dc6e40d62acbaf5dae41584/tl/generate/scheme/ton_api.tl#L359) را دریافت کنیم.

بسته دریافتی به همان صورت که فرستاده شده است، غیر سریالی می‌شود - همان الگوریتم را دارد اما در جهت مخالف، مگر اینکه پاسخ تنها در [ADNLAnswer](https://github.com/ton-blockchain/ton/blob/ad736c6bc3c06ad54dc6e40d62acbaf5dae41584/tl/generate/scheme/lite_api.tl#L23) پیچیده شود.

پس از رمزگشایی پاسخ، بسته‌ای به فرم زیر دریافت می‌کنیم:
...
برای اجرای روش قرارداد هوشمند، باید با استفاده از طرح TL یک درخواست ایجاد کرده و ارسال کنیم:

```tlb
liteServer.runSmcMethod mode:# id:tonNode.blockIdExt account:liteServer.accountId method_id:long params:bytes = liteServer.RunMethodResult
```

و منتظر پاسخ با طرح باشید:

```tlb
liteServer.runMethodResult mode:# id:tonNode.blockIdExt shardblk:tonNode.blockIdExt shard_proof:mode.0?bytes proof:mode.0?bytes state_proof:mode.1?bytes init_c7:mode.3?bytes lib_extras:mode.4?bytes exit_code:int result:mode.2?bytes = liteServer.RunMethodResult;
```

در درخواست می‌بینیم که شامل فیلدهای زیر است:

- mode:# - ماسک بیتی 32 بیتی از آنچه که می‌خواهیم در پاسخ ببینیم، به عنوان مثال، `result:mode.2?bytes` تنها اگر بیت با شاخص ۲ به یک تنظیم شده باشد در پاسخ حضور دارد.
- id:tonNode.blockIdExt - our master block state that we got in the previous chapter.
- account:[liteServer.accountId](https://github.com/ton-blockchain/ton/blob/ad736c6bc3c06ad54dc6e40d62acbaf5dae41584/tl/generate/scheme/lite_api.tl#L27) - داده‌های آدرس کارچین و قرارداد هوشمند.
- method_id:long - ۸ بایت که در آن crc16 با جدول XMODEM به نمایندگی از روش فراخوانده شده نوشته شده است + بیت ۱۷ تنظیم می‌شود [[محاسبه]](https://github.com/xssnick/tonutils-go/blob/88f83bc3554ca78453dd1a42e9e9ea82554e3dd2/ton/runmethod.go#L16)
- params:bytes - [Stack](https://github.com/ton-blockchain/ton/blob/ad736c6bc3c06ad54dc6e40d62acbaf5dae41584/crypto/block/block.tlb#L783) سریالی شده در [BoC](/v3/documentation/data-formats/tlb/cell-boc#bag-of-cells)، حاوی آرگومان‌هایی برای فراخوانی متد. [[مثال پیاده‌سازی](https://github.com/xssnick/tonutils-go/blob/88f83bc3554ca78453dd1a42e9e9ea82554e3dd2/tlb/stack.go)]

به عنوان مثال، ما فقط به `result:mode.2?bytes` نیاز داریم، بنابراین mode ما برابر با 0b100 یعنی ۴ خواهد بود. در پاسخ، ما دریافت خواهیم کرد:

- mode:# -> آنچه ارسال شده است - ۴.
- id:tonNode.blockIdExt -> بلاک اصلی ما که روش بر مبنای آن اجرا شد
- shardblk:tonNode.blockIdExt -> بلاک شاردی که حساب قرارداد در آن قرار دارد
- exit_code:int -> ۴ بایت که کد خروج هنگام اجرای روش است. اگر همه چیز موفقیت‌آمیز باشد، برابر با ۰ است، در غیر این صورت، برابر با کد استثنا است.
- result:mode.2?bytes -> [Stack](https://github.com/ton-blockchain/ton/blob/ad736c6bc3c06ad54dc6e40d62acbaf5dae41584/crypto/block/block.tlb#L783) سریالی شده در [BoC](/v3/documentation/data-formats/tlb/cell-boc#bag-of-cells)، که مقادیر بازگردانده شده توسط متد را شامل می‌شود.

بیایید تماس و گرفتن نتیجه از روش `a2` قرارداد `EQBL2_3lMiyywU17g-or8N7v9hDmPCpttzBPE2isF2GTzpK4` را تحلیل کنیم:
...
درخواست ما را پر کنید:

- `mode` = ۴، ما فقط نتیجه را نیاز داریم -> `04000000`
- `id` = نتیجه اجرای getMasterchainInfo
- `account` = workchain ۰ (۴ بایت `00000000`)، و int256 [به‌دست‌آمده از آدرس قرارداد ما](/v3/documentation/data-formats/tlb/tl-b-types#addresses)، یعنی ۳۲ بایت `4bdbfde5322cb2c14d7b83ea2bf0deeff610e63c2a6db7304f1368ac176193ce`
- `method_id` = [محاسبه شده](https://github.com/xssnick/tonutils-go/blob/88f83bc3554ca78453dd1a42e9e9ea82554e3dd2/ton/runmethod.go#L16) id از `a2` -> `0a2e010000000000`
- `params:bytes` = روش ما پارامترهای ورودی نمی‌پذیرد، بنابراین نیاز داریم به آن یک استک خالی (`000000`, cell 3 بایت - عمق استک 0) سریالی شده در [BoC](/v3/documentation/data-formats/tlb/cell-boc#bag-of-cells) -> `b5ee9c72010101010005000006000000` -> به بایت‌ها سریال کنید و `10b5ee9c72410101010005000006000000000000` 0x10 به دست آورید - اندازه، 3 بایت در انتهای - padding.

در پاسخ، ما دریافت می‌کنیم:

- `mode:#` -> جالب نیست
- `id:tonNode.blockIdExt` -> جالب نیست
- `shardblk:tonNode.blockIdExt` -> جالب نیست
- `exit_code:int` -> برابر با ۰ است اگر اجرا موفقیت‌آمیز باشد
- `result:mode.2?bytes` -> [Stack](https://github.com/ton-blockchain/ton/blob/ad736c6bc3c06ad54dc6e40d62acbaf5dae41584/crypto/block/block.tlb#L783) که داده‌های بازگردانده شده توسط متد در قالب [BoC](/v3/documentation/data-formats/tlb/cell-boc#bag-of-cells) را شامل می‌شود، ما آن را باز خواهیم کرد.

داخل `result` ما `b5ee9c7201010501001b000208000002030102020203030400080ccffcc1000000080aabbcc8` را دریافت کردیم، که این [BoC](/v3/documentation/data-formats/tlb/cell-boc#bag-of-cells) حاوی داده‌ها است. زمانی که آن را از قالب خارج کنیم، یک Cell دریافت خواهیم کرد:
...
خواهیم دید که اولین لینک `rest:^(VmStackList n)` - cell مقدار بعدی در استک است و مقدار ما `tos:VmStackValue` دومی می‌آید، بنابراین برای به دست آوردن مقدار باید لینک دوم که `32[0CCFFCC1]` بخوانیم - این cell اول ماست که قرارداد برگشت.

حالا می‌توانیم عمیق‌تر به میان برویم و عنصر دوم استک را بگیریم، از طریق اولین لینک می‌رویم، اکنون داریم:
...
ما همان فرآیند را تکرار می‌کنیم. اولین ۸ بیت = `03` - یعنی دوباره cell. مرجع دوم مقدار `32[0AABBCC8]` است و از آنجا که عمق stack ما ۲ است، عبور را کامل می‌کنیم. در مجموع، ما ۲ مقدار داریم که توسط قرارداد بازگشت داده شده است - `32[0CCFFCC1]` و `32[0AABBCC8]`.

توجه داشته باشید که آنها به‌طور معکوس هستند. به همین روش، وقتی فراخوانی عملکردی را ایجاد می‌کنید، باید آرگومان‌ها را به صورت معکوس از آنچه در کد FunC می‌بینیم، منتقل کنید.
...
برای دریافت داده‌های وضعیت حساب مانند موجودی، کد و داده‌های قرارداد، می‌توانیم از [getAccountState](https://github.com/ton-blockchain/ton/blob/ad736c6bc3c06ad54dc6e40d62acbaf5dae41584/tl/generate/scheme/lite_api.tl#L68) استفاده کنیم. برای درخواست، ما به یک [بلوک تازه master](#getmasterchaininfo) و آدرس حساب نیاز داریم. در پاسخ، ساختار TL [AccountState](https://github.com/ton-blockchain/ton/blob/ad736c6bc3c06ad54dc6e40d62acbaf5dae41584/tl/generate/scheme/lite_api.tl#L38) را دریافت خواهیم کرد.

بیایید طرح TL `AccountState` را تحلیل کنیم:
...
- `state` - [BoC](/v3/documentation/data-formats/tlb/cell-boc#bag-of-cells) TL-B [طرح وضعیت حساب](https://github.com/ton-blockchain/ton/blob/ad736c6bc3c06ad54dc6e40d62acbaf5dae41584/crypto/block/block.tlb#L232).

از تمام این داده‌ها، آنچه به آن نیاز داریم در state است، ما آن را تحلیل خواهیم کرد.

به عنوان مثال، وضعیت حساب `EQAhE3sLxHZpsyZ_HecMuwzvXHKLjYx4kEUehhOy2JmCcHCT` را بگیریم، `state` در پاسخ خواهد بود (در زمان نوشتن این مقاله):
...
حالا نیاز داریم که cell را بر اساس ساختار TL-B تجزیه کنیم:
...
ساختار ما به ساختارهای دیگر اشاره می‌کند، مانند:
...
اولین بیت ما از داده‌های بالا = ۱ است، بنابراین ما با `account$1` کار می‌کنیم و از طرح استفاده خواهیم کرد:
...
در مورد ما، n برابر با ۷ خواهد بود. در len ما `(#< 7)` خواهیم داشت که به معنای تعداد بیت‌هایی است که می‌توانند عددی تا حداکثر ۷ را نگهداری کنند. شما می‌توانید آن را با تبدیل ۷-۱=۶ به شکل دودویی - `110`، ما ۳ بیت می‌گیریم، بنابراین طول len = ۳ بیت. و حالت `(uint (len * 8))`. برای تعیین آن، نیاز است که ۳ بیت طول را بخوانیم، یک عدد بگیریم و در ۸ ضرب کنیم، این اندازه `value` خواهد بود، یعنی تعداد بیت‌هایی که باید خوانده شود تا ارزش VarUInteger به دست آید.

`cells:(VarUInteger 7)` را بخوانید، بیت‌های بعدی خود را از cell ریشه‌ای بگیرید، به ۱۶ بیت بعدی نگاه کنید تا متوجه شوید، این `0010011010101000` است. ۳ بیت اول len را می‌خوانیم، این `001`، یعنی ۱ است، اندازه را (uint (1 \* 8)) می‌گیریم، uint 8 می‌گیریم، ۸ بیت می‌خوانیم، آن `cells` خواهد بود، `00110101`، یعنی ۵۳ در شکل ده‌دهی. برای `bits` و `public_cells` نیز همین را انجام می‌دهیم.

ما با موفقیت `used:StorageUsed` را خواندیم، سپس `last_paid:uint32` را داریم، ما ۳۲ بیت می‌خوانیم. همه چیز با `due_payment:(Maybe Grams)` همینقدر ساده است، اینجا Maybe که ۰ خواهد بود، بنابراین Grams را رد می‌کنیم. اما، اگر Maybe ۱ باشد، می‌توانیم به گرام‌های `amount:(VarUInteger 16) = Grams` نگاه کنیم و سریعاً درک کنیم که چگونه با این کار کنیم. مانند دفعه قبل، تنها تفاوت این است که به جای ۷ ما ۱۶ داریم.

بعدی ما `storage:AccountStorage` داریم با یک طرح:

```tlb
account_storage$_ last_trans_lt:uint64 balance:CurrencyCollection state:AccountState = AccountStorage;
```

ما `last_trans_lt:uint64` را می‌خوانیم، این ۶۴ بیت است، که lt آخرین تراکنش حساب را ذخیره می‌کند. و در آخر، موجودی که با طرح نشان داده شده است:
...
بیایید بیت‌های باقی‌مانده خود را با توجه به داده‌های خود تجزیه و تحلیل کنیم:

```
100000000111010111110100100011100111011011101001111101100000011000000100110
```

ابتدا 4 bit را بخوانید - `1000`، این یک 8 است. 8\*8=64، 64 bit بعدی را بخوانید: `0000011101011111010010001110011101101110100111110110000001100000`. با حذف bitهای صفر اضافی، به دست می‌آوریم: `11101011111010010001110011101101110100111110110000001100000`، که برابر است با `531223439883591776` و با ترجمه از nano به TON، ما به `531223439.883591776` می‌رسیم.

ما در اینجا متوقف خواهیم شد، زیرا ما قبلاً همه موارد اصلی را تجزیه و تحلیل کرده‌ایم، بقیه را می‌توان به روش مشابه با آنچه که ما آنالیز کرده‌ایم به‌دست آورد. همچنین می‌توانید اطلاعات اضافی در مورد تجزیه TL-B را در [مستندات رسمی](/v3/documentation/data-formats/tlb/tl-b-language) پیدا کنید
...
## جزئیات فنی اضافی از دست دادن

### دریافت شناسه کلید

شناسه کلید، هش SHA256 از اسکیمای TL سریالی شده است.

اسکیمای TLکه بیشتر برای کلیدها استفاده می‌شوند عبارتند از:
...
### رمزگذاری داده بسته دست دادن

بسته دست دادن به صورت نیمه‌ باز ارسال می‌شود، فقط 160 بایت رمزگذاری شده که شامل اطلاعات رمزهای دائمی هستند.

برای رمزگذاری آنها، ما به یک رمز AES-CTR نیاز داریم ، برای به دست آوردن آن، نیاز به یک هش SHA256 از 160 بایت و [کلید مشترک ECDH](#getting-a-shared-key-using-ecdh) داریم

رمز به این صورت ساخته می‌شود:

- کلید = (0 تا 15 بایت از کلید عمومی) + (16 تا 31 بایت از هش)
- iv = (0 تا 3 بایت از هش) + (20 تا 31 بایت از کلید عمومی)

پس از ساخت رمز، ما 160 بایت خود را با آن رمزگذاری می‌کنیم.
...
### دریافت کلید مشترک با استفاده از ECDH

برای محاسبه کلید مشترک، ما به کلید خصوصی خود و کلید عمومی سرور نیاز داریم.

ماهیت DH این است که یک کلید مخفی مشترک بدست آوریم، بدون اینکه اطلاعات خصوصی را فاش کنیم. من یک مثال از چگونگی وقوع این امر را به ساده‌ترین شکل ممکن ارائه خواهم داد. فرض کنیم که باید یک کلید مشترک بین ما و سرور تولید کنیم، فرآیند به این صورت خواهد بود:

- ما اعداد مخفی و عمومی مانند **6** و **7** تولید می‌کنیم
- سرور اعداد مخفی و عمومی مانند **5** و **15** تولید می‌کند
- ما اعداد عمومی را با سرور مبادله می‌کنیم، **7** را به سرور می‌فرستیم، و آن به ما **15** می‌فرستد.
- ما محاسبه می‌کنیم: **7^6 mod 15 = 4**
- سرور محاسبه می‌کند: **7^5 mod 15 = 7**
- ما اعداد دریافتی را مبادله می‌کنیم، ما به سرور **4** می‌دهیم و آن به ما **7** می‌دهد
- ما محاسبه می‌کنیم: **7^6 mod 15 = 4**
- سرور محاسبه می‌کند: **4^5 mod 15 = 4**
- کلید مشترک = **4**

جزئیات خود ECDH برای سادگی حذف خواهد شد. این با استفاده از ۲ کلید، خصوصی و عمومی، از طریق یافتن یک نقطه مشترک بر روی منحنی محاسبه می‌شود. اگر علاقه‌مند هستید، بهتر است به صورت جداگانه درباره آن بخوانید.
