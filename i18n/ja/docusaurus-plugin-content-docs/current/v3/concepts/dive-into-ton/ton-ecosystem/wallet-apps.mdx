...
この記事では、開発の観点からウォレットについて説明します。最終的な目標は、TONの大量採用をサポートするウォレットアプリケーションを作成することです。
...
2. トキーパーのメイン画面に戻ります。
3. 画面の上部にあるウォレット名をクリックします。
4. Push the Add Wallet button at the bottom of the screen.
5. format@@0オプションを選択します。
6. 手順1のリカバリフレーズを入力します。
...
#### MyTonWalletテスト環境 (ブラウザ、デスクトップ、モバイルアプリケーション)

1. 設定セクションに移動します。
2. 下にスクロールし、MyTonWalletアプリケーションのバージョンをクリックします。
3. ポップアップウィンドウで環境を選択します。
...
ハードウェア財布は、インターネットから離れてあなたの暗号通貨資金への秘密鍵を保存する物理的なデバイスです。 たとえ取引を行ったとしても、ウォレットはオフライン環境でトランザクションを確認します。 このプロセスは、秘密鍵を常にインターネットのリスクから遠ざけるのに役立ちます。
...
- [Ledger TON ブログポスト](https://blog.ton.org/ton-is-coming-to-ledger-hardware-wallets) LedgerウォレットのTONのユーザーマニュアル
- [Ledger](https://www.ledger.com/) 公式サイト

### SafePal

[SafePal](https://www.safepal.com/en/) は、急速に拡大する分散アプリケーションの銀河へのゲートウェイです。

#### リンク

- [SafePal](https://www.safepal.com/en/) 公式サイト
...
[@wallet](https://t.me/wallet) — TelegramでP2Pを使ってTNを送受信またはトレードするボットです。Telegram MiniアプリUIをサポートしています。

<img src="/img/docs/wallet-apps/Wallet.png?raw=true" alt="Tonkeeper" width="240" height="480" style={{ display: 'block', margin: 'auto' }}/>

### @cryptobot

[@cryptobot](https://t.me/cryptobot) — TONを保管、送金、交換するテレグラムボットウォレット。
...
TonkeyはTON Blockchainにマルチシグネチャ機能を導入する高度なプロジェクトです。
