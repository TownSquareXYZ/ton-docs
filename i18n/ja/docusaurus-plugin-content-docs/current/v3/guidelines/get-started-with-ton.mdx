...
では、**TON Developer**になるための最初のステップに焦点を当てて、TONでNFTを採掘する方法を学びましょう！ 以下は、私たちが目指しているものの例です。
...
1. スマートフォンにTonkeeperアプリをインストールします。 [here](https://Tonkeeper.com/)をダウンロードできます。
...
- **NodeJS**: Node.jsはJavaScriptとTypeScriptのランタイム環境で、通常TONでアプリケーション開発に使用されます。 [here](https://nodejs.org/en/download/)をダウンロードできます。
- **JavaScript IDE**。JavaScript IDEは通常、ローカルの開発環境で開発に使用されます。このケースの例は Visual Studio Code ([VSCode](https://code.visualstudio.com/download)です。
...
- TON Blockchainへのリクエストを行うための **API provider_** 。TON には、異なる目的で複数の [API タイプ](/v3/guidelines/dapps/apis-sdks/api-types) があります。 テストネット版の [toncenter.com](https://toncenter.com/) APIを使用します。
...
鉱夫が正しく動作するには、2つの異なるスマートコントラクトアドレスタイプを追加する必要があります。
...
`Address. `@ton/ton` SDK にある arse()` メソッドを使用すると、開発者はアドレスオブジェクトを作成して、あるフォームから別のフォームに変換することができます。
...
TONオンボーディングチャレンジを完了するために必要な[README ファイル](https://github.com/ton-community/ton-onboarding-challenge#mining-process-deep-dive)を参照することで、最新のTONマイニングデータは`get_mining_data`メソッドを実行することで得られます。 開始すると、結果は次のようになります:
...
このプロセスが完了したら、コンソールに値を出力できます。コマンドを実行して、スクリプトを再実行してみてください。
...
- `complexity`は鉱山労働者にとって最も重要な数です。値のプルーフ・オブ・ワークの複雑さです。最終的なハッシュがcomplexity_より小さい場合は成功します。
...
- チュートリアルでは`targetDelta`、`minCpl`、`maxCpl`は使用されません。 しかし、スマートコントラクトでこれらがどのように使用されているかについては、プロジェクト内のコレクションのソースファイルで作業証明の複雑さを計算するための詳細をいつでも読むことができます。
...
幸いなことに、このチュートリアルで使用されるすべてのデータ構造はTypeScriptで既に記述されています。 _NftGiver.data.ts_ の `MineMessageParams` オブジェクトを使用して、_Queries_ でトランザクションをビルドします:
...
しばらくすると、NFT__ を採掘して TON ブロックチェーンで TON Developer になります。 儀式が完了しました。トンキーパーでNFTを見てください。
...
_まず休憩しましょう！あなたは大きなタスクをしました。あなたは今TON 開発者です。しかし、それは長い道のりの始まりにすぎません。_
...
:::info 何かご意見は?
あなたはここにいる最初の探検家の一人です。間違いや気分が重なっている場合は、[@SwiftAdviser](https://t)にフィードバックを送ってください。 e/SwiftAdviser). できるだけ早く修正します! :)
:::
