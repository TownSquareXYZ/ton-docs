...
- 新しい保管契約が受け入れられるかどうか。
- 最小と最大の*バッグ*サイズ(バイト単位)です。
- 料金 - ストレージコスト。1日あたりnanoTONで指定されています。
- 最大スパン:どのくらいの頻度でプロバイダが*Bag*ストレージの証明を提供すべきか。
...
パラメータ`accept`を`1`に設定すると、スマートコントラクトはクライアントからのリクエストの受け付けとストレージコントラクトの作成を開始します。 ストレージデーモンが自動的に処理しますが、*バッグ*をダウンロードして配布し、ストレージ証明を生成します。
