...
1. `blockchain.openContract()`を使用して特定のラップされた`Contract`エンティティを作成します。
2. Describe the actions your `Contract` should perform and save the execution result in the `res` variable.
3. `expect()` 関数と `toHaveTransaction()` マッチャを使用してプロパティを確認します。
...
| 成功      | boolean? | Custom Sandbox flag that defines the resulting status of a certain transaction. True if both the compute and the action phases succeeded. Otherwise, False. |
...
TONの最も貴重なコミュニティチュートリアルからテストの詳細をご覧ください:

- [レッスン 2: スマートコントラクトのテスト](https://github.com/romanovichim/TonFunClessons_Eng/blob/main/lessons/smartcontract/2lesson/secondlesson.md)
...
- [MassSender.spec.ts](https://github.com/Gusarich/ton-mass-sender/blob/main/tests/MassSender.spec.s)
- [Assurer.spec.ts](https://github.com/aSpite/dominant-assurance-contract/blob/main/tests/Assurer.spec.s)
