...
2. **条件付きデータ検索**： 現在時刻など、ある条件によって検索すべきデータが変わることがあります。
...
このメソッドは、jetton ウォレットに関連付けられたデータの完全なセットを返します。

- (int) 残高
- (スライス) owner_address
- (スライス) jetton_master_address
...
合計供給量、管理者のアドレス、jettonの内容、およびそのウォレットコードを含む、jettonマスターのデータを返します。
...
getメソッドは、format@@0タブで呼び出すことができます。

- https://ton.cx/address/EQAWrNGl875lXA6Fff7nIOwTiwiJMq0SmtJ5Txhgnz4tXI

### コードから get メソッドを呼び出します

以下の例では Javascript ライブラリとツールを使用します。

- [ton](https://github.com/ton-org/ton/) library
- [Blueprint](/v3/documentation/smart-contracts/getting-started/javascript/)

例えば、以下の get メソッドとの契約があるとしましょう。
...
このメソッドは、コントラクトデータから読み込まれた単一の数値を返します。
...
最後に、テストでこのメソッドを使用できます。`tests/Counter.spec.ts`に移動して、新しいテストを追加します。
...
この例では、コントラクトは、操作コードを解釈し、特定のメソッドを実行し、適切に応答を返すことによって、内部メッセージを受信して処理します。

- Op-code `1`は、契約データの番号を更新する要求を示します。
- Op-code `2`は、コントラクトのデータから番号を問い合わせるリクエストを意味します。
- Op-code `3` is used in the response message, which the calling smart contract must handle to receive the result.

簡単のため、操作コードには単純な小さな数字1、2、3だけを使用しました。しかし、実際のプロジェクトでは、標準に従って設定してください：
