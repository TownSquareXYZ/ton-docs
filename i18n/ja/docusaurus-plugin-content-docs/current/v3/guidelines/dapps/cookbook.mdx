...
### 文字列からアドレスを変換（user friendly <-> raw）、アセンブル、抽出するには？
...
| アドレスの最初の文字 |   バイナリ形式   |  バウンス可能 |   テストネットのみ  |
| :--------: | :--------: | :-----: | :---------: |
| E...       | 000100.01  |  はい     | いいえ         |
| U...       | 010100.01  | いいえ     | いいえ         |
| k...       | 100100.01  |  はい     |  はい         |
...
例全体を[ここ](/example-code-snippets/pythoniq/jetton-offline-address-calc-wrapper.py)で読みます。
...
:::info
/ton-blockchain /token-contract の NFT 標準では指定されていません。
:::
...
次に、合計トランザクションコストを正確に計算する必要があります。`0.015`の値はテストによって得られましたが、それぞれのケースで異なる場合があります。 これは主にNFTのコンテンツによって異なります。コンテンツのサイズが大きくなると**前方料金**（配送料金）が高くなります。

### コレクションのスマートコントラクトの所有者を変更するには？
...
- [Dedustドキュメント](https://docs.dedust.io/)
...
これは Toncoin **vault** への転送の本体のスキームです。

まず、スワップするJettonの**vault**アドレス、またはToncoinの**vault**アドレスを知る必要があります。これはコントラクト[**Factory**](https://docs.dedust.io/reference/factory)の`get_vault_address`取得メソッドを使用して行うことができます。引数として、スキームに従ってスライスを渡す必要があります：
...
また、交換自体のためには、`get_pool_address`メソッドから取得された**pool**アドレスが必要です。 引数として - 上記のスキームに従ってアセットスライスを指定します。 レスポンスでは、両方のメソッドは要求された **vault** / **pool** のアドレスのスライスを返します。
...
以下は、Dedustがアセット・コンセプトのために導入したTL-Bスキーマです。
...
dedust SDKも持参する必要があります。

```bash
npm install --save @dedust/sdk
```

いくつかのオブジェクトを初期化する必要があります。
...
トークンXをYとスワップする場合もプロセスは同じです。例えば、私たちはXトークンをVault Xに送ります。Vault Xは
、私たちのアセットを受け取り、それを保持し、このアドレスがスワップを要求していることを(X, Y) のPoolに通知します。

例えば、Jettonの場合、転送メッセージを使用してVaultに転送し、特定のforward_payloadを添付しますが、ネイティブコインの場合は、対応するTONの量を添付してVaultにスワップメッセージを送信します。

これは TON と jetton のスキーマです:

```tlb
swap#ea06185d query_id:uint64 amount:Coins _:SwapStep swap_params:^SwapParams = InMsgBody;
```

そのため、すべての保管庫と対応するプールは、特定のスワップ用に設計されており、特別なアセットに合わせた特別なAPIを備えています。

これはTONとjetton SCALEを入れ替えたものです。jettonとjettonを交換するプロセスは同じです。唯一の違いは、TL-Bスキーマに記述されているペイロードを提供しなければならないことです。
...
アカウントのトランザクションのリストは `getTransactions` API メソッドで取得することができます。これは `Transaction` オブジェクトの配列を返し、各項目は多くの属性を持ちます。しかし、最もよく使われるフィールドは以下のとおりです：

- このトランザクションを開始したメッセージの送信者、本文、値
- トランザクションのハッシュと論理時間（LT）

_Sender_フィールドと_Body_フィールドは、メッセージの種類(通常の転送、jetton転送、nft転送など) を決定するために使用できます。

以下は、任意のブロックチェーンアカウントで最新の5つのトランザクションをフェッチする方法の例です。 型に応じて解析し、ループで出力します。
...
### 特定のTON Connect結果のトランザクションを見つける方法は?
...
リスニングブロックチェーンを試みるために `retry` 関数を準備してください。
...
### トランザクションまたはメッセージのハッシュを見つけるにはどうすればよいですか?
