...
- **スケーラビリティ**: 大量のユーザーにトークンを配布する場合、従来の鋳造プロセスはリソース集約的でコストがかかります。
...
- ルート`merkle_hash`を計算します。

2. Deploy a jetton master contract:

- コントラクトのストレージに `merkle_hash` を含めます。
...
- コンパイルされたファイルは `build/crypto/mintless-proof-generator` として保存されます。
