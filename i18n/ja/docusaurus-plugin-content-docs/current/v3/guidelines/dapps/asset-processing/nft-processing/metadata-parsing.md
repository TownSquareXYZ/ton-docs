...
TONでは、エンティティはオンチェーン、セミチェーン、オフチェーンの3種類のメタデータを持つことができます。

- **オンチェーンメタデータ**：名前、属性、画像を含むデータがブロックチェーン内部に保存されます。
...
以下はTypeScriptでのSnakeフォーマットのエンコードとデコードの例です：
...
| 属性            | タイプ      | 必要条件 | 説明                                                                                                                      |
| ------------- | -------- | ---- | ----------------------------------------------------------------------------------------------------------------------- |
| `uri`         | ASCII文字列 | 任意   | A URI pointing to the JSON document with metadata used by the "Semi-chain content layout."              |
| `name`        | UTF8文字列  | 任意   | Identifies the asset.                                                                                   |
| `description` | UTF8文字列  | 任意   | Describes the asset.                                                                                    |
| `image`       | ASCII文字列 | 任意   | A URI pointing to a resource with a MIME type image.                                                    |
| `image_data`  | バイナリー    | 任意   | Either a binary representation of the image for the on-chain layout or base64 for the off-chain layout. |

## Jettonのメタデータ属性

1. `uri` - 任意。「セミチェーンコンテンツレイアウト」で使用されるASCII文字列で、メタデータを含むJSONドキュメントを指すURIです。
2. `name` - 任意。UTF8文字列で資産を識別します。
3. `description` - 任意。UTF8文字列で資産についてを説明します。
...
6. `symbol` - 任意。UTF8文字列。トークンのシンボル - 例えば"XMPL"。"You received 99 XMPL"という形式で使用されます。
...
| `description`  | UTF8文字列  | 任意   | Describes the asset                                                                                                                                                                                                                                                                                                                                                   |
| `image`        | ASCII文字列 | 任意   | A URI pointing to a resource with a mime type image                                                                                                                                                                                                                                                                                                                   |
| `image_data`   | バイナリー    | 任意   | Either a binary representation of the image of an on-chain layout or base64 for off-chain layout                                                                                                                                                                                                                                                                      |
| `symbol`       | UTF8文字列  | 任意   | Token symbol - for example, "XMPL" and uses the form "You have received 99 XMPL"                                                                                                                                                                                                                                                                                      |
| `decimals`     | UTF8文字列  | 任意   | The number of decimal places used by the token. If not specified, the default is 9. A UTF8-encoded string with numbers from 0 to 255. - for example, 8 means that the token amount must be divided by 100000000 to get its custom representation.                                                     |
| `amount_style` |          | 任意   | Required by external applications to understand the format of displaying the number of tokens. Defined using *n*, *n-of-total*, *%*.                                                                                                                                                                                                  |
| `render_type`  |          | 任意   | Needed by external applications to understand what group a token belongs to and how to display it. "currency" - displays as currency (default). "game" - display used for games, displays as NFT, but also displays the number of tokens and respects the amount_style value. |

> `amount_style` パラメータ:

- *n* - Jetton数（デフォルト値）。ユーザーが10進数0の100個のトークンを持つ場合、ユーザーが100個のトークンを持っていることが表示されます。
...
オンチェーンのNFT データを取得した後は、それを解析する必要があります。この処理を行うには、NFT の内部構造を構成する最初のバイトを読み取ることにより、NFT のコンテントタイプを判定する必要があります。
...
1. NFTメタデータを表示するには、`name`、`description`、`image`（または `image_data`）フィールドが必要です。
...
- [TONエンハンスメントプロポーザル64（TEP-64）](https://github.com/ton-blockchain/TEPs/blob/master/text/0064-token-data-standard.md)

## See also

- [TON におけるNFT処理](/v3/guidelines/dapps/asset-processing/nft-processing/nfts)
