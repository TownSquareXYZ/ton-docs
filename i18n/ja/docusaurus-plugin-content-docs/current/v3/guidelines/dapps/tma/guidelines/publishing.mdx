...
テキストデータの受信だけでは不十分な場合や、より高度でパーソナライズされたインターフェイスが必要な場合は、**web_app**タイプの[Inline KeyboardButton](https://core.telegram.org/bots/api#inlinekeyboardbutton)を使用してミニアプリを開くことができます。
...
デフォルトでは、ボットとのチャットには、リストされたすべての[コマンド](https://core.telegram.org/bots#commands)にすばやくアクセスできる便利な**メニューボタン**が常に表示されます。Bot API 6.0](https://core.telegram.org/bots/api-changelog#april-16-2022)では、このボタンを **ミニアプリ** の起動に使用できます。
...
- ボタンを**全ユーザー**用にカスタマイズするには、[@BotFather](https://t.me/botfather) (/setmenubuttonコマンドまたは_Bot Settings > Menu Button_)を使用します。
...
インラインミニアプリはチャットに**アクセス**できません - ユーザーの代わりにメッセージを読んだり、新しいメッセージを送信することはできません。メッセージを送信するには、ユーザーが**インラインモード**にリダイレクトされ、アクティブに結果を選択する必要があります。

**以下に適しています:**

- インラインモードでの本格的なウェブサービスと統合
