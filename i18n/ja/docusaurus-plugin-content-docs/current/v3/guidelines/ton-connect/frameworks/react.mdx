...
お勧めの React Apps は [UI React SDK](/v3/guidelines/ton-connect/guidelines/developers#ton-connect-react) です。 これは、TON Connect とやり取りする高レベルの方法を提供する React コンポーネントです。
...
マニフェストファイルを作成したら、MiniアプリのルートにTonConnectUIProviderをインポートし、マニフェストURLを渡します。
...
ウォレット接続後、[Telegram Mini App](/v3/guidelines/dapps/tma/overview)にユーザーをリダイレクトしたい場合は、`TonConnectUIProvider`要素をカスタマイズできます。
...
[GitHubで例を開く](https://github.com/ton-connect/demo-dapp-with-wallet/blob/master/src/App.tsx)
...
React アプリケーションで低レベルの TON Connect UI SDK 機能を使用したい場合は、`@tonconnect/ui-react` パッケージからフックを使用できます。
...
モーダルウィンドウを開閉するための関数にアクセスするには、このフックを使用します。 フックは、モーダルを開閉するための現在のモーダル状態とメソッドを持つオブジェクトを返します。
...
さらに、接続されているウォレットの名前、画像、その他の属性などの詳細情報にもアクセスできます([WalletInfo interface](https://ton-connect.github.io/sdk/types/_tonconnect_sdk.WalletInfo.html)を参照してください)。
...
[TonConnectUI インスタンスメソッドについて詳しく理解する](https://github.com/ton-connect/sdk/tree/main/packages/ui#send-transaction)
...
ユーザーが本当にアドレスを所有していることを確認するには、 `ton_proof` を使用します。

`tonConnectUI.setConnectRequestParameters` 関数を使用して、接続リクエストパラメータを設定します。

- Loading State:バックエンドからの応答を待っている間にロード状態を表示します。
- Ready State with tonProof: 状態を「準備完了」に設定し、tonProof値を含めます。
- エラーが発生した場合は、ローダーを取り外し、追加のパラメータなしで接続要求を作成します。
...
ウォレットが接続されていると、`wallet`オブジェクトに`ton_proof`結果が表示されます。
