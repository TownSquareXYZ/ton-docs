...
これを使用すると、`TonConnectUI`インスタンスとUIオプションの更新関数にアクセスできます。
...
:::tip
メッセージの署名と確認方法を理解する: [サインと認証](/v3/guidelines/ton-connect/guidelines/verifying-signed-in-users)
:::
...
アニメーションが動作しない場合に問題が発生している場合 ウェブアニメーション API のサポートがないためかもしれません。 この問題を解決するには、 `web-animations-js` polyfill を使用します。
...
または、次のスクリプトタグをHTMLに追加することで、CDN経由でポリフィルを含めることもできます。
