...
メッセージを作り出す前に、細胞の概念を紹介しましょう。どのメッセージ本体が作られているのか。
...
各セルには1023ビットの制限があります。これを超えると、エラーが発生します。
...
参照先(ネストされた)セルをロードするには、`loadRef()`を使用します。
...
手動でセルを扱うのは退屈な作業なので、 `@ton-community/assets-sdk` はメッセージのシリアライズとデシリアライズのための便利なメソッドを提供します。

`@ton-community/assets-sdk` を使用すると読みやすくなり、エラーが発生しやすくなります。
...
開発者がどのレベルのタスクを解決しても、通常は `@tonconnect/sdk` または `@tonconnect/ui` からコネクタエンティティを使用する必要があります。
`@tonconnect/sdk`と`@tonconnect/ui`に基づいて作成された例：
...
TON コネクト SDKにはメッセージ送信用のラッパーが含まれており、ペイロードのないデフォルトのトランザクションとして、2つのウォレット間でトンコインの定期的な送金を簡単に準備できます。
...
TON コネクト JS SDKを使用した通常のTON転送は、以下のように実行できます：
...
:::info
`assets-sdk`ライブラリは、`ton-connect`の場合でも、メソッドを使用できます。
:::
...
- `validUntil` - メッセージが有効になるまでの UNIX 時間
- `JettonWalletAddress` - JettonMaser と Wallet の契約に基づいて定義された JettonWallet のアドレス
...
- `JettonWalletAddress` - JettonMaser と Wallet の契約に基づいて定義された JettonWallet のアドレス
- `amount` - 整数、ナノトン単位でのガス支払い用のToncoinの量
- `body` - オペコード `burn#595f07bc` の jetton ウォレットのペイロード
...
`NFTitem` を新しい所有者 `NEW_OWNER_WALLET` に譲渡します。
...
コントラクト [nft-fixprice-sale-v3r3](https://github.com/getgems-io/nft-contracts/blob/main/packages/contracts/sources/nft-fixprice-sale-v3r3.fc)に従って、GetGemsマーケットプレイスでの販売のためのメッセージとトランザクションの準備例を示します。
...
<details>
    <summary>NFTセールメッセージ本文の作成アルゴリズム全体を表示します。</summary>
...
Pythonの例は、[PyTonConnect](https://github.com/XaBbl4/pytonconnect)と[pytoniq](https://github.com/yungwine/pytoniq)を使用しています。
...
:::tip
例を読む [ソース](https://github.com/yungwine/ton-connect-examples/blob/master/main.py)。
:::
...
:::tip
[TON Smart Contract Addresses](/v3/documentation/smart-contracts/addresses) の詳細をご覧ください。
:::
...
[nft-fixprice-sale-v3r2](https://github.com/getgems-io/nft-contracts/blob/main/packages/contracts/sources/nft-fixprice-sale-v3r2.fc)のNFT購入プロセスは、ペイロードなしで通常の転送で実行することができます。唯一の重要なことは、正確なTON量であり、以下のように計算されます：
`buyAmount=NftpriceTON+1.0TON`
...
Goの例としては、[tonconnect](https://github.com/cameo-engineering/tonconnect)と[tonutils-go](https://github.com/xssnick/tonutils-go)があります。
...
:::tip
[tonconnect](https://github.com/cameo-engineering/tonconnect/blob/master/examples/basic/main.go) と [tonutils-go](https://github.com/xssnick/tonutils-go?tab=readme-ov-file#how-to-use) の例を読んでください。
:::
