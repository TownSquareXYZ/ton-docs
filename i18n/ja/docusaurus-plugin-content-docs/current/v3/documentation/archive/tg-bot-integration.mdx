...
まず、Node.jsプロジェクトを作成します。ここではTypeScriptと[node-telegram-bot-api](https://www.npmjs.com/package/node-telegram-bot-api)ライブラリを使用しますが、お好みで他のライブラリを選択することもできます。また、QRコードの生成には[qrcode](https://www.npmjs.com/package/qrcode)ライブラリを使用しますが、他の同じライブラリに置き換えることもできます。

`ton-connect-bot` ディレクトリを作成しましょう。そこに以下の package.json ファイルを追加します。
...
内部にディレクトリ`src`と`bot.ts`ファイルを作成します。そこでTelegramBotインスタンスを作成しましょう。
...
`npm run compile` と `npm run start` を実行し、任意のメッセージをボットに送信することができます。 Botは「メッセージを受信しました」と返信します。TonConnect統合の準備が整いました。
...
ウォレットリストの取得から始めます。http-bridge互換のウォレットだけが必要です。src`の中に `ton-connect`フォルダを作成し、そこに`wallets.ts` ファイルを追加します：
また、`getWalletInfo`という関数を定義し、`appName`でウォレットの詳細情報を取得します。
name`と`appName`の違いは、`name`はウォレットの人間が読めるラベルで、`appName`はウォレットのuniq識別子です。
...
次に、TonConnectストレージを定義する必要があります。 TonConnect は `localStorage` を使用してブラウザで実行しているときに接続の詳細を保存しますが、NodeJS 環境には `localStorage` はありません。 そのため、カスタムのシンプルなストレージ実装を追加する必要があります。
...
ウォレット接続を実装しようとしています。
`src/mainを変更します。 s` と `connect` コマンドを追加します。このコマンドハンドラにウォレット接続を実装します。
...
私たちがここでやっていることを分析しましょう。まず、ウォレットリストを取得し、TonConnect インスタンスを作成します。
その後、ウォレットの変更をサブスクライブします。ユーザがウォレットを接続すると、ボットが「$${wallet.device.appName} wallet connected!」というメッセージを送信します。
次に、Tonkeeper のウォレットを見つけ、接続リンクを作成します。最後に、接続リンクを記載した QR コードを生成し、写真としてユーザに送信します。

Bot(`npm run compile`と`npm run start`)を実行し、`/connect` メッセージをBotに送信できます。 BotはQRを返信する必要があります。Tonkeepのウォレットでスキャンしてください。チャットには`Tonkeepのウォレットが接続されました！`というメッセージが表示されます。
...
Tonkeeperのウォレット接続は完了しました。 しかし、私たちはすべてのウォレットのユニバーサルQRコードを介して接続を実装しておらず、ユーザーが適切なウォレットを選択することができませんでした。今すぐカバーしましょう。

より良いUXのために、 `callback_query` と `inline_keyboard` Telegramの機能を使用します。 それに慣れていない場合は、 [here](https://core.telegram.org/bots/api#callbackquery) についてもっと読むことができます。
...
`connector.connect` の呼び出し引数を置き換えたことに注意してください。今度はすべてのウォレットに統一されたリンクを生成します。
...
ここでは、buttons ハンドラリストと `callback_query` パーサを定義します。 残念ながらコールバックデータは常に文字列ですので、 `callback_data` に JSON を渡し、 `callback_query` ハンドラで後で解析する必要があります。
...
ボットをコンパイルして実行します。ウォレットを選択ボタンをクリックすると、ボットはインラインキーボードボタンを置き換えます。

### 他のボタンハンドラを追加

このメニューを完了し、残りのコマンドハンドラを追加しましょう。

まず、ユーティリティ関数 `editQR` を作成します。 メッセージメディア(QRイメージ)を編集するのはちょっと難しいです。 ファイルに画像を保存し、Telegramサーバーに送信する必要があります。その後、このファイルを削除できます。
...
QRコードの有効期限とコネクタの停止はまだ検討されていないことにご注意ください。後で処理します。
...
トランザクションを送信する新しいコードを書く前に、コードをクリーンアップしましょう。 ボットコマンドハンドラ用の新しいファイルを作成します('/connect', '/send_tx', ...)
...
ここでは、ユーザーのウォレットが接続されているかどうかをチェックし、トランザクションの送信を処理します。
...
基本的なコマンドはすべて実行しました。しかし、各コネクタは一時停止されるまで SSE 接続を開いたままにしておくことが重要です。
また、ユーザが `/connect` を複数回呼び出したり、`/connect` や `/send_tx` を呼び出して QR をスキャンしなかったりした場合の処理も行っていません。サーバーのリソースを節約するために、タイムアウトを設定して接続を閉じるべきです。
その後、QR/トランザクション要求の有効期限が切れたことをユーザーに通知します。
...
アクティブユーザーコネクタをキャッシュに保存できます。 キャッシュにはコネクタがありません。新しいコネクタを作成し、タイムアウトを登録し、このコネクタを返します。
...
以上です！ボットをコンパイルして実行し、`/connect` を2回呼び出してみてください。

### @walletとのやり取りを改善

V3 TonConnect から @wallet のような TWA ウォレットへの接続がサポートされています。チュートリアルではボットを @wallet に接続することができます。
ただし、リダイレクト戦略を改善してUXをより良くする必要があります。さらに、最初の画面(「ユニバーサルQR」)に「@walletを接続」ボタンを追加しましょう。

まず、ユーティリティ関数をいくつか作成しましょう。
...
connect-wallet-menuとcommand-handlersでこの関数を使うだけです：
...
それだけです。 現在、ユーザーはメイン画面の特別なボタンを使用して@walletを接続することができます。また、TGリンクに適切なリターン戦略を提供しています。

## 永続的なストレージを追加

現時点では、MapオブジェクトにTonConnectセッションを保存します。 ただし、サーバーを再起動したときにセッションを保存するために、データベースやその他の永続的なストレージに保存したい場合があります。
Redisは使用しますが、永続的なストレージをお選びいただけます。
...
Redisを使用するには、redisサーバーを起動する必要があります。Dockerイメージを使用します：
`docker run -p 6379:6379 -it redis/redis-stack-server:latest` とします。

redis接続パラメータを`.env`に追加します。デフォルトのredisURLは`redis://127.0.0.1:6379`です。
...
動作させるには、 `main.ts` でredisの初期化を待つ必要があります。このファイルのコードを非同期関数にラップしましょう。
