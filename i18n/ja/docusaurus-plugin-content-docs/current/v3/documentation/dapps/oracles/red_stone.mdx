...
RedStone のオラクルは、スマートコントラクトにオラクルデータを提供する代替設計を使用しています。（データプロバイダによって）常にデータをコントラクトのストレージに永続化する代わりに、（エンドユーザーに）必要なときにのみ情報がオンチェーンされます。その瞬間まで、データはRedStoneライトキャッシュゲートウェイとストリームデータブロードキャストプロトコルによって提供される分散型キャッシュレイヤーに残ります。
ストリームデータブロードキャストプロトコルを使用しています。データはエンドユーザーによってコントラクトに転送されます。
を添付する必要があります。情報の完全性は、署名チェックを通じてオンチェーンで検証されます。
...
- RedStoneのOracleデータ [price_manager.fc](https://github.com/redstone-finance/redstone-oracles-monorepo/blob/main/packages/ton-connector/contracts/price_manager.fc) を使用するOracleコントラクトのサンプルです。
  FunCで書かれています。これは[TVM Upgrade 2023.07](/v3/documentation/tvm/changelog/tvm-upgrade-2023-07)が必要です。

#### 初期データ

上述したように、コントラクトに転送されたデータパッケージは署名チェックによって検証されます。
`signer_count_threshold`を達成するためにカウントされるには、渡されたデータに署名する署名者は初期データで渡された`署名者`の一人でなければいけません。また、`signer_count_threshold`が渡される必要があります。
...
ETH'`は16進数で`0x455448`、10進数で`4543560`となり、`256_256_ord('E')+256\*ord('T')+ord('H')\`となります。
...
オンフライ処理に関係なく、チェーン上の`payload`を処理する方法も存在しますが、
は集約された値を契約のストレージに保存/書き込みます。 値は、コントラクトのストレージに保持され、`read_prices` 関数を使用して読み取ることができます。 コントラクトに最後に保存/書き込んだデータのタイムスタンプは、 `read_timestamp` 関数を使用して読み込むことができます。

メソッドは TON 内部メッセージとして呼び出される必要があります。メッセージの引数は次のとおりです:
...
コントラクトに保存されている値をネットワークの外から読み込むことに関係なく、
`feed_id`のコントラクトに保存されている値をオンチェーンで直接フェッチできる可能性があります。
OP_REDSTONE_FETCH_DATA\` という内部メッセージを呼び出す必要があります。メッセージの引数は以下のとおりです：
...
送信側のストレージに保存されます。
最初のペイロードの `ref` (`initial_payload`) は、例えば最初のメッセージの送信者、を含む参照として追加されます。
...
`price` と `price_feed` の初期データと似ています。 TON 契約のアーキテクチャにより、最初のデータは、以下のように構成されている契約のストレージ構造とともに招集されなければなりません。
...
初期化時に設定されます。 渡されたデータパッケージの最小タイムスタンプも返します。
...
呼び出される `price_feed` を渡す必要があります。

#### 初期データ

初期データの `price_feed` と似ています。 TON コントラクトのアーキテクチャにより、最初のデータは、以下のように構成されているコントラクトのストレージ構造とともに招集されなければなりません。
