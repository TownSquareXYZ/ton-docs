...
リストはネストされた 2 要素のタプルとして表現できます。空のリストは TVM の `null` 値として表されます(`null()` を呼び出すことで取得できます)。 例えば、タプル `(1, 2, (3, null))` はリスト `[1, 2, 3]` を表します。
...
グローバル設定パラメータの値を、整数のインデックス`i`を`cell`または`null`値として返します。
...
セル`c`を永続コントラクトデータとして設定します。このプリミティブで永続コントラクトストレージを更新できます。
...
- [乱数生成](/develop/smart-contracts/guidelines/random-number-generation)
...
`0..range-1` の範囲に新しい擬似乱数`z`を生成します。または`range < 0`の場合は-1`range..-1`を生成します。 より正確には、`random`のように無署名の乱数`x`が生成されます。`z := x * range / 2^256`は
計算されます。
...
未署名の256ビット整数`x`をランダムなシード`r`にミックスします。ランダムシードを2つの32バイト文字列の連結のsha256に設定します。古いシード`r`の最初の大きなエンディアン表現です。 そして、`x`の大きなエンディアン表現を持つ2つ目。
...
- `addr_none`は`t = (0)`で表されます。つまり、
  1つの整数がゼロに等しいタプルです。
- `addr_extern` は`t = (1, s)`で表されます。スライス`s`は`external_address`の
  フィールドを持っています。 言い換えれば、`t`はペア（2つのエントリからなるタプル）で、1とスライスに等しい整数を含みます。
- `addr_std`は`t = (2, u, xs)`で表されます。 `u`は`null`（`anycast`が存在しない場合）または`rewrite_pfx`（`anycast`が存在する場合）を含むスライス`s'のどちらかです。 次に、整数`x`は`workchain_id`で、スライス`s\`はアドレスを含んでいます。
- `addr_var`は`t = (3, u, xs)`で表されます。`u`、`x`、`s`は`addr_std`と同じ意味を持ちます。
...
スライスから unsigned `len`-bit 整数をロードします。
...
署名された `len`-bit 整数をスライスからプリロードします。
...
未署名の `len`-bit 整数をスライスからプリロードします。
...
辞書を `load_dict` としてロードしますが、スライスの残りの部分だけを返します。
...
スライス`s`が空であるかどうかをチェックします(つまり、データのビットが含まれておらず、セル参照も含まれていません)。
...
辞書が空かどうかをチェックします。`cell_null?` と同等です。
