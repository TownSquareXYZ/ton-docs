...
何かのイベントが関連しているかどうかを確認したいとしましょう。これを行うには、flag 変数を使用します。 FunC の `true` は `-1` で、`false` は `0` であることを忘れないでください。
...
`refs` にのみ関心がある場合は、 `slice_refs_empty?()` を使用してその存在を確認する必要があります。
...
:::caution 下書き|下書き|下書き|下書き|下書き|下書き|下書き|下書き|下書き|下書き|下書き|下書き|
...
### デプロイメッセージの送信方法 (stateInitのみ、stateInitとbody)
