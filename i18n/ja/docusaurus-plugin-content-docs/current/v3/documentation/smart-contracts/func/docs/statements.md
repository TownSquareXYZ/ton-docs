...
また、ハスケル形式の呼び出しも可能ですが、必ずしも(後で修正する必要があります)
...
FunCは`くりかえし`、`while`、`do { ... } until`ループをサポートしています。`for`ループはサポートされていません。
...
条件`x < 100`の真理値は`int`型であることに注意してください。(cf. [boolean type](/v3/documentation/smart-contracts/func/docs/types#absence-of-boolean-type))。
