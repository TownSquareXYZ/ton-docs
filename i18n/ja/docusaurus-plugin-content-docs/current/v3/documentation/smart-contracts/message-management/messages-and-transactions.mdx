...
メッセージは、アクター (ユーザー、アプリケーション、またはスマート コントラクト) 間で交換されるデータのパケットです。通常、これには、ストレージの更新や新しいメッセージの送信など、実行するアクションを受信者に指示する情報が含まれています。
...
この種の通信を扱うことは、衛星を宇宙に打ち上げることを思い出させます。作成したメッセージはわかっていますが、結果を判断するには打ち上げ後の観察が必要です。

## What is a transaction?

TON 内のトランザクションは以下で構成されています:
...
- 成功、コード0または1を終了する
- 失敗、実行せずに `aborted: true` に失敗しました
- 失敗, [exit code](/v3/documentation/tvm/tvm/exit-codes), `aborted: true`
...
画像の場合、`in_msg_lt < tx0_lt < out_msg_lt`
...
1. 最初の可能な順序は `tx1'_lt < tx2'_lt`です:
...
2. 2 番目の可能な順序は `tx2'_lt < tx1'_lt`です:
...
TONブロックチェーンの非同期構造は、メッセージ配信保証のための課題を作成します。 論理的な時間は、イベントとトランザクションオーダーを確立するのに役立ちますが、シャードチェーン内のさまざまなルートにより、複数のスマートコントラクト間のメッセージ配信注文を保証するものではありません。 これらの複雑さにもかかわらず、TON は内部メッセージの配信を保証し、ネットワークの信頼性を維持します。 開発者は、革新的な分散型アプリケーションの構築におけるTONの可能性を最大限に活用するために、これらのニュアンスに適応する必要があります。
