...
メッセージの構成、解析、送信は、[TL-B スキーマ](/v3/documentation/data-formats/tlb/tl-b-langage)、[トランザクション フェーズ、および TVM](/v3/documentation/tvm/tvm-overview) の交差点にあります。

実際、FunC は、引数としてシリアル化されたメッセージを期待する [send_raw_message](/v3/documentation/smart-contracts/func/docs/stdlib#send_raw_message) 関数を公開しています。

TON は幅広い機能を備えた包括的なシステムであるため、この機能をすべてサポートする必要があるメッセージは非常に複雑に見える場合があります。ただし、その機能のほとんどは一般的なシナリオでは使用されず、ほとんどの場合、メッセージのシリアル化は次のように簡略化できます。
...
したがって、開発者は心配する必要はありません。このドキュメントの内容が最初に読んだだけでは理解できないように思えても、大丈夫です。一般的な考え方だけを把握してください。
...
「Maybe」や「Either」、その他の種類の表現は次のことを意味します。
...
- 次のゼロビットは、「init」フィールドがないことを意味します。
- 最後のゼロビットは、msg_body がインプレースでシリアル化されることを意味します。
...
| :---- | :-------------------------------------------- |
| `0`   | 一般的なメッセージ                                     |
| `64`  | 新しいメッセージに最初に示されている値に加えて、受信メッセージの残りの値をすべて運びます。 |
| `128` | メッセージに表示されている値の代わりに、現在のスマートコントラクトの残高をすべて運びます  |

| フラグ   | 説明                                                                                                                |
| :---- | :---------------------------------------------------------------------------------------------------------------- |
| `+1`  | メッセージ金額とは別に送金手数料を支払う                                                                                              |
