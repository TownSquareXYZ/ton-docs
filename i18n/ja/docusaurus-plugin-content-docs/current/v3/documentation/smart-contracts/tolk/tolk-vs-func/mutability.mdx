...
これは一般化です。関数にはいくつかの変更パラメータがあります。
...
`mutate` と `self` を組み合わせると、変更メソッドが得られます。

<h3 className="cmp-func-tolk-header">
  `mutate self` はドットで呼び出されるメソッドで、オブジェクト
</h3>
...
関数に `mutate` パラメータがある場合、実際には暗黙的にそれらを返し、引数に暗黙的に割り当てられます。
...
ですから、`asm`関数は、戻り値の前に\`self'をスタックの上に置く必要があります。
