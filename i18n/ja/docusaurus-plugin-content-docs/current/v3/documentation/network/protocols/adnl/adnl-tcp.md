...
ハンドシェイクを除く各ADNL TCPパケットは、以下の構造を持っています：

- 4 bytes of packet size in little-endian (N)
- 32バイトのnonce（チェックサム攻撃から保護するためのランダムバイト）
- (N - 64) ペイロードバイト
- ノンスとペイロードから32バイトのSHA256チェックサム
...
Pingパケットは5秒に1回送信するのが最適です。これは、データが送信されていない間も接続を維持するために必要なことで、そうしないとサーバーが接続を終了してしまう可能性があります。
...
- 32バイトのnonce -> ランダムな32バイト
- 4バイトのID TLスキーマ -> **9a2b084d**
- 8 bytes of request-id -> random uint64 number
- ノンスとペイロードから32バイトのSHA256チェックサム

パケットを送り、[tcp.pong](https://github.com/ton-blockchain/ton/blob/ad736c6bc3c06ad54dc6e40d62acbaf5dae41584/tl/generate/scheme/ton_api.tl#L23)を待つと、`random_id`はpingパケットで送ったものと同じになります。
...
- リトルエンディアンで4バイトのパケットサイズ→64+(4+32+(1+4+(1+4+3)+3)) = **116**)
- 32バイトのnonce -> ランダムな32バイト
- 4バイトのID ADNLQueryスキーマ -> **7af98bb4**
- 32 バイト `query_id:int256` -> ランダムな 32 バイト
  - 1バイトの配列サイズ -> **12**
  - 4バイトのID LiteQueryスキーマ -> **df068c79**
    - 1バイトの配列サイズ -> **4**
    - 4バイトのID getMasterchainInfoスキーマ -> **2ee6b589**
    - 3 0バイトのパディング（8へのアライメント）
  - 3 0バイトのパディング（16へのアラインメント）
- ノンスとペイロードから32バイトのチェックサムSHA256
...
これに対して、[liteServer.masterchainInfo](https://github.com/ton-blockchain/ton/blob/ad736c6bc3c06ad54dc6e40d62acbaf5dae41584/tl/generate/scheme/lite_api.tl#L30)を受け取ることを期待します。last:[ton.blockIdExt](https://github.com/ton-blockchain/ton/blob/ad736c6bc3c06ad54dc6e40d62acbaf5dae41584/tl/generate/scheme/tonlib_api.tl#L51) state_root_hash:int256とinit:[tonNode.zeroStateIdExt](https://github.com/ton-blockchain/ton/blob/ad736c6bc3c06ad54dc6e40d62acbaf5dae41584/tl/generate/scheme/ton_api.tl#L359)からなります。

受信パケットは送信パケットと同じ方法でデシリアライズされます。同じアルゴリズ ムを持つが、レスポンスが[ADNLAnswer](https://github.com/ton-blockchain/ton/blob/ad736c6bc3c06ad54dc6e40d62acbaf5dae41584/tl/generate/scheme/lite_api.tl#L23)でラップされることを除けば、逆方向です。

レスポンスをデコードすると、次のようなパケットになります：
...
スマート・コントラクト・メソッドを実行するには、TLスキーマを使ってリクエストを作成し、送信する必要がある：
...
- 例えば、`result:mode.2?bytes`は、インデックス2のビットが1にセットされている場合のみ、レスポンスに表示されます。
- id:tonNode.blockIdExt - our master block state that we got in the previous chapter.
- account:[liteServer.accountId](https://github.com/ton-blockchain/ton/blob/ad736c6bc3c06ad54dc6e40d62acbaf5dae41584/tl/generate/scheme/lite_api.tl#L27) - ワークチェーンとスマートコントラクトのアドレスデータ
- method_id:long - 8バイト。呼び出されたメソッドの代わりにXMODEMテーブルのcrc16が書き込まれ、ビット17がセットされます[[計算]](https://github.com/xssnick/tonutils-go/blob/88f83bc3554ca78453dd1a42e9e9ea82554e3dd2/ton/runmethod.go#L16)
- params:bytes - [BoC](/v3/documentation/data-formats/tlb/cell-boc#bag-of-cells)でシリアライズされた[Stack](https://github.com/ton-blockchain/ton/blob/ad736c6bc3c06ad54dc6e40d62acbaf5dae41584/crypto/block/block.tlb#L783)。メソッドを呼び出すための引数を含みます。[実装例](https://github.com/xssnick/tonutils-go/blob/88f83bc3554ca78453dd1a42e9e9ea82554e3dd2/tlb/stack.go)

例えば、`result:mode.2?bytes`だけが必要だとすると、モードは0b100、つまり4になります：

- mode:# -> 何が送信されたか - 4.
- id:tonNode.blockIdExt -> メソッドが実行されたマスターブロック。
- shardblk:tonNode.blockIdExt -> 契約口座があるシャードブロック
- exit_code:int -> 4バイトで、メソッド実行時の終了コード。全て成功した場合は=0、失敗した場合は例外コードと等しい。
- result:mode.2?bytes -> [BoC](/v3/documentation/data-formats/tlb/cell-boc#bag-of-cells)でシリアライズされた[Stack](https://github.com/ton-blockchain/ton/blob/ad736c6bc3c06ad54dc6e40d62acbaf5dae41584/crypto/block/block.tlb#L783)、メソッドによって返された値を含む。

EQBL2_3lMiyywU17g-or8N7v9hDmPCpttzBPE2isF2GTzpK4`の`a2\`メソッドの呼び出しと結果の取得を分析してみましょう：
...
- mode`= 4, 必要なのは結果のみ ->`04000000\`
- id\` = getMasterchainInfoの実行結果
- account`=ワークチェーン0（4バイト `00000000`）、int256［契約アドレスから取得］（/v3/documentation/data-formats/tlb/tl-b-types#addresses）、つまり32バイト `4bdbfde5322cb2c14d7b83ea2bf0deeff610e63c2a6db7304f1368ac176193ce\` です。
...
- params：bytes` = このメソッドは入力パラメータを受け付けないので、空のスタック (`000000`、セル3バイト - スタックの深さ0)を[BoC](/v3/documentation/data-formats/tlb/cell-boc#bag-of-cells)でシリアライズする -> `b5ee9c72010101000500000600000000`-> バイト単位でシリアライズして`10b5ee9c724101010005000006000000000000\` 0x10 - サイズ、最後に3バイト - パディング。

返答として、私たちは以下を得ます：

- `mode:#`→面白くない
- `id:tonNode.blockIdExt` -> 面白くない
- `shardblk:tonNode.blockIdExt`→興味深いものではない
- `exit_code:int`☞実行に成功した場合は0
- メソッドによって返されたデータを[BoC](/v3/documentation/data-formats/tlb/cell-boc#bag-of-cells)フォーマットで格納した[Stack](https://github.com/ton-blockchain/ton/blob/ad736c6bc3c06ad54dc6e40d62acbaf5dae41584/crypto/block/block.tlb#L783)を解凍します。

Result`の中には `b5ee9c7201010501001b00020800000203010202030400080ccffcc1000000080aabbcc8\` があり、これはデータを含む[BoC](/v3/documentation/data-formats/tlb/cell-boc#bag-of-cells)です。これをデシリアライズすると、セルが得られます：
...
最初のリンク`rest:^(VmStackList n)` - はスタック上の次の値のセルで、私たちの値`tos:VmStackValue`は2番目なので、値を得るには2番目のリンクを読む必要があります。

スタックの2つ目の要素を取得するために、最初のリンクを通過します：
...
同じ作業を繰り返します。最初の8ビット = `03`、つまりまたセルです。スタックの深さは2なので、パスが完了します。合計で、コントラクトが返す値は2つあります。

逆順であることに注意してください。同じように、関数を呼び出すときには引数を渡す必要があります。
...
残高、コード、コントラクトデータなどのアカウントの状態データを取得するには、[getAccountState](https://github.com/ton-blockchain/ton/blob/ad736c6bc3c06ad54dc6e40d62acbaf5dae41584/tl/generate/scheme/lite_api.tl#L68)を使用します。リクエストには、[新鮮なマスターブロック](#getmasterchaininfo)とアカウントアドレスが必要です。応答として、TL構造体[AccountState](https://github.com/ton-blockchain/ton/blob/ad736c6bc3c06ad54dc6e40d62acbaf5dae41584/tl/generate/scheme/lite_api.tl#L38)を受け取ります。

AccountState TLスキーマを分析してみよう：
...
- `state` - [BoC](/v3/documentation/data-formats/tlb/cell-boc#bag-of-cells) TL-B [アカウント状態スキーム](https://github.com/ton-blockchain/ton/blob/ad736c6bc3c06ad54dc6e40d62acbaf5dae41584/crypto/block/block.tlb#L232)

これらのデータのうち、必要なものは州にあるので、それを分析します。

例えば、アカウント`EQAhE3sLxHZpsyZ_HecMuwzvXHKLjYx4kEUehhOy2JmCcHCT`のステータスを取得してみましょう：
...
上のデータからの最初のビット=1なので、`account$1`で作業しており、スキーマを使用します：
...
7-1=6を2進数-110`に変換すると3ビットになるので、長さlen=3ビットとなります。そして値は `(uint (len \* 8))\` となります。つまり、VarUIntegerの値を得るために読み込む必要があるビット数です。

cells:(VarUInteger7)`を読み、ルートセルから次のビットを取り、次の16ビットを見て理解する、これは`00100110101000`です。lenの最初の3ビットを読むと、これは`001`、つまり1です。サイズ（uint (1 * 8)）を求めると、uint 8となり、8ビットを読むと`cells`、`00110101`、つまり10進数で53となります。同じことを `bits`と`public_cells\` にも行います。

`used:StorageUsed`の読み込みに成功し、次に`last_paid:uint32`があるので、32ビットを読み込みます。du_payment:(多分Grams)`では、Maybeは0になるので、Gramsをスキップします。しかし、もしMaybeが1であれば、Gramsの`amount:(VarUInteger 16) = Grams\`スキーマを見ることができ、これを扱う方法をすでに知っていることをすぐに理解できます。前回と同じように、7の代わりに16があるだけです。

次に、スキーマを持つ `storage:AccountStorage` があります：

```tlb
account_storage$_ last_trans_lt:uint64 balance:CurrencyCollection state:AccountState = AccountStorage;
```

`last_trans_lt:uint64`を読み込みます。これは64ビットで、最後の口座取引のltを格納します。最後に、スキーマで表される残高です：
...
データに従って残りのビットを分析してみましょう：

```
100000000111010111110100100011100111011011101001111101100000011000000100110
```

8\*8=64、次の64ビットを読みます= `00000111011101001000111001110111001110110000001100000`、余分なゼロビットを取り除くと `111011101110100100011110111001110110000001100000`となり、これは`531223439883591776`に等しく、ナノからトンに変換すると`531223439.883591776`となります。

主要なケースはすべて分析済みであり、残りは分析したものと同様の方法で得ることができるので、ここで止めておきます。また、TL-Bの解析に関する追加情報は[公式ドキュメント](/v3/documentation/data-formats/tlb/tl-b-language)にあります。
...
キーIDは、直列化されたTLスキーマのSHA256ハッシュです。

最もよく使われるキーのTLスキーマは以下の通りです：
...
### ハンドシェーク・パケット・データの暗号化

ハンドシェーク・パケットはセミオープン形式で送信され、160バイトだけが暗号化され、永久暗号に関する情報が含まれています。

それを暗号化するにはAES-CTR暗号が必要で、それを取得するには160バイトのSHA256ハッシュと[ECDH共有鍵](#getting-a-shared-key-using-ecdh)が必要です。

暗号はこのように作られています：

- キー = (0 - 15 バイトの公開キー) + (16 - 31 バイトのハッシュ)
- iv = (0 - 3 ハッシュバイト) + (20 - 31 公開鍵バイト)

暗号が組み立てられたら、160バイトを暗号化します。
...
共有鍵を計算するには、我々の秘密鍵とサーバーの公開鍵が必要です。

DHの本質は、秘密情報を開示することなく、共有秘密鍵を得ることです。これがどのように行われるのか、最も単純化した形で例を挙げます。我々とサーバーの間で共有鍵を生成する必要があるとすると、そのプロセスは次のようになります：

- 私たちは、**6**や**7**のような秘密番号や公開番号を生成します。
- サーバーは**5**や**15**のような秘密番号と公開番号を生成します。
- サーバーと公開番号を交換し、サーバーに**7**を送ると、サーバーから**15**が送られてきます。
- 我々は計算する：**7^6 mod 15 = 4**
- サーバーは計算する：**7^5 mod 15 = 7**
- 受信した数字を交換し、サーバーに**4**を与えると、サーバーは**7**を返してきます。
- 計算すると、**7^6 mod 15 = 4** となります。
- サーバーは計算する：**4^5 mod 15 = 4**
- 共有キー = **4**

ECDH自体の詳細は、簡単のため省略します。秘密鍵と公開鍵の2つの鍵を使って、曲線上の共通点を見つけることで計算されます。興味のある方は別途お読みください。
