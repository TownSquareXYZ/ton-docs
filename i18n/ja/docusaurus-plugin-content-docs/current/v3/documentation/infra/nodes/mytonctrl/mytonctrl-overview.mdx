...
インストールプロセスについてのフィードバックを積極的に求めています。ご質問やご提案がございましたら、[お問い合わせ](https://t.me/Alexgton) ください。
...
| フォーマット名               | 書式                                                                         | 例                                                                     | Description                                                                     |
| :-------------------- | :------------------------------------------------------------------------- | :-------------------------------------------------------------------- | ------------------------------------------------------------------------------- |
| 引数なし                  | `update`                                                                   | `update`                                                              | Update from current repo                                                        |
...
| フォーマット名               | 書式                                                                          | 例                                                                   | Description                                                                           |
| :-------------------- | :-------------------------------------------------------------------------- | :------------------------------------------------------------------ | ------------------------------------------------------------------------------------- |
| 引数なし                  | `upgrade`                                                                   | `upgrade`                                                           | Upgrade from current repo                                                             |
...
| フォーマット名   | 書式            | 例             | Description                                                                                                                     |
| --------- | ------------- | ------------- | ------------------------------------------------------------------------------------------------------------------------------- |
| 引数なし      | `status`      | `status`      | Full status report including validator efficiency and online validators.                                                        |
...
2. [mnemonic2key](https://github.com/ton-blockchain/mnemonic2key)を複製しインストールします:
   ...
   npm install
   ```
3. `word1`、`word2`...をあなたのニーモニックフレーズと`address`に置き換えて、次のコマンドを実行します。
   ```
...
8. `mg`コマンドを使って資金を送ることができます。ヘルプのドキュメントを見るには`mg`を入力してください。
   コマンド実行時にプレースホルダ(`< >`内の単語)を実際の値に置き換えてください。
...
| フォーマット名                       | 書式                                  | Description                                                                  |
| ----------------------------- | ----------------------------------- | ---------------------------------------------------------------------------- |
| 引数なし                          | `el`                                | Prints the table with ADNL, Pubkey, and Wallet reduced.                      |
| 以下の引数の任意の組み合わせ                | `el --json adnl past`               | Full ADNL, Pubkey, Wallet, and past election entries in JSON representation. |
...
| フォーマット名                       | Format                              | Description                                                                   |
| ----------------------------- | ----------------------------------- | ----------------------------------------------------------------------------- |
| 引数なし                          | `vl`                                | Prints the table with ADNL, Pubkey, and Wallet reduced.                       |
| 以下の引数の任意の組み合わせ                | `vl --json adnl pubkey wallet past` | Full ADNL, Pubkey, Wallet, and past validator entries in JSON representation. |
...
| `archive-ttl`      | _アーカイブブロックを保存しないようにノードを強制するには、 `86400`_ の値を使用します。                                      | 2592000リットルサーバーモードがインストール中に有効になっている場合、それ以外の場合は86400が有効になっています                       |
