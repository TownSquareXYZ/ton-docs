...
このドキュメントでは、TON ノードを実行する際に利用できる様々なフラグとオプションについて説明します。 各フラグには、短い名前、長い名前、デフォルト値(該当する場合)とその機能の説明があります。
...
| `-s`, `--state-ttl`     | Sets the TTL (time-to-live) for the state in seconds                                                    | `86400` 秒 (1 日)                | `-s <seconds>`              |
| `-m`, `--mempool-num`   | Specifies the maximum number of external messages in the mempool                                        | Unlimited                      | `-m <number>`               |
| `-b`, `--block-ttl`     | Sets the TTL for blocks in seconds.                                                                     | `86400` 秒 (1 日)                | `-b <seconds>`              |
| `-A`, `--archive-ttl`   | Sets the TTL for archived blocks in seconds                                                             | `604800` 秒 (7 日)               | `-A <seconds>`              |
| `-K`, `--key-proof-ttl` | Sets the TTL for key blocks in seconds.                                                                 | `315360000`秒（10年）              | `-K <seconds>`              |
| `-S`, `--sync-before`   | During initial sync, download all blocks for the last given number of seconds                           | `3600`秒（1時間）                   | `-S <seconds>`              |
