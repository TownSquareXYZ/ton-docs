...
| 構成        | Type                            | 必須   | Description                                                                                                                     |   |
| --------- | ------------------------------- | ---- | ------------------------------------------------------------------------------------------------------------------------------- | - |
| メッセージ$_   | Constructor                     |      | Defined according to the constructor rule. The empty tag `$_` indicates that no bits are added initially.                       |   |
| 情報        | [CommonMsgInfo](#commonmsginfo) | 必須   | Contains detailed message properties such as destination and value. It is always stored in the message's root cell.             |   |
| init      | [StateInit](#stateinit-tl-b)    | 省略可能 | The general structure is used in TON to initialize new contracts. It can be placed in either a cell reference or the root cell. |   |
| body      | X                               | 必須   | Message payload. It can be placed in either a cell reference or the root cell.                                                  |   |
...
| fwd_fees        | [VarUInteger 16](#varuinteger-n)          | Required | Validators set the fee for forwarding the message.                                                                              |
| created_lt      | uint64                                    | Required | Logic time of the message, used by validators to order actions in smart contracts.                                              |
| create_at       | uint32                                    | 必須       | UNIX timestamp indicating when the message was created.                                                                         |
...
| ihr_disabled       | ブール型                                 | Required | Flag indicating whether hyper routing is disabled. Currently, it is always set to `true`.                                       |
...
| import_fee         | [VarUInteger 16](#varuinteger-n)     | 必須       | The fee for executing and delivering the message.                                                                               |
...
| create_at           | uint32                               | 必須       | UNIX timestamp representing when the message was created.                                                                       |
...
| library     | HashmapE 256 SimpleLib\* | 省略可能     | Currently, `StateInit` is used without libraries.                                                                                                                                                                                                                                                                                                                        |
...
| 構成           | タイプ         | Required | Description                                                                                                                                   |
| ------------ | ----------- | -------- | --------------------------------------------------------------------------------------------------------------------------------------------- |
| addr_none$00 | Constructor | 必須       | The `$00` tag in serialization indicates that `MsgAddressExt` starts with `00` bits. This signifies that the entire external address is `00`. |
...
| external_address | (bits len)  | 必須       | A bitstring address, where the length of the address equals the previously defined `len`.                              |
...
| 構成           | タイプ          | Required | Description                                                                                                                                                                                                                                                |
| ------------ | ------------ | -------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| addr_std$10  | Constructor  | 必須       | The `$10` tag in serialization indicates that `MsgAddressInt` starts with `10` bits and describes an internal address.                                                                                                                                     |
| anycast      | Anycast\*    | 省略可能     | Represents additional address data. Currently not used in ordinary internal messages.                                                                                                                                                                      |
...
| address      | (bits256)    | 必須       | The smart contract account ID number.                                                                                                                                                                                                                      |
...
| 構成           | Type         | Required  | 説明                                                                                                                                                  |
| ------------ | ------------ | --------- | --------------------------------------------------------------------------------------------------------------------------------------------------- |
| addr_var$11  | Constructor  | 必須        | The `$11` tag indicates that in the serialization of `MsgAddressInt`, the address starts with `11` bits, representing an internal contract address. |
| anycast      | Anycast\*    | 省略可能      | Additional address data is not currently used in regular internal messages.                                                                         |
...
| address      | (bits256)    | 必須        | The payload address (this could be the account ID).                                                                                                 |
...
| grams        | (VarUInteger 16)        | 必須       | Represents the message value, expressed in nanoTons.                                                                                |
| その他          | ExtraCurrencyCollection | 省略可能     | ExtraCurrencyCollection is a dictionary intended for additional, typically empty currencies.                                        |
...
| len        | uintN             | 必須       | Specifies the bit-length used to encode the next value.                                                                     |
| value      | (uint (len \* 8)) | 省略可能     | An optional unsigned integer encoded in `(len * 8)` bits.                                                                   |
