...
| `some` または `some#`            | Serialize `crc32(equation) \| 0x80000000`.         |
| `some#_` または `some$_` または `_` | Serialize nothing.                                 |
...
- `UInt` - 256ビット
- `Bits` - 1023 bit
- `uint1` - `uint256` - 1 - 256ビット
- `int1` - `int257` - 1 - 257ビット
- `bits1` - `bits1023` - 1 - 1023ビット
...
許可される列車: `E` | `E = E` | `E <= E` | `E >E` | `E >= E` | `E > E` | `E + E` | `E * E` | `E * E` | `E ? E`
...
- [TLの古いバージョンの説明](https://core.telegram.org/mtproto/TL)
...
- [tonpy py codegen](https://github.com/disintar/ton/blob/master/cryp/tl/tlbc-gen-py.cpp)
