...
4. *MyTonCtrl* использует кошелек *Валидатор*, чтобы дать указание *Single Nominator* восстановить ставку в следующем избирательном цикле.
...
- Для подписи новых блоков узлу-валидатору требуется "горячий" кошелек. Данный кошелек небезопасен по своей сути, поскольку его приватный ключ подключен к Интернету. Даже если этот ключ будет скомпрометирован, *Валидатор* не сможет извлечь средства, использованные для валидации. Только *Владелец* может вывести эти средства.

- Даже если кошелек *Валидатор* скомпрометирован, *Владелец* может сказать *Single Nominator* изменить адрес валидатора. Это не позволит злоумышленнику в дальнейшем взаимодействовать с *Single Nominator*. Здесь нет состояния гонки, *Владелец* всегда будет иметь приоритет.

- На балансе *Single Nominator* хранятся только основные средства для стейкинга – этот баланс не используется для оплаты комиссии газа. Деньги на газ для начала избирательного цикла хранятся в кошельке *Валидатор*. Это решение не позволит злоумышленнику, скомпрометировавшему валидатор, слить основную сумму с помощью атаки на трату газа.
...
- В случае крайней необходимости, *Владелец* может установить код *Single Nominator* и отменить его текущую логику для устранения непредвиденных обстоятельств.
...
Это простейшая настройка, при которой MyTonCtrl подключен к тому же [стандартному кошельку](https://github.com/ton-blockchain/ton/blob/master/crypto/smartcont/wallet3-code.fc), на котором хранятся средства. Поскольку этот кошелек подключен к интернету, он считается горячим кошельком.

![image](/img/nominator-pool/hot-wallet.png)

Это небезопасно, так как злоумышленник может получить приватный ключ, поскольку кошелек подключен к интернету. С помощью приватного ключа злоумышленник может отправить средства в стейкинг кому угодно.

---

### 2. Кошелек с ограничениями

Эта настройка заменяет стандартный кошелек на [restricted-wallet](https://github.com/EmelyanenkoK/nomination-contract/blob/master/restricted-wallet/wallet.fc), который позволяет отправлять исходящие транзакции только в ограниченные места, такие как *Избиратель* и адрес *Владельца*.

![image](/img/nominator-pool/restricted-wallet.png)

Кошелек с ограничениями не обслуживается (заменен на Пул номинаторов) и имеет нерешенные векторы атак, например, атаки на трату газа. Поскольку в одном и том же кошельке на одном балансе хранятся и плата за газ и основная сумма ставки, злоумышленник, скомпрометировавший приватный ключ, может генерировать транзакции, что повлечет за собой значительные убытки. Кроме того, при попытке вывода средств злоумышленник и владелец могут столкнуться с состоянием гонки из-за коллизий seqno.
...
Смарт-контракт пул номинаторов чрезмерно сложен из-за поддержки 40 одновременно работающих номинаторов. Также, контракт должен защищать номинаторов от деплоера контракта, поскольку это отдельные сущности. Такая схема считается нормальной, но ее очень сложно полностью проверить из-за размера поверхности атаки. Это решение имеет смысл в основном тогда, когда валидатор не имеет достаточно средств для стейкинга для самостоятельной валидации или хочет разделить прибыль с третьими сторонами.
...
Если у вас есть один single nominator, который располагает всеми ставками для валидации, это самая надежная схема, которую можно использовать. Помимо простоты, этот контракт предоставляет владельцу множество аварийных защит, которые могут восстановить ставку в таких ситуациях, как обновление *Избирателя*, которое ломает интерфейс стейка.
...
Используется для вывода средств на кошелек владельца. Чтобы вывести средства, владелец должен отправить сообщение, содержащее: opcode=0x1000 (32 бита), query_id (64 бита) и сумму вывода (хранится как переменная coin). Смарт-контракт номинатора отправит средства с флагом BOUNCEABLE и mode=64. <br/><br/>
...
- withdraw-amount – сумма для снятия. Смарт-контракт номинатора оставит в контракте 1 ТОН, поэтому фактическая сумма, которая будет отправлена на адрес владельца, будет минимальной между запрошенной суммой и балансом контракта - 1. <br/>
...
- new-validator-address (по-умолчанию адрес ZERO) – это адрес нового валидатора. Если вы хотите сразу же отключить валидатор и только позже установить нового, возможно, будет удобно установить в качестве адреса валидатора адрес ZERO.
  Владелец должен запускать deeplink с телефона с кошельком tonkeeper. <br/>
...
- создайте и подпишите tx: `fift -s wallet-v3.fif my-wallet single_nominator_address sub_wallet_id seqno amount -B change-validator.boc`, где my-wallet – это pk-файл владельца (без расширения). Суммы 1 TON должно быть достаточно, чтобы оплатить комиссию (оставшаяся сумма будет возвращена владельцу). Change-validator.boc - это boc, сгенерированный выше.
- с компьютера с доступом в Интернет выполните следующие действия: `lite-client -C global.config.json -c 'sendfile wallet-query.boc'`, чтобы отправить boc-файл (wallet-query.boc), созданный на предыдущем шаге.
...
Вы можете использовать этот опкод, когда адрес смарт-контракта избирателя был неожиданно изменен, а средства все еще заблокированы в избирателе. В этом случае RECOVER_STAKE от валидатора не сработает, и владельцу придется создавать определенное сообщение. <br/>
