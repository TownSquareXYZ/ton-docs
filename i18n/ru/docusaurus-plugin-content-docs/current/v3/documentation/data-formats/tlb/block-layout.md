...
| `флаги`                         | (## 8)                    | Contains additional flags related to the block.                                                                                                    |
...
| ---------------------- | ----------------------------------------------------------------------------------- | ------------ | ---------------------------------------------------------------------------------------------------------------------------------------- |
| `global_id`            | int32                                                                               | Да           | An ID of the network where this shard resides. `-239` for Mainnet, `-3` for Testnet.                     |
| `shard_id`             | ShardIdent                                                                          | Да           | The unique identifier of the shard.                                                                                      |
| `seq_no`               | uint32                                                                              | Да           | The latest sequence number of this ShardChain.                                                                           |
| `vert_seq_no`          | #                                                                                   | Да           | The latest vertical sequence number of this ShardChain.                                                                  |
| `gen_utime`            | uint32                                                                              | Да           | Время генерации, связанное с созданием шарда.                                                                            |
| `gen_lt`               | uint64                                                                              | Да           | The logical time at which the shard was generated.                                                                       |
| `min_ref_mc_seqno`     | uint32                                                                              | Да           | The sequence number of the latest referenced MasterChain block.                                                          |
| `out_msg_queue_info`   | OutMsgQueueInfo                                                                     | Да           | Metadata about the shard’s outbound message queue. Stored in a reference.                                |
...
| `libraries`            | HashmapE 256 LibDescr                                                               | Да           | A hashmap of libraries used in the shard. This is usually empty, except in the MasterChain.              |
| `master_ref`           | BlkMasterInfo                                                                       | Нет          | A reference to the masterchain block info.                                                                               |
| `custom`               | McStateExtra                                                                        | Нет          | MasterChain-specific extra data. Present only in the MasterChain. Stored in a reference. |
...
| Поле             | Тип                           | Обязательное | Описание                                                                                                                                                                                |
| ---------------- | ----------------------------- | ------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `in_msg_descr`   | InMsgDescr                    | Да           | The descriptor for the incoming messages in the block. Stored in a reference.                                                                           |
| `out_msg_descr`  | OutMsgDescr                   | Да           | The descriptor for the outgoing messages in the block. Stored in a reference.                                                                           |
| `account_blocks` | ShardAccountBlocks            | Да           | The collection of all transactions processed in the block, along with updates to the states of accounts assigned to the shard and stored in a reference.                |
| `rand_seed`      | bits256                       | Да           | Случайное начальное число для блока.                                                                                                                                    |
| `created_by`     | bits256                       | Да           | The entity, usually a validator's public key, that created the block.                                                                                                   |
| `custom`         | [McBlockExtra](#mcblockextra) | Нет          | It contains masterchain-specific data, such as custom extra data for the block. Present only in the MasterChain. Stored in a reference. |
...
| `shard_hashes`        | ShardHashes                     | Да           | The hashes of the latest blocks for the corresponding ShardChains.                                                     |
| `shard_fees`          | ShardFees                       | Да           | The total fees are collected from all shards in this block.                                                            |
| `prev_blk_signatures` | HashmapE 16 CryptoSignaturePair | Да           | Signatures of the previous block.                                                                                      |
...
| `config`              | ConfigParams                    | Нет          | The actual configuration parameters for this block. Present only if `key_block` is set.                |
