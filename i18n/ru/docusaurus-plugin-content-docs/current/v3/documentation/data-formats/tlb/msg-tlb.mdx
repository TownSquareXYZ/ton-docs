...
| Структура      | Type                            | Обязательно   | Description                                                                                                                     |   |
| -------------- | ------------------------------- | ------------- | ------------------------------------------------------------------------------------------------------------------------------- | - |
...
| info           | [CommonMsgInfo](#commonmsginfo) | Обязательно   | Contains detailed message properties such as destination and value. It is always stored in the message's root cell.             |   |
| init           | [StateInit](#stateinit-tl-b)    | Необязательно | The general structure is used in TON to initialize new contracts. It can be placed in either a cell reference or the root cell. |   |
| body           | X                               | Обязательно   | Message payload. It can be placed in either a cell reference or the root cell.                                                  |   |
...
| Структура                 | Type                                      | Required    | Description                                                                                                                     |
| ------------------------- | ----------------------------------------- | ----------- | ------------------------------------------------------------------------------------------------------------------------------- |
...
| created_at                | uint32                                    | Обязательно | UNIX timestamp indicating when the message was created.                                                                         |
...
| Структура          | Тип                                                                  | Required    | Description                                                                                                                     |
| ------------------ | -------------------------------------------------------------------- | ----------- | ------------------------------------------------------------------------------------------------------------------------------- |
...
| import_fee         | [VarUInteger 16](#varuinteger-n)                                     | Обязательно | The fee for executing and delivering the message.                                                                               |
...
| created_at                          | uint32                               | Обязательно | UNIX timestamp representing when the message was created.                                                                       |
...
| Структура   | Тип                                       | Обязательно   | Description                                                                                                                                                                                                                                                                                                                                                              |
| ----------- | ----------------------------------------- | ------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
...
| library     | HashmapE 256 SimpleLib\*                  | Необязательно | Currently, `StateInit` is used without libraries.                                                                                                                                                                                                                                                                                                                        |
...
| Структура           | Тип               | Required    | Description                                                                                                                                   |
| ------------------- | ----------------- | ----------- | --------------------------------------------------------------------------------------------------------------------------------------------- |
| addr_none$00        | Constructor       | Обязательно | The `$00` tag in serialization indicates that `MsgAddressExt` starts with `00` bits. This signifies that the entire external address is `00`. |
...
| external_address              | (bits len)         | Обязательно | A bitstring address, where the length of the address equals the previously defined `len`.                              |
...
| Структура    | Тип                | Required      | Description                                                                                                                                                                                                                                                |
| ------------ | ------------------ | ------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| addr_std$10  | Constructor        | Обязательно   | The `$10` tag in serialization indicates that `MsgAddressInt` starts with `10` bits and describes an internal address.                                                                                                                                     |
| anycast      | Anycast\*          | Необязательно | Represents additional address data. Currently not used in ordinary internal messages.                                                                                                                                                                      |
...
| address      | (bits256)          | Обязательно   | The smart contract account ID number.                                                                                                                                                                                                                      |
...
| Структура    | Type              | Required       | Описание                                                                                                                                                                                                                             |
| ------------ | ----------------- | -------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| addr_var$11  | Constructor       | Обязательно    | The `$11` tag indicates that in the serialization of `MsgAddressInt`, the address starts with `11` bits, representing an internal contract address.                                                                                  |
| anycast      | Anycast\*         | Необязательно  | Additional address data is not currently used in regular internal messages.                                                                                                                                                          |
...
| address      | (bits256)         | Обязательно    | The payload address (this could be the account ID).                                                                                                                                                                                  |
...
| Structure    | Тип                                       | Required      | Описание                                                                                                                                                                                                   |
| ------------ | ----------------------------------------- | ------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
...
| grams        | (VarUInteger 16)                          | Обязательно   | Represents the message value, expressed in nanoTons.                                                                                                                                                       |
| other        | ExtraCurrencyCollection                   | Необязательно | ExtraCurrencyCollection is a dictionary intended for additional, typically empty currencies.                                                                                                               |
...
| len               | uintN                       | Обязательно   | Specifies the bit-length used to encode the next value.                                                                     |
| value             | (uint (len \* 8))           | Необязательно | An optional unsigned integer encoded in `(len * 8)` bits.                                                                   |
