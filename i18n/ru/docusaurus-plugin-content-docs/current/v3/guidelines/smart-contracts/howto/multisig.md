...
Напомним, что (n, k)-контракт с мультиподписью - это мультиподписной кошелек с n держателями закрытых ключей, который принимает запросы на отправку сообщений, если запрос (он же заявка) собирает не менее k подписей держателей.
...
Далее вам нужно выбрать кошелек, с которого вы будете отправлять монеты:
...
:::info
Если время, прошедшее до отправки запроса, равно `$AWAIT_TIME$`, то срок действия запроса истекает. Как обычно, время ожидания составляет пару часов (7200 секунд).
:::
