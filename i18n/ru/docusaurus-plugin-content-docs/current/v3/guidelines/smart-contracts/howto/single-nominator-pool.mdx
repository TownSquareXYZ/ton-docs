...
:::caution
Перед началом работы убедитесь, что вы пополнили и [активировали](/v3/guidelines/nodes/running-nodes/validator-node#activate-the-wallets) кошелек валидатора.
:::
...
2. Проверьте, включен ли режим single-nominator:
...
Если вы уже создали пул, то его можно импортировать:
...
Теперь вы можете работать с этим пулом через mytonctrl, как с обычным пулом номинаторов.

:::info
Если баланс пула достаточен для участия в обоих раундах (`баланс > min_stake_amount * 2`), то MyTonCtrl автоматически примет участие в обоих раундах, используя `stake = баланс / 2`, если только пользователь не сделает ставку вручную, используя команду `set stake`. Такое поведение отличается от использования пула номинаторов, но похоже на ставку с помощью кошелька валидатора.
:::
