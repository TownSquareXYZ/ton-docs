...
Строка кода выше проверяет `seqno`, пришедшее в сообщении, и сверяет его с `seqno`, которое хранится в смарт-контракте. Если они не совпадают, то контракт возвращает ошибку с кодом завершения `33`. Таким образом, если отправитель передал недействительное seqno, это означает, что он допустил какую-то ошибку в последовательности сообщений, и смарт-контракт защищает его от таких случаев.
