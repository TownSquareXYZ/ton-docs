---
description: В этом уроке вы узнаете, как полноценно работать с кошельками, сообщениями и смарт-контрактами.
---
...
Прежде чем приступать к разработке смарт-контрактов, крайне важно изучить как работают кошельки и транзакции на TON. Эти знания помогут разработчикам понять принцип взаимодействия между кошельками, сообщениями и смарт-контрактами для выполнения конкретных задач разработки.

:::tip
Перед чтением этого руководства рекомендуется ознакомиться со статьей [Типы контрактов кошелька](/v3/documentation/smart-contracts/contracts-specs/wallet-contracts).
:::

В этом разделе мы научимся создавать операции без использования предварительно настроенных функций, для лучшего понимания процесса разработки. Дополнительные ссылки на материалы для изучения, находятся в конце раздела.

## 💡 Перед началом работы

Этот урок требует базовых знаний JavaScript и TypeScript или Golang. Также необходимо иметь как минимум 3 TON (которые можно хранить на биржевом счете, в некастодиальном кошельке или в боте Кошелек от Telegram). Для понимания также необходимо иметь базовое представление о [ячейке](/v3/concepts/dive-into-ton/ton-blockchain/cells-as-data-storage), [адресах в TON](/v3/documentation/smart-contracts/addresses), [блокчейне блокчейнов](/v3/concepts/dive-into-ton/ton-blockchain/blockchain-of-blockchains).

:::info РАЗВИТИЕ MAINNET НЕОБХОДИМО\
Работа с TON Testnet часто приводит к ошибкам при развертывании, сложностям с отслеживанием транзакций и нестабильной работе сети. Поэтому было бы полезно завершить большую часть разработки в TON Mainnet, чтобы избежать проблем, которые могут возникнуть при попытках уменьшить количество транзакций, и понизить сборы, соответственно.
:::

## 💿 Исходный код

Все примеры кода, используемые в этом руководстве, можно найти в [репозитории GitHub](https://github.com/aSpite/wallet-tutorial).

## ✍️ Что нужно для начала работы

- Убедитесь, что NodeJS установлен.
- Требуются специальные библиотеки Ton: @ton/ton 13.5.1+, @ton/core 0.49.2+ и @ton/crypto 3.2.0+.

**ОПЦИОНАЛЬНО**: Если вы предпочитаете использовать Go вместо JS, то для разработки на TON необходимо установить библиотеку [tonutils-go](https://github.com/xssnick/tonutils-go) и GoLand IDE. Эта библиотека будет использоваться в данном руководстве для версии Go.
...
Для того чтобы создать проект TypeScript, выполните следующие шаги:

1. Создайте пустую папку (которую мы назовем WalletsTutorial).
2. Откройте папку проекта с помощью CLI.
3. Используйте следующие команды для настройки вашего проекта:
...
:::info
Чтобы помочь нам выполнить следующий процесс используется `ts-node` для непосредственного выполнения кода TypeScript без предварительной компиляции. `nodemon` используется для автоматического перезапуска приложения node при обнаружении изменений файлов в директории.
:::
...
5. Затем создайте конфигурацию `nodemon.json` в корне вашего проекта со следующим содержанием:
...
8. Далее необходимо добавить следующий код:
...
9. Запустите код с помощью терминала:
...
:::tip Blueprint
Сообщество TON создало отличный инструмент для автоматизации всех процессов разработки (развертывание, написание контрактов, тестирование) под названием [Blueprint](https://github.com/ton-org/blueprint). Однако нам не понадобится такой мощный инструмент, поэтому мы предлагаем следовать приведенным выше инструкциям.
:::

\*\*ОПЦИОНАЛЬНО: \*\* При использовании Golang следуйте следующим инструкциям:

1. Установите GoLand IDE.
2. Создайте папку проекта и файл `go.mod` со следующим содержимым (для выполнения этого процесса может потребоваться изменить **версию Go**, если текущая используемая версия устарела):
...
4. Создайте файл `main.go` в корне вашего проекта со следующим содержанием:
...
Кроме того, в каждом новом разделе будут указаны только те импорты, которые необходимы для конкретного раздела кода, а новые импорты нужно будет добавлять и объединять со старыми.\
:::

## 🚀 Давайте начнем!

В этом руководстве мы узнаем, какие кошельки (версии 3 и 4) чаще всего используются на блокчейне TON, и познакомимся с тем, как работают их смарт-контракты. Это позволит разработчикам лучше изучить различные типы сообщений на платформе TON, упростить их создание и отправку в блокчейн, разворачивать кошельки и, в конечном итоге, работать с highload-кошельками.

Наша основная задача – создать сообщения, используя различные объекты и функции: @ton/ton, @ton/core, @ton/crypto (ExternalMessage, InternalMessage, Signing и т.д.), чтобы понять, как выглядят сообщения в больших масштабах. Для осуществления этого процесса мы будем использовать две основные версии кошелька (V3 и V4), поскольку биржи, кошельки, не связанные с хранением, и большинство пользователей используют только эти версии.
...
**ВАЖНО:** В данном руководстве используется [код кошелька V3](https://github.com/ton-blockchain/ton/blob/master/crypto/smartcont/wallet3-code.fc) для лучшего понимания процесса разработки. Следует отметить, что версия 3 имеет две подверсии: r1 и r2. В настоящее время используется только вторая подверсия, поэтому, когда мы ссылаемся на V3 в этом документе, это означает v3r2.
:::

## 💎 Кошельки TON Blockchain

Все кошельки, которые функционируют и запускаются на блокчейне TON, на самом деле являются смарт-контрактами, точно так же, как и все, что работает на TON, является смарт-контрактом. Как и в большинстве блокчейнов, в сети можно разворачивать смарт-контракты и настраивать их для различных целей. Благодаря этой функции возможна **полная кастомизация кошелька**. В кошельке TON смарт-контракты помогают платформе взаимодействовать с другими типами смарт-контрактов. Однако важно учитывать, как происходит взаимодействие кошельков.

### Взаимодействие с кошельком

Обычно в блокчейне TON существует два типа сообщений: `internal` (внутренние) и `external` (внешние). Внешние сообщения позволяют отправлять сообщения в блокчейн из внешнего мира, тем самым обеспечивая связь со смарт-контрактами, которые принимают такие сообщения. Функция, отвечающая за выполнение этого процесса, выглядит следующим образом:
...
Прежде чем мы перейдем к более подробной информации о кошельках, давайте рассмотрим, как кошельки принимают внешние сообщения. На TON все кошельки хранят `public key` (публичный ключ), `seqno` (порядковый номер) и `subwallet_id` (идентификатор subwallet) владельца. При получении внешнего сообщения кошелек использует метод `get_data()` для получения данных из хранилища кошелька. Затем он проводит несколько процедур верификации и определяет, принимать сообщение или нет. Этот процесс происходит следующим образом:
...
>
> [`get_data()` в docs](/v3/documentation/smart-contracts/func/docs/stdlib/#load_bits)
>
> [`begin_parse()` в docs](/v3/documentation/smart-contracts/func/docs/stdlib/#load_bits)
>
...
>
> [`load_uint()` в docs](/v3/documentation/smart-contracts/func/docs/stdlib/#load_int)
>
...
>
> [`accept_message()` в docs](/v3/documentation/smart-contracts/transaction-fees/accept-message-effects)

Теперь давайте посмотрим поближе.

### Защита от повторения – Seqno

Защита от повторения сообщений в смарт-контракте кошелька напрямую связана с сообщением seqno (Sequence Number – порядковый номер), который отслеживает, какие сообщения отправляются и в каком порядке. Очень важно не повторять ни одно сообщение из кошелька, поскольку это полностью нарушает целостность системы. Если мы изучим код смарт-контракта кошелька, то `seqno` обычно обрабатывается следующим образом:

```func
throw_unless(33, msg_seqno == stored_seqno);
```

Эта строка кода выше проверяет `seqno`, пришедшее в сообщении, и сверяет его с `seqno`, которое хранится в смарт-контракте. Если seqno не совпадают, то контракт возвращает ошибку с кодом завершения `33`. Таким образом, если отправитель передал недействительное seqno, это означает, что он допустил какую-то ошибку в последовательности сообщений, и контракт защищает его от таких случаев.

:::note
Также важно учитывать, что внешние сообщения могут быть отправлены кем угодно. Это означает что, если вы отправите кому-то 1 TON, кто-то другой сможет повторить это сообщение. Однако, когда seqno увеличивается, предыдущее внешнее сообщение становится недействительным, и никто не сможет его повторить, что предотвращает возможность кражи ваших средств.
:::

### Подпись

Как уже упоминалось ранее, смарт-контракты кошелька принимают внешние сообщения. Однако, так как эти сообщения приходят из внешнего мира, то этим данным нельзя доверять на 100%. Поэтому в каждом кошельке хранится публичный ключ владельца. При получении внешнего сообщения смарт-контракт использует публичный ключ для проверки легитимности подписи сообщения, которое владелец подписал с помощью приватного ключа. Таким образом проверяется, что сообщение отправлено именно от владельца контракта.

Для выполнения этого процесса кошелек должен сначала получить подпись из входящего сообщения, которое загружает публичный ключ из хранилища и проверить подпись с помощью следующих процедур:
...
И если все процедуры верификации завершены корректно, смарт-контракт принимает сообщение и обрабатывает его:
...
:::info accept_message()
Поскольку сообщение приходит из внешнего мира, оно не содержит toncoin, необходимых для оплаты комиссии за транзакцию. При отправке TON с помощью функции `accept_message()` применяется `gas_credit` (на момент написания статьи его значение составляет 10 000 единиц газа), который позволяет производить необходимые расчеты бесплатно, если газ не превышает значение `gas_credit`. После использования функции `accept_message()` весь потраченный газ (в TON) берется с баланса смарт-контракта. Подробнее об этом процессе можно прочитать [здесь](/v3/documentation/smart-contracts/transaction-fees/accept-message-effects).
:::

### Срок действия транзакции

Еще одним шагом, используемым для проверки действительности внешних сообщений, является поле `valid_until`. Как видно из наименования переменной, это время в UNIX до котрого сообщение будет действительным. Если процесс проверки завершился неудачей, контракт завершает обработку транзакции и возвращает код завершения `35`:
...
Этот алгоритм работает как защита от восприимчивости к различным ошибкам, например, когда сообщение уже недействительно, но все равно было отправлено в блокчейн по неизвестной причине.

### Различия V3 и V4 кошелька

Единственное различие между V3 и V4 кошелька заключается в том, что кошелек V4 использует плагины, которые можно устанавливать и удалять. Эти плагины представляют собой специальные смарт-контракты, которые могут запрашивать в определенное время определенное количество TON у смарт-контракта кошелька.

Смарт-контракты кошелька, в свою очередь, отправят в ответ необходимое количество TON без необходимости участия владельца. Это похоже на модель **подписки**, для которой создаются плагины. Мы не будем углубляться в детали, поскольку это выходит за рамки данного руководства.

### Как кошельки облегчают взаимодействие со смарт-контрактами

Как мы уже говорили, смарт-контракт кошелька получает внешние сообщения, проверяет их и принимает, если все проверки пройдены. Затем контракт запускает цикл извлечения сообщений из тела внешних сообщений, после чего создает внутренние сообщения и отправляет их в блокчейн:
...
:::tip `touch()`
На TON все смарт-контракты выполняются в виртуальной машине TON (TVM), основанной на стековой процессорной архитектуре. `~ touch()` помещает переменную `cs` на вершину стека, чтобы оптимизировать выполняемый код для меньшего расхода газа.
:::

Поскольку в одной ячейке может храниться **максимум 4 ссылки**, то на одно внешнее сообщение мы можем отправить максимум 4 внутренних сообщения.
...
В этом разделе мы узнаем больше о `internal` (внутренних) и `external` (внешних) сообщениях, а также создадим сообщения и отправим их в сеть, чтобы свести к минимуму использование заранее подготовленных функций.

Чтобы выполнить этот процесс, необходимо воспользоваться готовым кошельком, который облегчит задачу. Для этого:
...
4. Отправьте сообщение на другой адрес (вы также можете отправить его себе на тот же кошелек).

Таким образом, приложение кошелька Tonkeeper развернет смарт-контракт кошелька, и мы сможем использовать его для следующих шагов.

:::note
На момент написания этой статьи большинство приложений-кошельков на TON по умолчанию используют 4 версию кошелька. В этом разделе руководства плагины не требуются, и мы будем использовать функциональность, предоставляемую кошельком V3. Tonkeeper во время использования позволяет пользователю выбрать нужную ему версию кошелька. Поэтому рекомендуется установить кошелек версии 3 (V3).
:::

### TL-B

Как уже отмечалось, все в блокчейне TON – это смарт-контракт, состоящий из ячеек. Чтобы правильно сериализовать и десериализовать данные, нам нужны стандарты. Для осуществления процесса сериализации и десериализации был создан `TL-B`, как универсальный инструмент для описания различных типов данных различными способами с различными последовательностями внутри ячеек.

В этом разделе мы рассмотрим [block.tlb](https://github.com/ton-blockchain/ton/blob/master/crypto/block/block.tlb). Этот файл будет очень полезен при дальнейшей разработке, поскольку в нем описывается, как должны быть собраны различные ячейки. В нашем конкретном случае он подробно описывает устройство внутренних и внешних сообщений.

:::info
В этом руководстве представлена основная информация. Для получения более подробной информации, пожалуйста, обратитесь к нашей [документации по TL-B](/v3/documentation/data-formats/tlb/tl-b-language).
:::

### CommonMsgInfo

Изначально каждое сообщение должно хранить `CommonMsgInfo` ([TL-B](https://github.com/ton-blockchain/ton/blob/24dc184a2ea67f9c47042b4104bbb4d82289fac1/crypto/block/block.tlb#L123-L130)) или `CommonMsgInfoRelaxed` ([TL-B](https://github.com/ton-blockchain/ton/blob/24dc184a2ea67f9c47042b4104bbb4d82289fac1/crypto/block/block.tlb#L132-L137)). Это позволяет нам определить технические детали, относящиеся к типу сообщения, времени сообщения, адресу получателя, техническим флагам и сборам.

Читая файл `block.tlb`, мы можем заметить три типа CommonMsgInfo: `int_msg_info$0`, `ext_in_msg_info$10`, `ext_out_msg_info$11`. Мы не будем вдаваться в подробности, описывая специфику TL-B-структуры `ext_out_msg_info`. Тем не менее, это тип внешних сообщений, которые смарт-контракт может отправлять для использования в качестве внешних логов. В качестве примера такого формата рассмотрите контракт [Избирателя](https://tonscan.org/address/Ef8zMzMzMzMzMzMzMzMzMzMzMzMzMzMzMzMzMzMzMzMzM0vF).

[Взглянув на TL-B](https://github.com/ton-blockchain/ton/blob/24dc184a2ea67f9c47042b4104bbb4d82289fac1/crypto/block/block.tlb#L127-L128), вы заметите, что **только CommonMsgInfo доступно при использовании с типом ext_in_msg_info**. Это происходит потому, что такие поля сообщения, как `src`, `created_lt`, `created_at` и другие, переписываются валидаторами во время обработки транзакций. В данном случае поле `src` в сообщении наиболее важно, поскольку при отправке сообщений отправитель неизвестен, и оно записывается валидаторами во время проверки. Это гарантирует, что адрес в поле `src` корректен и не может быть изменен.

Однако структура `CommonMsgInfo` поддерживает только спецификацию `MsgAddress`, но адрес отправителя обычно неизвестен, и требуется записать `addr_none` (два нулевых бита `00`). В этом случае используется структура `CommonMsgInfoRelaxed`, которая поддерживает адрес `addr_none`. Для `ext_in_msg_info` (используемых для входящих внешних сообщений) используется структура `CommonMsgInfo`, поскольку эти типы сообщений не используют отправителя и всегда используют структуру [MsgAddressExt](https://hub.com/ton/ton.blockchain/ton/blob/24dc184a2ea67f9c47042b4104bbb4d82289fac1/crypto/block/block.tlb#L100) (`addr_none$00` означает два нулевых бита), что означает отсутствие необходимости перезаписывать данные.
...
Внутренние сообщения используются для передачи сообщений между контрактами. При анализе различных типов контрактов (таких как [NFT](https://github.com/ton-blockchain/token-contract/blob/f2253cb0f0e1ae0974d7dc0cef3a62cb6e19f806/nft/nft-item.fc#L51-L56) и [Jetons](https://github.com/ton-blockchain/token-contract/blob/f2253cb0f0e1ae0974d7dc0cef3a62cb6e19f806/ft/jetton-wallet.fc#L139-L144)), которые отправляют сообщения, включающие написание контрактов, часто используются следующие строки кода:
...
Давайте сначала рассмотрим `0x18` и `0x10` (x - шестнадцатеричные), которые представляют собой шестнадцатеричные числа, расположенные следующим образом (учитывая, что мы выделяем 6 бит): `011000` и `010000`. Это означает, что приведенный выше код можно переписать следующим образом:
...
Теперь давайте подробно рассмотрим каждый вариант:

|   Параметр   |                                                                                                                                                                                                                                                                                                    Описание                                                                                                                                                                                                                                                                                                    |
| :----------: | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------: |
| IHR Disabled | В настоящее время данная опция отключена (это означает, что мы храним значение `1`), поскольку Мгновенная Маршрутизация Гиперкуба (Instant Hypercube Routing) реализована не полностью. Кроме того, она будет необходима, когда в сети появится большое количество [шардчейнов](/v3/concepts/dive-into-ton/ton-blockchain/blockchain-of-blockchains#many-accountchains-shards). Подробнее об опции IHR Disabled можно прочитать в [tblkch.pdf](https://ton.org/tblkch.pdf) (глава 2). |
|    Bounce    |     При отправке сообщений в процессе обработки смарт-контракта могут возникать различные ошибки. Чтобы избежать потери TON, необходимо установить опцию `Bounce` в значение `1` (true). В этом случае, если во время обработки транзакции возникнут ошибки в контракте, сообщение будет возвращено отправителю, и будет получено то же количество TON за вычетом комиссии. Подробнее о невозвращаемых сообщениях можно прочитать [здесь](/v3/documentation/smart-contracts/message-management/non-bounceable-messages).    |
|    Bounced   |                                                                                                                                                           Возвращаемые сообщения (Bounced) – это сообщения, которые возвращаются отправителю из-за ошибки, возникшей при обработке транзакции с помощью смарт-контракта. Эта опция сообщает о том, является ли полученное сообщение *вернувшимся*.                                                                                                                                                          |
...
- Мы указываем получателя и количество TON для отправки.

Теперь давайте посмотрим на оставшиеся строки кода:
...
|      Forwarding fee      |                                                                                                                            Комиссия за передачу сообщения. Подробнее об этом можно прочитать [здесь](/v3/documentation/smart-contracts/transaction-fees/fees-low-level#transactions-and-phases)                                                                                                                           |
| Logical time of creation |                                                                                                                                                                                        Время, затраченное на создание корректной очереди сообщений                                                                                                                                                                                        |
|   UNIX time of creation  |                                                                                                                                                                                                 Время, когда сообщение было создано в UNIX                                                                                                                                                                                                |
|        State Init        |          Начальное состояние. Код и исходные данные для развертывания смарт-контракта. Если бит установлен в `0`, это означает, что у нас нет State Init. Но если он установлен в `1`, то необходимо записать еще один бит, который будет указывать, хранится ли State Init в той же ячейке (0) или записан как ссылка (1).         |
|       Message body       | Тело сообщения. Эта часть определяет, как хранится тело сообщения. Иногда тело сообщения слишком велико, чтобы поместиться в само сообщение. В этом случае его следует хранить как **ссылку**, при этом бит устанавливается в `1`, чтобы показать, что тело сообщения используется в качестве ссылки. Если бит равен `0`, тело находится в той же ячейке, что и сообщение |

Значения, описанные выше (включая `src`), за исключением битов `State Init` и `Message Body`, перезаписываются валидаторами.

:::note
Если значение числа помещается в меньшее количество бит, чем установлено, то недостающие нули добавляются к левой части значения. Например, 0x18 умещается в 5 бит -> `11000`. Однако, как было сказано выше, требуется 6 бит, тогда конечный результат принимается `011000`.
:::

Далее мы приступим к формированию сообщения, которое будет отправлено с некоторым количеством toncoin на другой кошелек V3.
Для начала, допустим, пользователь хочет отправить 0.5 TON самому себе с текстом **Hello, TON!**. Чтобы узнать как отправить сообщение с комментарием обратитесь к [этому разделу документации](/v3/documentation/smart-contracts/func/cookbook#how-to-send-a-simple-message).
...
В соответствии с информацией, которую мы изучили ранее, создается `InternalMessage`:
...
Можно получить информацию о блоке genesis (`zero_state`) из [файла конфигурации](https://ton.org/global-config.json). Разбираться в сложностях и деталях этого не обязательно, но важно помнить, что по умолчанию значение `subwallet_id` равно `698983191`.
...
Нам нужно будет добавить вышеуказанное значение в `initial data` (первичные данные) контракта, поэтому переменную нужно сохранить следующим образом:
...
Теперь, когда у нас четко определены приватный и публичный ключи, а также subwallet_id, нам нужно скомпилировать код кошелька. Для этого воспользуемся кодом [wallet V3](https://github.com/ton-blockchain/ton/blob/master/crypto/smartcont/wallet3-code.fc) из официального репозитория.

Для компиляции кода кошелька необходимо использовать библиотеку [@ton-community/func-js](https://github.com/ton-community/func-js).
С помощью этой библиотеки можно скомпилировать код FunC и получить ячейку, содержащую этот код. Чтобы начать работу, необходимо установить библиотеку и сохранить (--save) ее в `package.json` следующим образом:
...
:::info
Ничего страшного, если ваш IDE-плагин конфликтует с `() set_seed(int) impure asm "SETRAND";` в файле `stdlib.fc`.
:::
...
Теперь давайте напишем код для компиляции нашего смарт-контракта и запустим его с помощью команды `npm run start:dev`:
...
После этого можно получить ту же самую ячейку (используя вывод в кодировке base64) с помощью кода нашего кошелька, используя другие библиотеки и языки:
...
|              library             |                          Библиотека, которая работает с [мастерчейном](/v3/concepts/dive-into-ton/ton-blockchain/blockchain-of-blockchains#masterchain-blockchain-of-blockchains) и может быть использована различными смарт-контрактами. Она не будет использоваться для кошелька, поэтому бит установлен в `0`. Информацию об этом можно найти в [tblkch.pdf](https://ton.org/tblkch.pdf) (1.8.4)                          |

Далее мы подготовим `initial data` (первичные данные), которые будут представлены в хранилище нашего контракта сразу после развертывания:
...
На этом этапе присутствует как `code` контракта, так и `initial data` (первичные данные). С помощью этих данных мы можем создать наш **адрес кошелька**. Адрес кошелька зависит от `State Init`, которое включает в себя код и первичные данные.
...
Развертывание с помощью внешних сообщений представлено здесь в основном в образовательных целях, на практике гораздо удобнее [разворачивать смарт-контракты через кошельки](/v3/guidelines/smart-contracts/howto/wallet#contract-deployment-via-wallet), что будет описано позже.
:::
...
Наконец-то, мы можем отправить наше сообщение в блокчейн, чтобы развернуть наш кошелек и использовать его.
...
:::info
Код контракта, который мы использовали – [верифицирован](https://tonscan.org/tx/BL9T1i5DjX1JRLUn4z9JOgOWRKWQ80pSNevis26hGvc=), вы можете посмотреть пример [здесь](https://tonscan.org/address/EQDBjzo_iQCZh3bZSxFnK9ue4hLTOKgsCNKfC8LOUM4SlSCX#source).
:::

## 💸 Работа со смарт-контрактами кошелька

После завершения первой половины этого руководства мы теперь гораздо лучше знакомы со смарт-контрактами кошелька и с тем, как они разрабатываются и используются. Мы узнали как разворачивать и уничтожать их, а также отправлять сообщения, не завися от предварительно настроенных библиотечных функций. Чтобы применить больше из того, что мы узнали выше, в следующем разделе мы сосредоточимся на создании и отправке более сложных сообщений.

### Отправка нескольких сообщений одновременно

Как вы, возможно, уже знаете, [одна ячейка может хранить до 1023 бит данных и до 4 ссылок](/v3/documentation/data-formats/tlb/cell-boc#cell) на другие ячейки. В первом разделе этого руководства мы подробно описали, как внутренние сообщения доставляются в "полном" цикле в виде ссылки и отправляются. Это означает, что внутри внешнего сообщения можно **хранить до 4 внутренних сообщений**. Это позволяет отправлять четыре сообщения одновременно.

Для этого необходимо создать 4 разных внутренних сообщения. Мы можем сделать это вручную или с помощью цикла – `loop`. Сначала нам нужно определить 3 массива: массив количества TON, массив комментариев, массив сообщений. Для сообщений нам нужно подготовить еще один массив – internalMessages.
...
[Режим отправки](/v3/documentation/smart-contracts/message-management/sending-messages#message-modes) для всех сообщений установлен на `mode 3`.  Однако, если требуются разные режимы, можно создать массив для выполнения различных целей.
...
Теперь давайте воспользуемся нашими знаниями из [второй главы](/v3/guidelines/smart-contracts/howto/wallet#-deploying-a-wallet), чтобы создать сообщение для нашего кошелька, которое может отправлять 4 сообщения одновременно:
...
Особенно нам необходимо детально разобраться в TL-B для [NFT Transfers](https://github.com/ton-blockchain/TEPs/blob/master/text/0062-nft-standard.md#1-transfer).
...
Теперь давайте завершим сообщение, как было изложено в предыдущих разделах этого руководства. Корректный код, необходимый для завершения сообщения, находится в [репозитории GitHub](/v3/guidelines/smart-contracts/howto/wallet#-source-code).
...
Смарт-контракты часто используют [GET-методы] (/v3/guidelines/smart-contracts/get-methods), однако они работают на стороне клиента, а не внутри блокчейна. GET-методы имеют множество применений и обеспечивают смарт-контрактам доступ к различным типам данных. Например, метод [get_nft_data() в смарт-контрактах NFT](https://github.com/ton-blockchain/token-contract/blob/991bdb4925653c51b0b53ab212c53143f71f5476/nft/nft-item.fc#L142-L145) позволяет пользователям получать информацию о владельце, коллекции NFT, содержимым `content`.

Ниже мы познакомимся с основами GET-методов, используемых в [V3](https://github.com/ton-blockchain/ton/blob/e37583e5e6e8cd0aebf5142ef7d8db282f10692b/crypto/smartcont/wallet3-code.fc#L31-L41) и [V4](https://github.com/ton-blockchain/wallet-contract/blob/4111fd9e3313ec17d99ca9b5b1656445b5b49d8f/func/wallet-v4-code.fc#L164-L198). Начнем с методов, которые одинаковы для обеих версий кошелька:

|                                       Метод                                       |                                                                                                                                                                      Описание                                                                                                                                                                      |
| :-------------------------------------------------------------------------------: | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------: |
|                           int seqno()                          |                                                                                      Необходим для получения текущего seqno и отправки сообщений с корректным значением. В предыдущих разделах руководства этот метод вызывался довольно часто                                                                                     |
...
| int is_plugin_installed(int wc, int addr_hash) | Данный метод нужен для передачи в блокчейн данных об установке плагина. Для вызова этого метода необходимо передать [воркчейн](/v3/concepts/dive-into-ton/ton-blockchain/blockchain-of-blockchains#workchain-blockchain-with-your-own-rules) и хэш адреса плагина. |
|                       tuple get_plugin_list()                       |                                                                                                                              Ввозвращает адрес установленных плагинов                                                                                                                              |

Давайте рассмотрим методы `get_public_key` и `is_plugin_installed`. Эти два метода были выбраны для того, чтобы сначала извлечь публичный ключ из данных длиной 256 бит, а затем научиться передавать *slice* – срез полученных данных, а также различные типы данных в GET-методы. Это очень удобно и должно помочь правильно пользоваться данными методами.
...
Теперь нам нужно получить хэш-адрес плагина, чтобы перевести его в число и отправить в GET-метод.
...
В третьей главе мы развернули кошелек: для этого мы отправили несколько TON, а затем сообщение из кошелька – для разворачивания смарт-контракта. Однако данный вариант не широко используется с внешними сообщениями и зачастую применяется только для кошельков. При разработке контрактов процесс развертывания инициализируется путем отправки внутренних сообщений.

Для этого мы воспользуемся смарт-контрактом кошелька V3R2, который использовался в [третьей главе](/v3/guidelines/smart-contracts/howto/wallet#compiling-wallet-code).
В этом случае мы установим `subwallet_id` значение `3` или любое другое число, необходимое для получения другого адреса при использовании того же приватного ключа (его можно менять):
...
Далее мы получим адрес из нашего контракта и создадим InternalMessage. Также мы добавим комментарий "Развертывание..." к нашему сообщению.
...
Далее мы подготовим сообщение для нашего кошелька и отправим его:
...
При работе с большим количеством сообщений за короткий промежуток времени возникает необходимость в специальном кошельке, называемом Highload Wallet (highload-кошелек). Highload-кошелек V2 долгое время был основным кошельком на TON, но с ним нужно было быть очень осторожным, вы можете [заблокировать все средства](https://t.me/tonstatus/88).

[С появлением highload-кошелька V3](https://github.com/ton-blockchain/highload-wallet-contract-v3) эта проблема была решена на уровне архитектуры, было снижено потребление газа. В этой главе мы рассмотрим основы highload-кошелька V3 и важные нюансы, о которых следует помнить.
...
Прежде всего, [TL-B schema](https://github.com/ton-blockchain/highload-wallet-contract-v3/blob/d58c31e82315c34b4db55942851dd8d4153975c5/contracts/scheme.tlb#L1C1-L3C21) поможет нам в изучении структуры хранилища контрактов:
...
В хранилище контрактов мы можем найти следующие поля:

|                            Поле                           |                                                                                                               Описание                                                                                                               |
| :-------------------------------------------------------: | :----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------: |
|              public_key              |                                                                                                       Публичный ключ контракта                                                                                                       |
|             subwallet_id             |                                            [Идентификатор subwallet](#subwallet-ids). Это позволит вам создать множество кошельков, используя один и тот же публичный ключ                                           |
...
Подробнее о работе с обработанными запросами мы поговорим в разделе [Защита от повторения](#replay-protection).
...
Основная идея заключается в том, что теперь каждый Query ID занимает всего 1 бит в словаре, при этом расход газа в большинстве случаев не увеличивается.
...
Если такая ячейка найдена, она пропускает `bit_number` битов, чтобы добраться до бита с индексом `bit_number` (важно понимать разницу между bit_number как количеством и bit_number как индексом). Если такой бит найден, это означает, что запрос с таким Query ID уже был обработан, и выдается ошибка:
...
Если такая ячейка найдена, контракт разрезает ее на 2 части: `0...bit_number-1` (head) и `bit_number...1023` (tail). Затем из начала tail считывается один бит (номер этого бита равен `bit_number`, если начать отсчет с 0, т.е. это индекс нужного бита). Если он положительный, то запрос с таким Query ID уже был обработан, и будет выдана ошибка. В противном случае бит устанавливается в 1, и все фрагменты снова объединяются в одну ячейку и записываются обратно в словарь `queries`:
...
Если вы [ознакомитесь](https://docs.ton.org/v3/documentation/tvm/instructions) с работой опкода `LDSLICEX` (функция `load_bits` использует этот опкод), вы заметите, что сначала возвращаются считанные данные (head), а затем уже оставшиеся данные (tail), но в коде контракта они расположены в обратном порядке.На самом деле, они идут в обратном порядке потому, что в `stdlib` в сигнатуре функции возвращаемые данные [идут в обратном порядке](https://github.com/ton-blockchain/highload-wallet-contract-v3/blob/d58c31e82315c34b4db55942851dd8d4153975c5/contracts/imports/stdlib.fc#L321): `(slice, slice) load_bits(slice s, int len) asm(s len -> 1 0) "LDSLICEX";`. Здесь `-> 1 0` означает, что сначала возвращается аргумент с индексом `1` (tail), а затем `0` (head).
:::

Таким образом, по сути, мы работаем с матрицей, где `shift` – это индекс строки, а `bit_number` – индекс столбца. Это позволяет нам хранить до 1023 запросов в одной ячейке, а значит, расход газа будет увеличиваться только каждые 1023 запроса из-за добавления новой ячейки в словарь. Важно понимать, что это произойдет, если значения будут расти последовательно, а не случайно, поэтому необходимо правильно увеличить Query ID, [используя для этого специальный класс](https://github.com/aSpite/highload-wallet-contract-v3/blob/main/wrappers/HighloadQueryId.ts).

Такой подход позволяет хранить огромное количество запросов за один таймаут (1023 \* 8192 = 8 380 416), но как можно заметить [класс HighloadQueryId поддерживает 8 380 415](https://github.com/ton-blockchain/highload-wallet-contract-v3/blob/d58c31e82315c34b4db55942851dd8d4153975c5/wrappers/HighloadQueryId.ts#L32). Это сделано для того, чтобы всегда оставался 1 бит для одного экстренного запроса на таймаут, если весь лимит будет исчерпан. Это значение установлено из-за [ограничения на максимально возможное количество ячеек в стеке аккаунта](https://github.com/ton-blockchain/ton/blob/5c392e0f2d946877bb79a09ed35068f7b0bd333a/crypto/block/mc-config.h#L395) в блокчейне (на момент написания статьи).
...
:::info
Ранее, в highload-кошельке V2, каждый Quiery ID (64-битный) хранился в отдельной ячейке словаря и представлял собой объединение 32-битных полей `expire_at` и `query_id`. Это приводило к очень быстрому росту потребления газа после очистки старых запросов.
:::

### Защита от повторения

Поскольку мы знаем, что внешние сообщения в TON [не имеют отправителя и могут быть отправлены любым человеком в сети](#replay-protection---seqno), важно иметь список обработанных запросов во избежание повторной обработки. Для этой цели highload-кошелек V3 использует словари `queries` и `old_queries`, а также значения `last_clean_time` и `timeout`.
...
Дело в том, что контракт не отслеживает, когда именно был выполнен запрос. Это означает, что если `timeout` равен 3 часам, но последний запрос был выполнен за минуту до истечения 3 часов, то запрос будет считаться устаревшим минутой позже, несмотря на 3-часовой таймаут. Чтобы решить эту проблему, во втором словаре хранятся те же самые запросы как минимум еще столько же времени.
...
Когда все проверки и очистка завершены, контракт может принять сообщение, внести изменения в свое хранилище и вызвать функцию `commit`, которая будет считать Фазу вычисления успешной, даже если в следующий момент произойдет какая-то ошибка:
...
Это сделано для того, чтобы при выполнении дальнейшего кода, если в сообщении, которое пытается отправить пользователь, произошла ошибка, и контракт не вернулся в предыдущее состояние. В противном случае внешние запросы останутся в силе и могут быть приняты несколько раз, что приведет к напрасной трате баланса.

Однако необходимо решить еще одну проблему – возможные ошибки во время **Фазы действий**. Хотя у нас есть флаг, позволяющий игнорировать ошибки (2) при отправке сообщения, он работает не во всех случаях, поэтому нужно убедиться, что во время этой фазы не возникнет ошибок, которые могут привести к откату состояния и сделать `commit()` бессмысленным.
...
При работе с `internal_transfer` есть один важный нюанс. Как мы уже обсуждали выше, контракт отправляет сообщение самому себе, но это сообщение полностью собирается на стороне пользователя. Проблема заключается в том, что вам необходимо правильно посчитать, сколько TON будет прикреплено к сообщению.
...
Представим, что мы хотим разослать большое количество токенов. После рассылки оставшиеся TON возвращаются на наш кошелек, поскольку мы указали наш адрес в поле `response_destination`. Мы начинаем рассылать несколько внешних сообщений одновременно и возникает следующая ситуация:

1. Внешнее сообщение A принимается, обрабатывается и отправляет весь баланс контракта через `internal_transfer`.
2. До того, как внешнее сообщение B дойдет до адресата, дойдет и часть комиссии от уже завершенной отправки токенов. Тем самым непустой баланс контракта позволяет снова отправить весь баланс во внутреннее сообщение B, но на этот раз отправляется очень малое количество TON.
3. Внутреннее сообщение A получено, обработано. Сообщения об отправке токенов отправлены.
4. Прежде чем внутреннее сообщение B дойдет до адресата, внешнее сообщение C успевает дойти и снова отправляет весь баланс.
5. При получении внутреннего сообщения B контракт имеет так мало TON, что даже если будет получено некоторое количество дополнительных TON от отправки токенов, запрос будет считаться неуспешным с кодом завершения = `37` на Фазе действий (недостаточно средств).

Таким образом, контракт показывает, что запрос был обработан, хотя на самом деле это не так. Чтобы избежать этого сценария, **рекомендуется всегда класть 1 TON** на `internal_transfer`. Поэтому [мы работаем с модифицированной оберткой](#-high-load-wallet-v3), которая требует от пользователя указывать количество TON. Этого значения будет достаточно для всех случаев, поскольку размер внешнего сообщения ограничен 64 КБ, и сообщение, близкое к этому пределу, потратит меньше 1 TON.
...
:::info
Хотя лимит внешних сообщений составляет 64 КБ, чем больше внешнее сообщение, тем больше вероятность того, что оно потеряется при доставке, поэтому 150 сообщений – это оптимальное решение.
:::
...
:::tip
Рекомендуется передавать `true` для `need_clean`, если ситуация не требует иного, поскольку в этом случае будут возвращены самые актуальные состояния словаря.
:::

Благодаря тому, как организован Query ID в Highload-кошельке V3, мы можем отправить сообщение с тем же Query ID еще раз, если оно не пришло, не опасаясь, что запрос будет задублирован.

Однако необходимо учитывать, что с момента первой попытки отправки должно было пройти не более `timeout` времени. В противном случае запрос может быть обработан, но уже удален из словарей. Поэтому рекомендуется устанавливать `timeout` не менее часа и не более 24 часов.
...
Если у вас кошелек версии V4, то вы можете использовать класс `WalletContractV4`. Теперь нам осталось только развернуть контракт:
...
Просмотрев выведенный в консоли адрес в блокчейн-обозревателе, мы можем убедиться, что наш кошелек развернут.
...
Теперь нам нужно создать экземпляр класса `HighloadQueryId`. Этот класс облегчает работу с `shift` и `bit_number`. Чтобы создать его, мы используем метод `fromShiftAndBitNumber`:
...
Как мы уже обсуждали ранее, seqno обычного кошелька после каждой транзакции увеличивается на `1`. При использовании последовательности кошельков нам приходилось ждать, пока это значение обновится, затем извлекать его с помощью GET-метода и отправлять новое сообщение.
...
После того, как контракт принял внешнее сообщение, начинается цикл, в котором берутся срезы – `slices`, хранящиеся в словаре. В этих срезах хранятся режимы сообщений и сами сообщения. Отправка новых сообщений происходит до тех пор, пока словарь не опустеет.
...
Обратите внимание, что если значение найдено, `f` всегда равно -1 (true). Операция `~ -1` (побитовое "не") всегда будет возвращать значение 0, что означает, что цикл следует продолжить. В то же время, когда словарь заполнен сообщениями, необходимо начинать вычисление тех из них, которые **имеют значение больше -1** (например, 0), и продолжать увеличивать это значение на 1 с каждым сообщением. Такая структура позволяет отправлять сообщения в правильной последовательности.

### Удаление просроченных запросов

Как правило, [смарт-контракты на TON сами оплачивают свое хранение](/v3/documentation/smart-contracts/transaction-fees/fees-low-level#storage-fee). Это означает, что объем данных, которые могут хранить смарт-контракты, ограничен для предотвращения высокой нагрузки на сеть. Чтобы система работала эффективнее, сообщения, возраст которых превышает 64 секунды, удаляются из хранилища. Это происходит следующим образом:
...
Обратите внимание, что с переменной `f` необходимо взаимодействовать несколько раз. Поскольку [TVM – это стековая машина](/v3/documentation/tvm/tvm-overview#tvm-is-a-stack-machine), то во время каждого взаимодействия с переменной `f` необходимо перебрать все значения, чтобы получить нужную переменную. Операция `f~touch()` помещает переменную `f` на вершину стека, чтобы оптимизировать выполнение кода.
...
В результате к числу справа добавляется 32 бита. Это означает, что **существующие значения сдвигаются на 32 бита влево**. Например, давайте рассмотрим число `3` и переведем его в двоичную форму с результатом `11`. Применяя операцию `3 << 2`, 11 сдвигается на 2 разряда. Это означает, что справа от строки добавляется два бита. В итоге мы получаем `1100`, что равно `12`.

В этом процессе необходимо запомнить, что функция `now()` возвращает результат uint32, что означает, что результирующее значение будет состоять из 32 битов. Сдвинув 32 бита влево, мы освобождаем место для еще одного uint32, в результате чего получаем правильный `query_id`. Таким образом, **timestamp и query_id могут быть объединены** в одной переменной для оптимизации.
...
Выше мы выполнили операцию сдвига числа 64 на 32 бита, чтобы **вычесть 64 секунды** из нашего timestamp. Таким образом, мы сможем сравнить прошлые `query_id` и посмотреть, не меньше ли они полученного значения. Если значение меньше, то срок действия истек более 64 секунд назад:
...
Чтобы лучше понять это, давайте воспользуемся числом `1625918400` в качестве примера `timestamp`. Его двоичное представление (с левосторонним добавлением нулей на 32 бита) – 01100000111010011000101111000000. Выполнив 32-битный сдвиг влево, вы получите 32 нуля в конце двоичного представления нашего числа.

После этого **можно добавить любой query_id (uint32)**. Затем, вычитая `64 << 32`, вы получите timestamp, который 64 секунды назад имел тот же самый query_id. Этот факт можно проверить, выполнив следующие вычисления `((1625918400 << 32) - (64 << 32)) >> 32`. Таким образом, мы можем сравнить необходимые части нашего числа (timestamp), и в то же время query_id не будет мешать.
...
|                                       Метод                                       |                                                                                                                                                   Описание                                                                                                                                                  |
| :-------------------------------------------------------------------------------: | :---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------: |
|        int processed?(int query_id)       | Уведомляет пользователя о том, что определенный запрос был обработан. Это означает, что он возвращает `-1`, если запрос был обработан, и `0`, если нет. Также этот метод может вернуть `1`, если ответ неизвестен, поскольку запрос старый и больше не хранится в контракте |
| int get_public_key() |                                                                                                               Извлекает публичный ключ. Мы уже рассматривали этот метод ранее                                                                                                               |

Давайте внимательно рассмотрим метод `int processed?(int query_id)`, чтобы понять, почему нам нужно использовать last_cleaned:
...
Значение `last_cleaned` извлекается из хранилища контракта и словаря старых запросов. Если запрос найден, то возвращается `true`, а если нет, то выражение `- (query_id <= last_cleaned)`. Last_cleaned содержит последний удаленный запрос **с наибольшим timestamp**, поскольку при удалении запросов мы начинали с наименьшего timestamp.

Это означает, что если `query_id`, переданный в метод, меньше последнего значения `last_cleaned`, то невозможно определить, был ли он когда-либо в контракте или нет. Поэтому `query_id <= last_cleaned` возвращает `-1`, а минус перед этим выражением меняет ответ на `1`. Если `query_id` больше `last_cleaned`, то метод еще не был обработан.

### Развертывание highload-кошелька V2

Для того чтобы развернуть highload-кошелек, необходимо заранее сгенерировать мнемонический ключ, который будет применяться пользователем. Можно использовать тот же ключ, что и в предыдущих разделах этого руководства.

Чтобы приступить к развертыванию highload-кошелька, необходимо скопировать [код смарт-контракта](https://github.com/ton-blockchain/ton/blob/master/crypto/smartcont/new-highload-wallet-v2.fif) в ту же директорию, где находятся stdlib.fc и wallet_v3, и не забыть добавить `#include "stdlib.fc";` в начало кода. Далее мы скомпилируем код highload-кошелька, как мы это делали в [третьем разделе](/v3/guidelines/smart-contracts/howto/wallet#compiling-wallet-code):
...
Теперь нам нужно получить ячейку, содержащую `initial data` (первичные данные), создать State Init и вычислить адрес highload-кошелька. После изучения кода смарт-контракта становится понятно, что `subwallet_id`, `last_cleaned`, `public_key` и `old_queries` последовательно сохраняются в хранилище:
...
:::caution
Все, что мы описали выше, соответствует тем же шагам, что и в разделе [развертывание контракта через кошелек](/v3/guidelines/smart-contracts/howto/wallet#contract-deployment-via-wallet). Для лучшего понимания ознакомьтесь с полной версией [исходного кода на GitHub](https://github.com/aSpite/wallet-tutorial).
:::
...
:::note ВАЖНО
Обратите внимание, что при использовании JavaScript и TypeScript наши сообщения сохранены в массив без использования режима отправки. Так происходит потому, что при использовании библиотеки @ton/ton предполагается, что разработчик будет выполнять процесс сериализации и десериализации собственноручно. Поэтому передается метод, который сначала сохраняет режим сообщения, а затем сохраняет само сообщение. Если мы используем спецификацию `Dictionary.Values.Cell()` для метода value, то в таком случае сообщение сохраняется целиком в виде ссылки на ячейку, не сохраняя отдельно режим сообщения.
:::
...
После завершения этого процесса можно посмотреть на наш кошелек и убедиться, что на него было отправлено 12 исходящих сообщений. Также можно вызвать GET-метод `processed?`, используя query_id, который мы первоначально использовали в консоли. Если этот запрос был обработан правильно, он выдаст результат `-1` (true).

## 🏁 Заключение

Это руководство позволило нам лучше понять, как различные типы кошельков работают в блокчейне TON. Мы изучили как создавать внешние и внутренние сообщения без использования предопределенных библиотечных методов.
...
Если у вас есть вопросы, комментарии или предложения, пожалуйста, свяжитесь с автором этого раздела документации в [Telegram](https://t.me/aspite) (@aSpite или @SpiteMoriarty) или [GitHub](https://github.com/aSpite).
...
- [Типы контрактов кошелька](/v3/documentation/smart-contracts/contracts-specs/wallet-contracts#wallet-v4)
