...
Мы научимся создавать операции без использования предварительно настроенных функций, для лучшего понимания процесса разработки. Дополнительные ссылки на материалы для изучения находятся в конце каждого раздела.

## 💡 Перед началом работы

Изучение данного руководства потребует базовых знаний JavaScript и TypeScript или Golang. На балансе кошелька должно быть как минимум 3 TON (это может быть биржевой счет, некастодиальный кошелек или бот Кошелек от Telegram). Для понимания также необходимо иметь представление о [ячейке](/v3/concepts/dive-into-ton/ton-blockchain/cells-as-data-storage), [адресах в TON](/v3/documentation/smart-contracts/addresses), [блокчейне блокчейнов](/v3/concepts/dive-into-ton/ton-blockchain/blockchain-of-blockchains).
...
**ВАЖНО:** В данном руководстве для лучшего понимания процесса разработки используется [код кошелька V3](https://github.com/ton-blockchain/ton/blob/master/crypto/smartcont/wallet3-code.fc). Следует отметить, что версия 3 имеет две ревизии: R1 и R2. В настоящее время используется только вторая ревизия, поэтому, когда мы ссылаемся на V3 в этом документе, это означает V3R2.
:::
...
Прежде чем мы перейдем к более подробной информации о кошельках, давайте рассмотрим, как кошельки принимают внешние сообщения. На TON все кошельки хранят `public key`, `seqno` и `subwallet_id` владельца. При получении внешнего сообщения кошелек использует метод `get_data()` для извлечения данных из части хранилища кошелька. Затем проводится несколько процедур верификации и определяется, принимать сообщение или нет. Это происходит следующим образом:
...
Для выполнения этого процесса кошелек должен сначала получить подпись из входящего сообщения, которое загружает публичный ключ из хранилища, а затем проверить ее с помощью следующих процедур:
...
Если все процедуры верификации завершены корректно, смарт-контракт принимает сообщение и обрабатывает его:
...
Этот алгоритм работает как защита от возникновения различных ошибок, например, когда сообщение уже недействительно, но все равно было отправлено в блокчейн по неизвестной причине.
...
В этом разделе мы познакомимся с устройством внутренних `internal` и внешних `external` сообщений, создадим и отправим их в сеть, при этом постараемся свести к минимуму использование заранее подготовленных функций.

Для облегчения задачи воспользуемся готовым кошельком. Для этого:
...
В результате приложение Tonkeeper развернет смарт-контракт кошелька, и мы сможем его использовать для следующих шагов.

:::note
На момент написания руководства большинство приложений-кошельков на TON по умолчанию используют кошелек V4. В этом разделе плагины не требуются, а значит будет достаточно функциональности кошелька V3. Tonkeeper позволяет пользователю выбрать нужную ему версию кошелька, поэтому рекомендуется развернуть кошелек V3.
:::

### TL-B

Как уже отмечалось, все в блокчейне TON – это смарт-контракт, состоящий из ячеек. Чтобы правильно сериализовать и десериализовать данные, нам нужны стандарты. Так был создан `TL-B` – универсальный инструмент для описания типов данных разными способами и с различными последовательностями внутри ячеек.

В этом разделе мы изучим [block.tlb](https://github.com/ton-blockchain/ton/blob/master/crypto/block/block.tlb). Этот файл будет полезен при дальнейшей разработке, поскольку в нем описывается, как должны быть собраны различные ячейки. В нашем конкретном случае там описывается устройство внутренних и внешних сообщений.
