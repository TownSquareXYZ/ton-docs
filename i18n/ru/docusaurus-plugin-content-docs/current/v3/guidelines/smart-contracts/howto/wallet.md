...
В этом разделе мы изучим схему данных [block.tlb](https://github.com/ton-blockchain/ton/blob/master/crypto/block/block.tlb). Этот файл будет полезен при дальнейшей разработке, поскольку в нем описывается, как должны быть собраны различные ячейки. В нашем конкретном случае там описывается устройство внутренних и внешних сообщений.
...
Читая файл `block.tlb`, можно заметить три типа CommonMsgInfo: `int_msg_info$0`, `ext_in_msg_info$10`, `ext_out_msg_info$11`. Если не углубляться в детали, то конструктор `ext_out_msg_info` – это внешнее сообщение, которое отправляется смарт-контрактом для использования в качестве внешних логов. Как пример подобного формата изучите смарт-контракт [Избирателя](https://tonscan.org/address/Ef8zMzMzMzMzMzMzMzMzMzMzMzMzMzMzMzMzMzMzMzMzM0vF).
