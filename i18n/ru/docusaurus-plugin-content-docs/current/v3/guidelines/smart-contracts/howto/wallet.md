...
**ВАЖНО:** В данном руководстве используется [код кошелька V3](https://github.com/ton-blockchain/ton/blob/master/crypto/smartcont/wallet3-code.fc). Следует отметить, что версия 3 имеет две ревизии: R1 и R2. В настоящее время используется только вторая ревизия, поэтому, когда мы ссылаемся на V3, это означает V3R2.
:::

## 💎 Кошельки TON Blockchain

Все кошельки, которые функционируют и запускаются на блокчейне TON, на самом деле являются смарт-контрактами, точно так же, как и все, что работает на TON, является смарт-контрактом. Как и в большинстве блокчейнов, в сети можно разворачивать смарт-контракты и настраивать их для различных целей. Благодаря этой особенности возможна **полная кастомизация кошелька**. В TON смарт-контракты кошелька помогают платформе взаимодействовать с другими типами смарт-контрактов. Однако важно учитывать, как происходит данное взаимодействие.
...
Защита от повторения сообщений в смарт-контракте кошелька напрямую связана с сообщением `seqno` (Sequence Number) – порядковым номером, который учитывает, какие сообщения отправляются и в каком порядке. Важно не повторять ни одно сообщение из кошелька, поскольку это нарушит целостность системы. Если изучить код смарт-контракта кошелька, то `seqno` обычно обрабатывается следующим образом:

```func
throw_unless(33, msg_seqno == stored_seqno);
```

Строка кода выше проверяет `seqno`, пришедшее в сообщении, и сверяет его с `seqno`, которое хранится в смарт-контракте. Если seqno не совпадают, то контракт возвращает ошибку с кодом завершения `33`. Таким образом, если отправитель передал недействительное seqno, это означает, что он допустил какую-то ошибку в последовательности сообщений, и смарт-контракт защищает его от таких случаев.
