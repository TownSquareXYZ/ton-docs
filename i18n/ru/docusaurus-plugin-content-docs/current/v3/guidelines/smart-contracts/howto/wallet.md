...
В этом разделе мы будем работать со схемой данных [block.tlb](https://github.com/ton-blockchain/ton/blob/master/crypto/block/block.tlb). Этот файл будет полезен при дальнейшей разработке, поскольку в нем описывается, как должны быть собраны различные ячейки. В нашем конкретном случае там описывается устройство внутренних и внешних сообщений.
...
В схеме [TL-B указано](https://github.com/ton-blockchain/ton/blob/24dc184a2ea67f9c47042b4104bbb4d82289fac1/crypto/block/block.tlb#L127-L128), что **при использовании ext_in_msg_info доступен только CommonMsgInfo**. Это происходит потому, что такие поля сообщения, как `src`, `created_lt`, `created_at` и другие, переписываются валидаторами во время обработки транзакций. В данном случае поле `src` в сообщении наиболее важно, поскольку при отправке сообщений отправитель неизвестен, и значение данного поля записывается валидаторами во время верификации. Это гарантирует, что адрес в поле `src` корректен и не может быть изменен.
