...
Информацию о блоке genesis (`zero_state`) можно получить из [файла конфигурации](https://ton.org/global-config.json). Разбираться в сложностях и деталях этого не обязательно, но важно помнить, что по умолчанию значение `subwallet_id` равно `698983191`.
...
После завершения первой половины этого руководства, мы гораздо лучше знакомы со смарт-контрактами кошелька и с тем, как они разрабатываются и используются. Мы узнали как разворачивать и уничтожать их, и как отправлять сообщения без использования заранее настроенных библиотечных функций. Следующий раздел будет посвящен созданию и отправке более сложных сообщений, чтобы применить то, что мы узнали выше.
