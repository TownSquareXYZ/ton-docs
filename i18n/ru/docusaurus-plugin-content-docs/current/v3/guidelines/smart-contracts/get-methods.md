...
:::note
Прежде чем продолжить, рекомендуется ознакомиться с основами [языка программирования FunC](/v3/documentation/smart-contracts/func/overview) на блокчейне TON. Это поможет лучше понять представленную информацию.
:::

## Введение

Get-методы - это специальные функции в смарт-контрактах, которые используются для получения определённых данных из них. Они выполняются вне блокчейна и не требуют комиссии.

Эти функции очень часто используются во многих смарт-контрактах. Например, стандартный [контракт Кошелька](/v3/documentation/smart-contracts/contracts-specs/wallet-contracts) содержит несколько get-методов, таких как `seqno()`, `get_subwallet_id()` и `get_public_key()`. Они применяются в кошельках, SDK и API для получения данных о кошельках.
