...
2. **Получение агрегированных данных.** Ещё один распространённый шаблон - методы, которые извлекают сразу несколько значений из состояния контракта за один вызов. Это удобно, если данные часто используются вместе. Такие методы применяются, например, в контрактах [Jetton](#jettons) и [NFT](#nfts).
...
1. **Получение вычисляемых данных.** В некоторых случаях данные не хранятся напрямую в состоянии контракта, а вычисляются на основе текущего состояния и входных аргументов.
...
2. **Получение данных с учетом условий.** Иногда данные можно извлечь только при выполнении определённых условий, например, в зависимости от текущего времени.
...
Возвращает данные главного контракта жетона, включая общий объём, адрес администратора, содержимое жетона и код кошелька.
...
С учетом индекса и [индивидуального содержимого NFT](#get_nft_data), этот метод получает и возвращает объединённые общие и индивидуальные данные NFT.
...
Этот метод возвращает одно число из данных контракта.
...
Сначала добавьте специальный метод в обертку контракта, который будет выполнять get-метод и возвращать типизированный результат. Допустим, ваш контракт называется *Counter*, и в нем уже реализован метод обновления числа. Откройте `wrappers/Counter.ts` и добавьте следующий метод:
...
Этот метод выполняет get-запрос и получает результирующий стек. В контексте get-методов стек - это данные, которые метод возвращает. В данном примере кода мы считываем из него одно число. В более сложных случаях, когда возвращается сразу несколько значений, можно просто вызвать методы типа `readSomething` несколько раз, чтобы обработать весь результат.
...
В этом примере контракт получает и обрабатывает внутренние сообщения, распознает коды операций, выполняет нужные методы и отправляет соответствующие ответы:
...
Для простоты мы использовали в качестве кодов операций простые числа 1, 2 и 3. Но для реальных проектов лучше использовать стандартные значения:
...
Get-методы - это важный инструмент для получения данных из смарт-контрактов в блокчейне TON. Хотя у них есть свои ограничения, их понимание и умение обходить - ключ к эффективному использованию get-методов в ваших смарт-контрактах.
