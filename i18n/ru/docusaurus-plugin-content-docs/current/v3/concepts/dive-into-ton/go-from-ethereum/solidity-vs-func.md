...
В следующих разделах будут кратко проанализированы следующие аспекты этих языков, а именно типы данных, хранилище, функции, структуры управления потоком и словари (хэш-карты).

## Схема хранения

Solidity обеспечивает плоскую модель хранения, которая означает, что все переменные состояния хранятся в едином непрерывном блоке памяти, называемом хранилищем. Хранилище представляет собой хранилище значений ключей, где каждый ключ представляет собой 256-битное (32-байтовое) целое число, представляющее номер ячейки хранения, а каждое значение - это 256-битное слово, хранящееся в этой ячейке. Ячейки нумеруются последовательно, начиная с нуля, и в каждой ячейке может храниться одно слово. Solidity позволяет программисту указать структуру хранилища, используя ключевое слово storage для определения состояния переменных. Порядок, в котором определяются переменные, определяет их положение в хранилище.
